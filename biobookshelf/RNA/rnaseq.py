...
def STAR_Index(
    ...
    if os.path.exists(f"{dir_folder_index}SA"):  # exit if STAR index exists
        print(
            f"[STAR_Index] it appears that STAR index at {dir_folder_index} already exists"
        )
        return -1

    run_star = subprocess.run(
        [
            "STAR",
            "--runThreadN",
            str(int(n_threads)),
...
def STAR_Align(
    dir_folder_index,
    dir_file_read_1,
    dir_file_read_2=None,
    dir_prefix_output=None,
    n_threads=10,
    index_bam=True,
    star_option=None,
    return_bash_script=False,
):
    """
    # 2021-07-05 21:34:33
    ...
    if dir_prefix_output is None:  # set default prefix for STAR output
        os.makedirs(
            f"{dir_folder_read}STAR_out/", exist_ok=True
    ...
    if (
        dir_file_read_1.rsplit(".", 1)[1].lower() == "gz"
    ):  # if given input file containing reads is gzipped
    ...
    dir_file_bam = (
        f"{dir_prefix_output}Aligned.sortedByCoord.out.bam"  # output file name
    )
    # return bash script
    if return_bash_script:
        return " ".join(l_args) + (
            f" && samtools index {dir_file_bam}" if index_bam else ""
        )
...
def Coverage(dir_file_bam, thres_mapq=200):
    ...
    with pysam.AlignmentFile(dir_file_bam, "rb") as samfile:
        ...
        dict_refname_to_cov = dict(
            (r["SN"], np.zeros(r["LN"], dtype=int)) for r in sam_header.to_dict()["SQ"]
        )  # dictionary containing arrays containing coverages
        ...
        for r in samfile.fetch(until_eof=True):  # for all records in the given BAM file
            if r.mapq > thres_mapq:  # for record with mapq above the given threshold
                for start, end in r.blocks:
                    dict_refname_to_cov[r.reference_name][
                        start:end
                    ] += 1  # increase coverage of aligned regions
...
def Variant_Calling(
    ...
    str_seq_ref = dict_genome[r.reference_name][
        r.reference_start : r.reference_start + r.alen
            ...
            if str_oper == "M":
                for _ in range(n_bases):
                    ...
                    if str_base_read != str_base_ref:
                        id_mut = (
                            f"{r.reference_name}:{r.reference_start + 1 + pos_ref}_X_{str_base_read}"
                            if flag_return_as_string
                            else (
                                r.reference_name,
                                r.reference_start + pos_ref,
                                "X",
                                str_base_read,
                                str_qual_read,
                            )
                        ...
                        id_mut = (
                            f"{r.reference_name}:{r.reference_start + 1 + pos_ref}_M_{str_base_read}"
                            if flag_return_as_string
                            else (
                                r.reference_name,
                                r.reference_start + pos_ref,
                ...
                id_mut = (
                    f"{r.reference_name}:{r.reference_start + 1 + pos_ref}_I_{r.seq[ pos_read : pos_read + n_bases ]}"
                    if flag_return_as_string
                    else (
                        r.reference_name,
                        r.reference_start + pos_ref,
                        "I",
                        r.seq[pos_read : pos_read + n_bases],
                        r.query_qualities[pos_read : pos_read + n_bases],
                    )
                ...
                if not flag_ignore_indel and (
                    ...
                    l_mut.append(id_mut)
                    str_seq_corrected_read += r.seq[
                        pos_read : pos_read + n_bases
                    ]  # add inserted sequence (if insertion is valid)
                pos_read += n_bases
            elif str_oper == "D":
                id_mut = (
                    f"{r.reference_name}:{r.reference_start + 1 + pos_ref}_D_{n_bases}"
                    if flag_return_as_string
                    else (
                        r.reference_name,
                        r.reference_start + pos_ref,
                ...
                if not flag_ignore_indel and (
                    ...
                    str_seq_corrected_read += str_seq_ref[

...
from scipy.stats import (
    fisher_exact,
)  # usage : For GESA p_value, stats.fisher_exact([[80, 2000], [80, 200]])
...
from scipy.spatial.distance import (
    pdist,
)  # c, coph_dists = cophenet(Z, pdist(X)) #compares (correlates) the actual pairwise distances of all your samples to those implied by the hierarchical clustering.

import umap
from sklearn.decomposition import (
    PCA,
    FactorAnalysis,
)  # import modules for multi-dimentional data visualization (t-SNE)
...
def Jupyter_Notebook_Extension__Build_Snippet(str_code, str_name, indentation=2):
    ...
    str_json_block = (
        ...
        + '{str_indent}    "name" : "{str_name}",\n{str_indent}    "snippet" : {str_code_formatted}\n'.format(
            ...
            str_name=str_name,
            str_code_formatted=str_code_formatted,
        )
        + str_indent
        + "},\n"
    )
...
known_reference_genes = [
    ...
    5430,
    7846,
]  # List_Gene__2__List_Gene_ID( ['ACTB', 'TUBB', 'PARK7', 'ACTA2', 'GAPDH', 'RPL32', 'EF1A', 'HPRT', 'POLR2A', 'TUBA1A' ] )
known_reference_genes_including_predicted = [
    60,
    203068,
    11315,
    59,
...
def S3_ls(path_s3url_folder, flag_recursion=False):
    ...
    l_l = list(
        line.split()
        for line in OS_Run(["aws", "s3", "ls", path_s3url_folder], return_output=True)[
            "stdout"
        ]
        .strip()
        .split("\n")
    ...
    try:
        df_ls = pd.read_csv(
            StringIO(
                "\n".join(
                    list(
                        "\t".join(["", "", "", l[1]] if len(l) == 2 else l) for l in l_l
                    )
                )
            ),
            sep="\t",
    ...
    df_ls["path_s3url"] = (
        path_s3url_folder + df_ls["file_name"]
    )  # retrieve s3 object URL
    ...
    if flag_recursion:
        ...
        for name_folder in list(e for e in df_ls.file_name.values if e[-1] == "/"):
            l_df_ls.append(
                S3_ls(
                    f"{path_s3url_folder}{name_folder}", flag_recursion=flag_recursion
                )
            )
...
def Program__Get_Absolute_Path_of_a_File(path_file):
    ...
    if path_file[0] == "/":
        return path_file  # if path_file is an absolute path, return path_file without modification
    current_working_directory = os.getcwd() + "/"
    if path_file[:2] == "./":
        return current_working_directory + path_file[2:]
    else:
        return current_working_directory + path_file
...
def GLOBALS_Find_object(query=None, case_specific=False):  # 20190903
    ...
    if not case_specific:  # perform case-insensitive search
        _, mask_matched = (
            Search_list_of_strings(
                list(map(str.upper, arr_object_names)),
                query=query.upper(),
                return_mask_matched=True,
            )
    ...
    return list(
        Search_list_of_strings(arr_object_names, query=query)
        if query is not None
        else l_object_names
    )
...
an_intercept_vector = np.array(
    [[0.0], [0.0]]
)  # an empty 2D vector then will store the value of an intercept and become an intercept vector # for LINREGRESS
bbox_props_white = dict(
    boxstyle="round", fc="w", ec="0.5", alpha=0.9
)  # define box properties that will be used in a function below for annotation
...
def INTEGER_Spread(n_values):  # 2020-08-07 01:34:56
    ...
    while len(l_encoding) < n_values:  # until all integers are assigned
        ...
        if int_encoding >= n_values:  # detect the end of cycle, and start new cycle
            ...
            index_cycle += 1
            step = max(
                1, int(n_values / math.pi ** (index_cycle / 2))
            )  # use irrational number to define step for each cycle to spread integers more evenly # minimum step = 1INTEGER_Spread
            continue
        elif int_encoding in l_encoding:
            continue
...
def INT_Get_Ranges_from_List_of_Integers(l, flag_sorted=True):
    ...
    if not flag_sorted:  # sort the list if 'flag_sorted' is not True
        l = np.sort(l)
    diff_l = np.diff(l)
    l_index_break_points = (
        [-1] + list(np.where(diff_l != 1)[0]) + [len(diff_l)]
    )  # identify start positions of break points
    l_range = list()
...
def BASE(number, base, n_numbers=None):  # 2020-07-25 16:08:23
    ...
    while True:
        ...
        if quotient == 0:
            break
    if (
        n_numbers is not None and len(l_value) < n_numbers
    ):  # if 'n_numbers' is given and length of the output list is smaller than 'n_numbers', pad zeros and return a list of integers with the length of 'n_numbers'
        for index in range(n_numbers - len(l_value)):
            l_value.append(0)
    return l_value[::-1]
...
def Decode_List_of_Strings(
    str_encoded, chr_separator=";", chr_representing_repeated_string="="
):  # 2020-08-12 22:16:00
    ...
    for entry in str_encoded.split(chr_separator):
        if len(entry) == 0:
            l_value.append(np.nan)
        elif entry == chr_representing_repeated_string:
            l_value.append(entry_previous)
        else:
            l_value.append(entry)
            entry_previous = entry
...
def ASCII_Decode(
    ...
    if l_ascii_to_exclude is not None:
        set_ascii_to_exclude = set(
            ascii_to_exclude
            for ascii_to_exclude in l_ascii_to_exclude
            if ascii_to_exclude - ascii_min >= 0 and ascii_to_exclude - ascii_min < base
        )  # retrieve set of ascii code to exclude in the encoding
    n_levels = (
        (base - len(set_ascii_to_exclude)) ** n_char - 1
        if n_char > 1
        else base - len(set_ascii_to_exclude) - 1
    )  # last level is for interpreting invalid value # ascii characters in l_ascii_to_exclude will be excluded in when interpreting ascii characters as levels
    step = (value_max - value_min) / n_levels  # amount of an increment for each level
    arr_ascii_to_value = np.full(
        base**n_char, np.nan
    )  # any ascii character outside the range of 'ascii_min' and 'ascii_max' and in 'set_ascii_to_exclude' will be considered as an invalid value (np.nan)
    value = value_min
    for index in range(
        len(arr_ascii_to_value) - 1
    ):  # last level is for interpreting invalid value (np.nan)
        set_ascii_used_for_representation = set(
            number + ascii_min for number in BASE(index, base, n_char)
        )
        if (
            sum(
                ascii_to_exclude in set_ascii_used_for_representation
                for ascii_to_exclude in set_ascii_to_exclude
            )
            > 0
        ):
            continue
        ...
        value += step
    bool_flag_input_is_dict = isinstance(
        l_seq, dict
    )  # booL_flag indicating whether input is given as a dictionary datatype
    l_arr_value = dict() if bool_flag_input_is_dict else list()
    for seq in l_seq:  # for each sequence
        if bool_flag_input_is_dict:
            key, seq = (
                seq,
                l_seq[seq],
            )  # if dictionary has been given, retrieve key and sequence accordingly.
        len_seq = len(seq)
        ...
        for index in range(0, len_seq, n_char):
            ...
            if n_char > 1:
                ...
                int_ascii_encoding = ord(str_ascii) - ascii_min
            arr_value[int(index / n_char)] = arr_ascii_to_value[int_ascii_encoding]
        if bool_flag_input_is_dict:
            l_arr_value[key] = arr_value
        else:
...
def ASCII_Encode(
    ...
    str_ascii_encoding_representing_invalid_value = (
        chr(ascii_max) * n_char
    )  # an ascii_encoding string representing an invalid value
    arr_level_to_ascii_encoding = np.full(
        n_levels + 1, str_ascii_encoding_representing_invalid_value
    )  # initialize 'arr_level_to_ascii_encoding' filled with ascii_encoding representing an invalid value
    l_ascii = list(
        chr(value)
        for value in range(ascii_min, ascii_max + 1, 1)
        if value not in set_ascii_to_exclude
    )  # list of valid ascii characters excluding 'set_ascii_to_exclude'
    for level in range(
        n_levels
    ):  # encode each level with available ascii characters (valid ascii characters, 'l_ascii')
        if n_char == 1:
            arr_level_to_ascii_encoding[level] = l_ascii[level]
        else:
            arr_level_to_ascii_encoding[level] = "".join(
                list(l_ascii[number] for number in BASE(level, len(l_ascii), n_char))
            )  # encode each level with available ascii characters (valid ascii characters, 'l_ascii')
    arr_level_to_ascii_encoding[-1] = arr_level_to_ascii_encoding[
        -2
    ]  # max + 1 level encode the same level as the max level
    l_seq = (
        dict(
            (
                l_value,
            ...
            for l_value in l_l_value
        )
        if isinstance(l_l_value, dict)
        else list(
            "".join(
                list(
                    (
    ...
    )  # prepare the output according to the input datatype
    return l_seq
...
def PARSE_Empty_Space_Delimited_with_Quotations(line):  # 2020-07-17 01:04:25
    ...
    bool_flag_the_end_of_quotation = (
        False  # only true right after the end of a quotation
    )
    str_value = ""
    l_value = list()
    for character in line.strip():
        if char_quotation_mark is None:  # if quotation has not been encountered
            ...
            if (
                bool_flag_the_end_of_quotation
            ):  # if flag indicating the potential end of quotation has been raised
                if character == " ":  # confirm the end of quotation
                    bool_flag_the_end_of_quotation = False
                    l_value.append(str_value)
                    str_value = ""
                    char_quotation_mark = None
                elif character == char_quotation_mark:
                    str_value += character  # false positive (the flag still raised)
                else:
                    bool_flag_the_end_of_quotation = (
                        False  # false positive (the flag is withdrawn)
                    )
                    str_value += character
            elif (
                character == char_quotation_mark
            ):  # raise a flag indicating the potential end of quotation
    ...
    l_value.append(str_value)
    return list(
        value for value in l_value if len(value) > 0
...
def DICT_Apply(
    dict_data, Function=None, inplace=False, return_series=True, name_series=None
):
    """# 2021-04-30 18:07:15
    ...
    if len(dict_value) != 0:  # if the dictionary is not empty
        for key in dict_value:
            ...
            if value_min is None:
                ...
                value_min == value
            ):  # if the another key contains the current min value, add the key to the list of keys with min values
                l_key_min.append(key)
...
def DICTIONARY_Merge_Min(*l_dict):
    ...
    for dict_to_be_merged in l_dict[1:]:
        for key in dict_to_be_merged:
            dict_merged[key] = (
                min(dict_merged[key], dict_to_be_merged[key])
                if key in dict_merged
                else dict_to_be_merged[key]
            )
    return dict_merged
...
def DICTIONARY_Merge_Max(*l_dict):
    ...
    for dict_to_be_merged in l_dict[1:]:
        for key in dict_to_be_merged:
            dict_merged[key] = (
                max(dict_merged[key], dict_to_be_merged[key])
                if key in dict_merged
                else dict_to_be_merged[key]
            )
    return dict_merged
...
def DICTIONARY_Filter(dict_data, above=None, below=None, include=True):
    ...
    if above is None and below is None:
        return (
            dict_data  # if no threshold is given, return the given dictionary as-it-is
        )
...
def UTIL_Assign_integer_rank_using_hashing(arr):
    ...
    return (
        pd.Series(dict((entry, entry.__hash__()) for entry in set(arr)))
        .argsort()
        .to_dict()
    )
...
def ENSEMBL_remove_version_info(list_ensembl_ID):
    ...
    return list(
        ensembl_ID.split(".")[0] if "." in ensembl_ID else ensembl_ID
        for ensembl_ID in list_ensembl_ID
    )
...
def LIST_intersection_with_set(a_list, a_set):
    ...
    if (
        type(a_list) is np.ndarray
    ):  # if received array is numpy array, also return numpy array
...
def SET_A_B_Intersection(
    arr_A, arr_B, return_intersection=False, label_A="A", label_B="B"
):
    set_A, set_B = set(arr_A), set(arr_B)
    print(
        "{label_A} = {n_A}, {label_B} = {n_B}, {label_A} ∩ {label_B} = {n_intersection}".format(
            label_A=label_A,
            label_B=label_B,
            n_A=len(set_A),
            n_B=len(set_B),
...
def SET_A_B_union_intersec_diff(A, B):
    ...
    print(
        "union, intersection : {}, {}".format(len(A.union(B)), len(A.intersection(B)))
    )
    print(
        "difference of A and B : {}, {}".format(
    ...
    if tree == None or len(tree) == 0:
        ...
        for key, val in tree.items():
            if isinstance(val, dict):
                ...
                if (
                    type(val) is set or type(val) is list
                ):  # if leaf node is set or list type, print the number of items
                    if print_elements:
                        print("\t" * d, key)
                        for element in val:
                            print("\t" * (d + 1), element)
...
def PHOSPHOPROTEOME_Gene_phosphosites_display(Gene, list_samples=None):
    ...
    if (
        not Gene_ID in df_phosphoproteome.Gene_ID.values
    ):  # Check whether the given Gene_ID is valid
        print("Invalid gene")
        return
    ...
    Gene_data = df_phosphoproteome[df_phosphoproteome.Gene_ID == Gene_ID][
        list_samples
    ]  # data of list_samples of df_phosphoproteome for a given Gene
    indices = Gene_data.index.values.tolist()
    list_num_samples = list(
        len(list_samples) - np.sum(np.isnan(Gene_data), axis=1).values
    )
    return pd.DataFrame(
        dict(indices=indices, number_of_samples=list_num_samples)
    ).set_index(
        "indices"
...
def Gene_2_Gene_ID(Gene):
    ...
    if type(Gene) == str:
        ...
        if Gene in dict_Symbol_ID_simple:
            ...
            Gene.upper() in dict_Symbol_ID_simple
        ):  # try again after converting lower case characters into upper clase characters
...
def List_Gene_ID__2__List_Gene_Symbol(
    list_Gene_ID,
    add_gene_name=False,
    return_sorted=False,
    limit_gene_name_len=35,
    return_mask_mapped=False,
):
    ...
    for Gene_ID in list_Gene_ID:  # for each Gene_ID
        if Gene_ID not in dict_ID_Symbol_simple:
            ...
            if add_gene_name:  # convert to Geme_Symbol + Gene_Name
                ...
                if (
                    len(gene_name) > limit_gene_name_len
                ):  # if length of gene_name exceed limit_gene_name_len, cut and add '..'
                    gene_name = gene_name[: limit_gene_name_len - 2] + ".."
                Gene_label = (
                    gene_name + " (" + dict_ID_Symbol[Gene_ID][2] + ")"
                )  # "Gene_Name (Geme_Symbol)"
    ...
    if return_sorted:
        ...
        return np.array(List_Symbol, dtype=object), np.array(
            List_mask_mapped, dtype=bool
        )  # return list of symbols and mask of mapped symbols
    else:
        return List_Symbol
...
def List_Gene_Symbol__2__List_Gene_IDs(list_Gene_Symbols):
    ...
    for Gene_Symbol in list_Gene_Symbols:  # for each Gene_Symbol
        if Gene_Symbol in dict_Symbol_ID_simple:  # when Gene_Symbol is valid
            ...
            print(
                Gene_Symbol, "not exist in the annotation dictionary"
            )  # if Symbol is invalid, report the symbol
...
def List_Gene_Symbol__2__List_Gene_IDs(list_Gene_Symbols, return_mask_mapped=False):
    ...
    if return_mask_mapped:
        return np.array(List_IDs, dtype=object), np.array(
            List_mask_mapped, dtype=bool
        )  # return list of symbols and mask of mapped symbols
    else:
        return List_IDs
...
def List_Gene_ID_Phosphosite__2__List_Gene_IDs(list_Gene_ID_Phosphosites):
    ...
    return list(
        float(Gene_ID_Phosphosite.split("|")[0])
        for Gene_ID_Phosphosite in list_Gene_ID_Phosphosites
    )
...
def SEARCH_gene_symbol_name(
    ...
    if query_Symbol is not None:
        ...
        return -1
    search_result, list_mask_matched = Search_list_of_strings(
        search_list, query=query, return_mask_matched=True
    )
    if (
        negative_query is not None
    ):  # if negative query has been given, remove entries that were matched with the negative query
        search_result_neg_query, list_mask_matched_neg_query = Search_list_of_strings(
            search_list, query=negative_query, return_mask_matched=True
        )
...
def GET_list_Gene_IDs_Phosphosites__from__list_Gene_IDs(Gene_IDs, print_messages=False):
    ...
    for Gene_ID in Gene_IDs:
        if Gene_ID in dict_Gene_ID__list_phosphoproteome_indices:
            list_phosphoproteome_indices.extend(
                dict_Gene_ID__list_phosphoproteome_indices[Gene_ID]
            )
        elif print_messages:
...
def non_nan_percentage(arr):
    ...
    print(
        ...
        length_arr,
        ", non-NaN percantage :",
        round(num_non_nan / length_arr * 100, 2),
        "%",
    )
...
def non_nan_percentages(arr):
    ...
    num_non_nan = np.sum(
        ~np.isnan(arr), axis=1
    )  # the number of non-nan elements in the array
...
def common_non_NaN_values(data_1, data_2):
    ...
    return (
        data_1[both_non_NaN],
        data_2[both_non_NaN],
    )  # return data where only pairs with non NaN values are retained
...
def get_data__tumor_grades(gene_id, df=None):
    ...
    return dict(
        G1=G1_data, G2=G2_data, G3=G3_data, G4=G4_data
    )  # return dictionary of data of genes
...
def GENE_SET_Read_GMT(path_file):
    ...
    dict_gene_sets = dict(
        (e[0], e[2:]) for e in list(line.split("\t") for line in l_line)
    )
    return dict_gene_sets  # return dict_gene_sets


def FIND_Gene_Sets__with_Gene_or_Gene_Set_Name(
    dict_Gene_Sets, Gene=None, Gene_Set_Name=None
):
    ...
    if Gene is not None:
        ...
        for (
            ...
            dict_Gene_Sets
        ):  # for each Gene_Set in a dict_Gene_Sets, check whether a given Gene_ID exist in the Gene_Set
...
def GET_series_sample_data_from_series_patient_data(series_patient_data):
    ...
    for Sample_ID, Patient_ID in (
        df_samples.loc[All_samples].ParticipantID.to_dict().items()
    ):
...
def OUTLIERS_GET_mask_for_outliers(arr, n_std_for_outliers, outlier_percentile_std=5):
    ...
    if outlier_percentile_std == 0:
        ...
        arr_masked = np.ma.masked_array(
            data=arr,
            mask=(arr.T < np.percentile(arr, outlier_percentile_std, axis=1)).T
            | (arr.T > np.percentile(arr, 100 - outlier_percentile_std, axis=1)).T,
        )
        arr_mean, arr_std = (
            arr_masked.mean(axis=1).data,
            arr_masked.std(axis=1).data,
        )  # retrive mean and std of outliers
    lower_limit, upper_limit = (
        arr_mean - n_std_for_outliers * arr_std,
        arr_mean + n_std_for_outliers * arr_std,
    )  # set limits for outliers
    return (
        (arr.T < lower_limit) | (upper_limit < arr.T)
    ).T  # return mask indicates location of outliers
...
def GET_non_NaN_without_outliers(data_1, data_2, n_std_for_outliers=3):
    ...
    mask_data_1, mask_data_2 = OUTLIERS_GET_mask_for_outliers(
        np.array([data_1, data_2]), n_std_for_outliers=n_std_for_outliers
    )  # get mask for outliers for each gene
    mask_non_outliers = ~(
        mask_data_1 | mask_data_2
    )  # build a mask for pairs of data that do not contain outliers
    return (
        data_1[mask_non_outliers],
        data_2[mask_non_outliers],
...
def S_percentile_bottom_top(
    s, bottom_percentile=5, top_percentile=95, return_label=True
):
    ...
    s_bottom, s_top = (
        s[s < np.percentile(s, bottom_percentile)],
        s[s > np.percentile(s, top_percentile)],
    )
    return (
        (s_bottom.index.values, s_top.index.values)
        if return_label
        else (s_bottom, s_top)
    )
...
def PANDAS_Reindex(df):
    ...
    return df.rename(
        index=dict(
            ...
            for index_before, index_after in zip(df.index.values, np.arange(len(df)))
        )
    )
...
def DF_Explore_Class(df, duplicate_filter=2):
    ...
    for col in df.columns.values:
        if isinstance(
            df[col].values[0], (str, object)
        ):  # only display when string data is stored
            s = LIST_COUNT(
                df[col].values, duplicate_filter=duplicate_filter
            )  # filter classes based on the number of their appearances
            if len(s) > 0:
                print("\n👉", col)
                display(s)
...
def DF_Transform_without_loading_in_memory(
    path_file_dataframe,
    path_file_dataframe_transformed,
    function_transformation,
    flag_output_gzipped=True,
    chunksize=10000,
    sep_input="\t",
    sep_output="\t",
    index=False,
    header=0,
    **dict_arg_pd_read_csv,
):
    """# 2022-06-01 20:08:07
    ...
    """ open an output file """
    newfile = (
        gzip.open(path_file_dataframe_transformed, "wb")
        if flag_output_gzipped
        else open(path_file_dataframe_transformed, "w")
    )
    ...
    for df_chunk in iter_tabular_data:
        ...
        if not flag_header_was_written and header is not None:
            ...
            flag_header_was_written = (
                True  # set the flag indicating the header has been written
            )
...
def DF_Sort_without_loading_in_memory(
    path_file,
    path_file_sorted,
    l_col_for_sorting,
    delimiter="\t",
    key_function_for_sorting=None,
    l_type=None,
    int_num_lines_for_a_chunk=1000000,
    flag_header_line_exists=True,
):
    """# 2021-12-26 20:44:17
    ...
    """
    """ handle input """
    flag_merge_multiple_input_files = not isinstance(
        path_file, str
    )  # retrieve a flag indicating that multiple sorted files will be merged into a single sorted file
    l_col_for_sorting = (
        [l_col_for_sorting]
        if isinstance(l_col_for_sorting, (str, int))
        else l_col_for_sorting
    )  # make 'l_col_for_sorting' as a list of string or integer if just a single string or a single integer has been given

    if flag_merge_multiple_input_files:
        l_path_file_chunk = path_file
        path_file = l_path_file_chunk[
            0
        ]  # use the first file to parse header and retrieve 'key_function_for_sorting'
    ...
    with gzip.open(path_file, "rb") as file:
        ...
        if flag_header_line_exists:
            ...
            if key_function_for_sorting is None:
                l_int_index_col_for_sorting = list(
                    l_header.index(col) for col in l_col_for_sorting
                )
        ...
        if key_function_for_sorting is None:
            ...
            def key_function_for_sorting(str_line):
                ...
                l_val = (
                    str_line.split(delimiter)
                    if l_type is None
                    else Parse_Line(str_line, l_type, delimiter)
                )  # 'l_type' is given, parse data according to each column's datatype
                return tuple(
                    list(
                        l_val[int_index_col]
                        for int_index_col in l_int_index_col_for_sorting
                    )
                )
        ...
        if not flag_merge_multiple_input_files:
            ...
            while True:
                ...
                if (
                    int_n_count_lines >= int_num_lines_for_a_chunk
                    or flag_all_line_parsed
                ):
                    ...
                    path_file_chunk = (
                        f"{path_file_sorted}.chunk_{int_index_chunk}.tsv.gz"
                    )
    ...
    l_files = [
        __parse_line__(
            path_file,
            flag_header_line_exists=(
                flag_header_line_exists if flag_merge_multiple_input_files else False
            ),
        )
        for path_file in l_path_file_chunk
    ]
...
def DF_Count_and_Drop_Duplicates(
    df,
    l_col_for_identifying_duplicates,
    ...
    if not isinstance(l_col_for_identifying_duplicates, (list)):
        l_col_for_identifying_duplicates = list(l_col_for_identifying_duplicates)
    set_col_for_identifying_duplicates = set(l_col_for_identifying_duplicates)
    l_col = list(
        col for col in df.columns.values if col != col_name_for_duplicate_counts
    )  # retrieve all column names except for a column for duplicate counts
    df_containing_col_not_for_identifying_duplicates = df.drop_duplicates(
        subset=l_col_for_identifying_duplicates, **dict_kw_drop_duplicates
    ).set_index(
        l_col_for_identifying_duplicates
    )  # this dataframe can contains a column for duplicate counts
    if col_name_for_duplicate_counts in df.columns.values:
        df_containing_col_not_for_identifying_duplicates.drop(
    ...
    df = (
        ...
        .sum()
        .join(df_containing_col_not_for_identifying_duplicates)
    )  # count duplicates by summing 1 or the previous duplicate counts given by the 'col_name_for_duplicate_counts' column
    df.reset_index(inplace=True)
    df = df[
        l_col + [col_name_for_duplicate_counts]
    ]  # rearange columns to match the order in the original dataframe
    return df
...
def DF_COLUMN_labels_replace_incompatible_characters(
    df,
    dict_replacement={
        "%": "_Percent_",
        "+": "_Plus_",
    ...
    dict_column_b4_column_after = (
        dict()
    )  # an empty dictionary that will be used to rename columns
    ...
    for column in columns:  # for each column
        ...
        for (
            ...
            incompatible_characters
        ):  # for each incompatible charactor, if the charactor exists in column, replace it with '_'
            if incompatible_character in column_after:
                column_after = column_after.replace(
                    incompatible_character,
                    dict_replacement.get(incompatible_character, "_"),
                )
        if column_after[
            0
...
PANDAS_COLUMN_labels_replace_incompatible_characters = (
    DF_COLUMN_labels_replace_incompatible_characters  # define aliases for this function
)
...
def GET_MASK_of_intersection(arr, annotation):
    ...
    list_mask = list(
        True if entry in annotation else False for entry in arr
    )  # create mask in list type
...
def PD_Subset(
    ...
    index=None,
    columns=None,
    preserve_order_in_df=True,
    subset_difference=False,
    return_numeric_column_only=False,
):
    """
    # 2021-02-01 16:10:25
    ...
    if (
        index is not None
    ):  # subset df according to given index and columns before subsetting
        df = PD_Subset(
            df, subset=index, axis=0, preserve_order_in_df=preserve_order_in_df
        )
    if columns is not None:
        df = PD_Subset(
            df, subset=columns, axis=1, preserve_order_in_df=preserve_order_in_df
        )
    if subset is not None:
        ...
        if (
            type(df) is pd.Series
        ):  # if pandas Series is given, only indices will be subsetted since columns do not exist
            axis = 0
        ...
        if axis == 0:
            if subset_difference:
                df = (
                    df[
                        list(
                            False if index in set_subset else True
                            for index in df.index.values
                        )
                    ]
                    if preserve_order_in_df
                    else df.loc[
                        list(entry for entry in subset if entry not in set_index)
                    ]
                )
            else:
                df = (
                    df[
                        list(
                            True if index in set_subset else False
                            for index in df.index.values
                        )
                    ]
                    if preserve_order_in_df
                    else df.loc[list(entry for entry in subset if entry in set_index)]
                )
        elif axis == 1:
            set_columns = set(df.columns.values)
            if subset_difference:
                df = (
                    df.loc[
                        :,
                        list(
                            False if column in set_subset else True
                            for column in df.columns.values
                        ),
                    ]
                    if preserve_order_in_df
                    else df[list(entry for entry in subset if entry not in set_columns)]
                )
            else:
                df = (
                    df.loc[
                        :,
                        list(
                            True if column in set_subset else False
                            for column in df.columns.values
                        ),
                    ]
                    if preserve_order_in_df
                    else df[list(entry for entry in subset if entry in set_columns)]
                )
    if (
        return_numeric_column_only and len(df) > 0
    ):  # 'return_numeric_column_only' : if True, return columns with only numeric datatypes
        df = df[
            list(
                key
    ...
    for col, queries in dict_search.items():
        ...
        if col == "index":  # retrive data to be searched
            ...
            if type(df) is not pd.Series:
                continue  # if df is not Series, ignore this query
            data = df.values
        else:
            data = df[col].values
        if ignore_case:
            data = np.char.lower(data.astype(str)).astype(
                object
            )  # convert all strings to lower cases if 'ignore_case' is set to True.
        else:
            data = data.astype(str).astype(
                object
            )  # convert data_types of data values to be searched to string
        ...
        #             continue
        if isinstance(
            queries, (list, tuple, np.ndarray, set)
        ):  # if the given queries is iterable
            len_entries = (
                df.shape[1] if col == "columns" or col == "col" else df.shape[0]
            )
            mask = (
                np.ones(len_entries).astype(bool)
                if query_AND_operation
                else np.zeros(len_entries).astype(bool)
            )  # mask that will contain all entries matching a given list of data
            for query in queries:
                if ignore_case:
                    query = (
                        query.lower()
                    )  # convert query to lower cases if 'ignore_case' is set to True.
                mask_current_query = Search_list_of_strings_Return_mask(
                    data, query, is_negative_query=is_negative_query
                )
                mask = (
                    mask & mask_current_query
                    if query_AND_operation
                    else mask | mask_current_query
                )  # merge mask for each data entry
        else:
            if ignore_case:
                query = (
                    queries.lower()
...
def PANDAS_Align_two(df_1, df_2, axis=1):
    ...
    if axis == 1:
        ...
        indices_shared = LIST_intersection_with_set(
            df_1.index.values, df_2.index.values
        )
...
def PANDAS_Subset_relation_df(df, indices):
    ...
    valid_indices = list(
        True if index in indices else False for index in df.index.values
    )
...
def PANDAS_relation_dataframe_make_symmetric(
    ...
    if diagonal_data is None:
        ...
        diagonal_data == "retain"
    ):  # if 'diagonal_data' is set to 'retain', save diagonal values
    ...
    if (
        diagonal_data == "retain"
    ):  # if 'diagonal_data' is set to 'retain', put a saved diagonal values back to relation_data matrix
...
def PDA_Calculate_CV(df):
    ...
    df = df[
        df.sum(axis=1) != 0
    ]  # remove rows with total coutns == 0 sort rows based on averages
    df = df.iloc[
        df.sum(axis=1).values.argsort()[::-1]
    ]  # sort rows based on total counts of each row
    dfcv = pd.DataFrame(
        dict(AVG=df.mean(axis=1), STD=df.std(axis=1))
    )  # calculate summary of df
...
def PD_Locate_gene(df, gene):
    ...
    if geneid == -1:  # check whether the given gene is valid
        ...
        if isinstance(
            df.index.values[0], (int, float, np.float64, np.int64)
        ):  # first check indices of a given df # if index contain a number (potentially geneid)
            if geneid in df.index.values:
                return df.loc[geneid]
        elif (
            type(df) is pd.DataFrame
        ):  # if the type of indices is other than a number, check whether it is dataframe, and check whether geneid is in the columns
...
def PD_Add_gene_annotation(
    df, in_place=False, geneid_to_genesymbol=False, type_gene_id="ensembl"
):
    ...
    if (
        not in_place
    ):  # if gene annotation is added not in_place, copy dataframe before adding annotations
        df = deepcopy(df)
    if geneid_to_genesymbol:
        df_annotated = (
            PD_Add_gene_annotation(df)
            .set_index("Approved_Symbol")
            .drop(columns="Approved_Name")
        )
        return (
            df_annotated.iloc[:, 0] if type(df) is pd.Series else df_annotated
        )  # return series if series was given
    Gene_IDs = df.index.values
    if (
        type(Gene_IDs[0]) is tuple
    ):  # if df are indexed with multiIndex, retrive only the first index from index tuple as a list of Gene_IDs
        Gene_IDs = np.array(
            list(multiIndex[0] for multiIndex in Gene_IDs), dtype=object
        )
    ...
    df_anno = (
        df_anno_biomaRt_gene
        if (
            (type(Gene_IDs[0]) is str and "ENS" in Gene_IDs[0])
            and type_gene_id == "ensembl"
        )
        else df_ID_Symbol
    )
    set_valid_gene_id = set(df_anno.index.values)
    mask_mapped = list(
        True if gene_id in set_valid_gene_id else False for gene_id in Gene_IDs
    )
    if np.sum(mask_mapped) != len(
        df
    ):  # if there are some entries that are not mapped with current annotation data, retrive only valid entries
        Gene_IDs = Gene_IDs[mask_mapped]
        df = df[mask_mapped]
    df_annotation = df_anno.loc[
        Gene_IDs
    ]  # retrive Gene_Name and Gene_Symbols of valid Gene_IDs
    arr_gene_names = df_annotation.Approved_Name.values
    arr_gene_symbols = df_annotation.Approved_Symbol.values
    if type(df) is pd.DataFrame:
        if (
            "Approved_Symbol" in df.columns.values
        ):  # return the given dataframe if the given df already has annotation columns
            return df
        df["Approved_Name"], df["Approved_Symbol"] = arr_gene_names, arr_gene_symbols
        return df[
            ["Approved_Symbol", "Approved_Name"] + list(df.columns.values)[:-2]
        ]  # reorder the columns and return the dataframe
    elif type(df) is pd.Series:
        data_name = df.name if df.name is not None else "unknown_data"
        return pd.DataFrame(
            dict(
                Approved_Symbol=arr_gene_symbols,
                Approved_Name=arr_gene_names,
                data=df.values,
            ),
            index=Gene_IDs,
        ).rename(columns=dict(data=data_name))
...
def RESULT_filter(
    ...
    thres_p_value=1,
    thres_adj_p_value=1,
    label_adj_p_value="adjusted_p_value",
    label_p_value="p_value",
    thres_n_gene=0,
    label_n_genes="number_valid_genes",
    thres_abs_log2fc=0,
    label_log2fc="Log2_Fold_Change",
    thres_abs_diff=0,
    label_diff="Difference",
    thres_avg_all=0,
    label_avg_all="average_all",
    log2fc_positive=None,
):
    ...
    if (
        thres_n_gene > 0
    ):  # if valid thres_n_gene is given, drop entries with n_genes below 'thres_n_gene'
        df_result = df_result[df_result[label_n_genes] >= thres_n_gene]
    if thres_p_value < 1:
        df_result = df_result[
            df_result[label_p_value] <= thres_p_value
        ]  # return p-value filtered result if valid thres_p_value is given
    if thres_adj_p_value < 1:
        df_result = df_result[
            df_result[label_adj_p_value] <= thres_adj_p_value
        ]  # return p-value filtered result if valid thres_p_value is given
    if thres_abs_log2fc > 0:
        df_result = df_result[
            np.abs(df_result[label_log2fc]) >= thres_abs_log2fc
        ]  # return Log2FC filtered result if valid 'thres_abs_log2fc' is given (entries with absolute value of Log2FC below thres_abs_log2fc is filtered out)
    if thres_abs_diff > 0:
        df_result = df_result[
            np.abs(df_result[label_diff]) >= thres_abs_diff
        ]  # return difference filtered result if valid 'thres_abs_diff' is given (entries with absolute value of Difference below thres_abs_diff is filtered out)
    if thres_avg_all > 0:
...
def DF_Tabular_2_Linear_with_filter(df, thres_lower=None, thres_higher=None):
    ...
    s_indices, s_columns = pd.Series(
        DICTIONARY_Build_from_arr(df.index.values)
    ), pd.Series(DICTIONARY_Build_from_arr(df.columns.values))
    indices, columns = np.where(arr_filter)
    data_values = data[arr_filter]
    return pd.DataFrame(
        dict(
            Index=s_indices.loc[indices].values,
            Column=s_columns.loc[columns].values,
            Data_Value=data_values,
        )
    )  # .sort_values( 'Correl_Coeffi', ascending = False )
...
def PANDAS_MULTIINDEX_get_indices_from_multiIndex_of_df(df, index_among_multiIndex=0):
    ...
    if type(arr_multiIndex[0]) is not tuple:
        return -1
    return np.array(
        list(multiIndex[index_among_multiIndex] for multiIndex in arr_multiIndex),
        dtype=object,
    )
...
def PANDAS_MULTIINDEX_get_multiIndex_as_np_array(df):
    ...
    if type(arr_multiIndex[0]) is not tuple:
        return -1
    return np.array(
        list(list(index for index in multiIndex) for multiIndex in arr_multiIndex),
        dtype=object,
    )
...
def PEARSONR(arr_x, arr_y):  # 2020-07-30 16:17:30
    ...
    if n_non_nan_pairs > 1:
        return (*stats.pearsonr(arr_x_valid, arr_y_valid), n_non_nan_pairs)
    else:
        return np.nan, np.nan, n_non_nan_pairs
...
def ROW_NORMALIZE_log_mean(df):
    ...
    df.iloc[:, :] = (
        df.values.T / np.power(2, np.log2(df.values).mean(axis=1))
    ).T * 100  # normalize data by log-mean of each entry
    return df.dropna()
...
def NORMALIZE_Relative_Log_Expression(
    df,
    sample_list_for_pseudoreference=None,
    sample_list=None,
    Gene_Set=None,
    ...
    )  # set default sample_list and sample_list_for_pseudoreference
    sample_list_for_pseudoreference = (
        LIST_intersection_with_set(sample_list_for_pseudoreference, df.columns.values)
        if sample_list_for_pseudoreference is not None
        else sample_list
    )
    df = deepcopy(df)  # make a copy dataframe to be safe
    df_all_input_genes = (
        df[sample_list].dropna()
        if dropna or return_normalized_ratios
        else df[sample_list]
    )  # drop NaN values for sample_list samples if 'dropna' is True or return_normalized_ratios
    df = df.loc[Gene_Set, sample_list].replace(0, np.nan)
    df = (
        df if allow_nan_values else df.dropna()
    )  # drop NaN and zero values for sample_list samples
    ...
    if allow_nan_values:
        data, data_pseudoref = np.ma.masked_array(
            data, np.isnan(data)
        ), np.ma.masked_array(data_pseudoref, np.isnan(data_pseudoref))
    pseudoreference_values = np.power(
        2, np.mean(np.log2(data_pseudoref), axis=1)
    )  # (pseudoreference) retrieve log-average values for each genes (by applying power of 1 / n_samples to a product of values of all samples)
    relative_ratios = (data.T / pseudoreference_values).T
    if use_median_ratio:
        scaling_ratios = (
            np.ma.median(relative_ratios, axis=0)
            if allow_nan_values
            else np.median(relative_ratios, axis=0)
        )  # retrive median ratios of genes in Gene_Set to pseudoreference for each sample
    else:
        scaling_ratios = np.mean(
            relative_ratios, axis=0
        )  # retrive mean ratios of genes in Gene_Set to pseudoreference for each sample
    series_scaling_ratios = pd.Series(
        scaling_ratios.data if allow_nan_values else scaling_ratios,
        index=df.columns.values,
    )  # build pandas series with median values with index as Sample_IDs
    series_scaling_ratios = series_scaling_ratios.rename(
        "Median " * use_median_ratio
        + "scaling_ratios_used_for_normalization"
        + normalization_description
    )  # name a scale factor with a description given by 'normalization_description'
    if (
        scale_all_genes
    ):  # if 'scale_all_genes' is set to True, return a data of all genes that are scaled with a median_ratio calculated from a given subset of genes
        data, index, columns = (
            df_all_input_genes.values,
            df_all_input_genes.index.values,
            df_all_input_genes.columns.values,
        )
        if (
            return_normalized_ratios
        ):  # 'return_normalized_ratios' and 'scale_all_genes' are set to True, compute relative_ratios for all genes and scale them with median ratio
            data_pseudoref = df_all_input_genes[
                sample_list_for_pseudoreference
            ].values.astype(float)
            if allow_nan_values:
                data_pseudoref = np.ma.masked_array(
                    data_pseudoref, np.isnan(data_pseudoref)
                )
            pseudoreference_values = np.power(
                2, np.mean(np.log2(data_pseudoref), axis=1)
            )
    ...
    data_norm = (
        relative_ratios / scaling_ratios
        if return_normalized_ratios
        else data / scaling_ratios
    )  # divide relative ratios or data of each sample by its median ratio (scaling values)
    df_norm = pd.DataFrame(
        data_norm.data if allow_nan_values else data_norm, index=index, columns=columns
    )
...
def GET_DATA_of_gene(Gene, df=None, sample_list=None):
    ...
    return np.array(
        df.loc[Gene_ID, sample_list].values, dtype=float
    )  # retrive data of a given gene in a given df and sample_list.
...
def GENE_SET_GET_Gene_Set_by_name(
    ...
    for Gene_Sets_Name in dict_GeneSets_ALL.keys():
        if Gene_Set_name in dict_GeneSets_ALL[Gene_Sets_Name]:
            ...
            if (
                return_Gene_Name_Symbol
            ):  # if 'return_Gene_Name_Symbol' is True, return sorted list of Gene_Name_Symbol of a gene_set.
                return List_Gene_ID__2__List_Gene_Symbol(
                    Gene_Set, add_gene_name=True, return_sorted=True
                )
            else:
                return Gene_Set
...
def GENE_SET_Search_MSigDB_Gene_Set_Name(query):
    ...
    for GeneSets_name in dict_MSigDB_name__set_name__gene_set:
        Matched_Gene_Sets.extend(
            Search_list_of_strings(
                list(dict_MSigDB_name__set_name__gene_set[GeneSets_name].keys()),
                query=query.upper(),
            )
        )  # retriving Gene_Sets matching the query # Since its MSiGDB genesets only, change query to upper characters
...
def GENE_SET_Search_Gene_Set_Name_in_Gene_Sets(
    dict_Gene_Sets, query, return_GeneSet_names=True
):
    """Search Gene_Sets maching a given query in a given dict_Gene_Sets, and print the result"""
    Matched_Gene_Sets = Search_list_of_strings(
        list(dict_Gene_Sets.keys()), query=query
    )  # retriving Gene_Sets matching the query
    print(
        "number_of_genes", "\t", "Gene_Set_Name"
    )  # print the number of genes for each matched gene set
...
def GENE_SET_Search_Gene_in_Gene_Sets(
    dict_Gene_Sets, Gene, return_subset_dict_Gene_Sets=False
):
    ...
    if Gene_ID == -1:  # if Gene is invalid, return an error value
        return -1
    list_Gene_Set_Names = list(
        Gene_Set_Name
        for Gene_Set_Name, Gene_Set in dict_Gene_Sets.items()
        if Gene_ID in Gene_Set
    )
    if (
        return_subset_dict_Gene_Sets
    ):  # if 'return_subset_dict_Gene_Sets' is set to True, by using a pandas series indexing, return a subset of dict_Gene_Sets
...
def GENE_SET_Get_a_union_of_GeneSets_from_list_of_MSigDB_GeneSet_names(
    list_MSigDB_GeneSet_names,
):
    ...
    for MSigDB_GeneSet_name in list_MSigDB_GeneSet_names:
        set_union.update(
            GENE_SET_GET_Gene_Set_by_name(
                MSigDB_GeneSet_name, return_Gene_Name_Symbol=False
            )
        )
    return set_union
...
def GSEA_pathway(pathway, Gene_Set, background_Gene_Set):
    ...
    num_overlaped_background_genes = len(
        pathway_gene_set.intersection(background_Gene_Set)
    )
    odd_ratio, p_value = fisher_exact(
        [
            [num_overlaped_background_genes, num_background_genes],
            [num_overlaped_genes, num_gene_set],
        ]
    )
    return dict(
        num_overlaped_genes=num_overlaped_genes,
        num_overlaped_background_genes=num_overlaped_background_genes,
        odd_ratio=odd_ratio,
        p_value=p_value,
    )
...
def KEGG_Pathways_GSEA(Gene_IDs, backgroud_genes):
    ...
    dict_pathway_GSEA = (
        {}
    )  # perform GSEA for every KEGG_pathway gene set and save result to a dictionary
    for pathway in dict_pathway:
        if len(dict_pathway[pathway]["genes"]) != 0:
            dict_pathway_GSEA[pathway] = GSEA_pathway(
                pathway, Gene_Set, backgroud_gene_set
            )
    ...
    return dict(
        over_represented=df[df.odd_ratio < 1],
        under_represented=df[df.odd_ratio > 1],
        result=df,
    )
...
def ANALYSIS_GSEA__dict_Gene_Sets(
    ...
    for Gene_Set_Name in dict_Gene_Sets:
        ...
        if len(Gene_Set) == 0:
            ...
            return -1
        num_overlaped_genes = len(
            Gene_Set.intersection(Gene_IDs)
        )  # perfoem GSEA of a current Gene_Set
        num_overlaped_background_genes = len(Gene_Set.intersection(background_Gene_IDs))
        odd_ratio, p_value = fisher_exact(
            [
                [num_overlaped_genes, num_genes],
    ...
    df["adj_p_val"] = multipletests(df["p_value"].values, 0.05, "fdr_bh", False, False)[
        1
    ]  # perform fdr correction of p_values
    df = df[
        df.adj_p_val.values < thres_adj_p_val
    ]  # filter out entries that have higher p_values than a given threshold p_value
    df = df.sort_values(["p_value"], axis="index")  # sort result according to p_value
...
def ANALYSIS_GSEA__MSigDB_Gene_Sets(
    ...
    for Gene_Sets_Name in Gene_Sets_Names:  # for each Gene_Sets data in MSigDB datasets
        print(
            Gene_Sets_Name
        )  # report the name of Gene_Sets data that will be used for GSEA
        df_result = ANALYSIS_GSEA__dict_Gene_Sets(
            Gene_IDs=Gene_IDs,
            background_Gene_IDs=background_Gene_IDs,
            dict_Gene_Sets=dict_MSigDB_name__set_name__gene_set[Gene_Sets_Name],
            thres_adj_p_val=thres_adj_p_val,
        )
    ...
    df["adj_p_val"] = multipletests(df["p_value"].values, 0.05, "fdr_bh", False, False)[
        1
    ]  # perform multiple correction again using p_values
...
def MULTIPLE_TEST_p_value(arr_p_values):
    ...
    arr_adj_p_values[mask_valid_p_value] = multipletests(
        arr_p_values[mask_valid_p_value], 0.05, "fdr_bh", False, False
    )[1]
...
def Change__normal_vs_tumor__Gene_Set(df, Gene_Set=None, print_error_msg=False):
    ...
    valid_gene_set = set(
        df[dict_type_sample["normal_or_tumor"]].loc[existing_gene_set].dropna().index
    )  # only process genes that do not contain NaN values
    ...
    if num_valid_genes == 0:  # if there is no valid genes, end the method
        ...
        return -1
    coverage = round(
        num_valid_genes / num_genes * 100, 1
    )  # calculate pathway or gene set coverage
    # data values
    # compute average of values to assess Gene_Set activity
    normal_data = np.average(
        df.loc[valid_gene_set, dict_type_sample["normal"]].values, axis=0
    )
    tumor_data = np.average(
        df.loc[valid_gene_set, dict_type_sample["tumor"]].values, axis=0
    )
    # perform T test
    p_value = stats.ttest_ind(normal_data, tumor_data)[1]
    Log2FC_ratio_of_tumor_to_normal = np.log2(
        np.average(tumor_data) / np.average(normal_data)
    )
    return dict(
        num_valid_genes=num_valid_genes,
        num_genes=num_genes,
        p_value=p_value,
        Log2FC_ratio_of_tumor_to_normal=Log2FC_ratio_of_tumor_to_normal,
        coverage=coverage,
    )
...
def Calculate_change__dict_Gene_Sets(df, dict_Gene_Sets, print_error_msg=False):
    ...
    for Gene_Set_name in dict_Gene_Sets:
        ...
        if len(Gene_Set) != 0:  # if current gene_set is valid
            result = Change__normal_vs_tumor__Gene_Set(
                df, Gene_Set=Gene_Set, print_error_msg=print_error_msg
            )
    ...
    df["adjuested_p_value"] = multipletests(p_values, 0.05, "bonferroni", False, False)[
        1
    ]
...
def Calculate_min_by_max(df, sample_list, Gene_Set=None, n_std_for_outliers=3):
    ...
    data = np.ma.masked_array(
        data,
        OUTLIERS_GET_mask_for_outliers(data, n_std_for_outliers=n_std_for_outliers),
    )
...
def CALCULATE_masked_arr__average__std_by_mean(arr):
    ...
    if arr.ndim == 1:
        arr = np.ma.masked_array(
            arr, np.isnan(arr)
        )  # mask their NaN values for downstream calculations
        ...
        arr_std_by_mean = (
            np.std(arr, axis=1) / arr_average
        )  # calculate std divided by mean
...
def Calculate_Log2FC_p_value__A_vs_B(
    df,
    Gene_IDs=None,
    ...
    method_average="log_mean",
    add_raw_data=False,
    replace_zero_with=None,
    drop_avg=False,
    drop_std=True,
):
    ...
    Gene_IDs = (
        df.index.values
        if Gene_IDs is None
        else LIST_intersection_with_set(Gene_IDs, df.index.values)
    )  # if Gene_IDs was not given, set indices of df as Gene_IDs
    ...
    sample_list_condition_A, sample_list_condition_B = LIST_intersection_with_set(
        sample_list_condition_A, set_all_samples
    ), LIST_intersection_with_set(
        sample_list_condition_B, set_all_samples
    )  # retrive only valid samples
    print(
        "Number of valid sample A: {}, sample B: {}".format(
            len(sample_list_condition_A), len(sample_list_condition_B)
        )
    )  # print number of valid samples
    if len(sample_list_condition_A) != len(
        sample_list_condition_B
    ):  # if valid numbers of samples of condition A and B are not the same, calculate ttest_independent using ttest_ind function
        p_value_ttest_rel = False
    if (
        len(sample_list_condition_A) == 0 or len(sample_list_condition_B) == 0
    ):  # if number of samples of one condition is zero, return -1
        return -1
    all_samples = list(sample_list_condition_A) + list(
        sample_list_condition_B
    )  # create a sample list with all samples
    ...
    condition_A_data, condition_B_data, data_all = (
        df[sample_list_condition_A].values,
        df[sample_list_condition_B].values,
        df[all_samples].values,
    )
    condition_A_data, condition_B_data = np.ma.masked_array(
        condition_A_data, np.isnan(condition_A_data)
    ), np.ma.masked_array(
        condition_B_data, np.isnan(condition_B_data)
    )  # mask their NaN values for downstream calculations
    data_all = np.ma.masked_array(data_all, np.isnan(data_all))
    if method_average == "mean":
        condition_A_average, condition_B_average, average_all = (
            np.average(condition_A_data, axis=1),
            np.average(condition_B_data, axis=1),
            np.average(data_all, axis=1),
        )
    else:
        condition_A_average, condition_B_average, average_all = (
            np.power(2, np.log2(condition_A_data).mean(axis=1)),
            np.power(2, np.log2(condition_B_data).mean(axis=1)),
            np.power(2, np.log2(data_all).mean(axis=1)),
        )
    condition_A_std_by_mean, condition_B_std_by_mean = (
        np.std(condition_A_data, axis=1) / condition_A_average,
        np.std(condition_B_data, axis=1) / condition_B_average,
    )  # calculate std divided by mean
    std_by_mean_all = np.std(data_all, axis=1) / average_all
    if (
        len(sample_list_condition_A) == 1 or len(sample_list_condition_B) == 1
    ):  # if number of samples of one condition is zero, p_values and adj_p_values are np.nan
        p_values, adj_p_values = np.full(len(df), np.nan), np.full(len(df), np.nan)
    else:
        p_values = (
            mstats.ttest_rel(condition_A_data, condition_B_data, axis=1).pvalue
            if p_value_ttest_rel
            else mstats.ttest_ind(condition_A_data, condition_B_data, axis=1).pvalue
        )  # perform the t-test (row by row) # use mstats module which supports masked arrays
    ...
    if label_condition_A == "cl name" or label_condition_B == "cl name":
        label_condition_A, label_condition_B = "_".join(
            list(cl.split("_")[0] for cl in sample_list_condition_A)
        ), "_".join(list(cl.split("_")[0] for cl in sample_list_condition_B))
    dict_rename = dict(
        condition_A_average="{A}_average".format(A=label_condition_A),
        condition_B_average="{B}_average".format(B=label_condition_B),
        condition_A_std_by_mean="{A}_std_by_mean".format(A=label_condition_A),
        condition_B_std_by_mean="{B}_std_by_mean".format(B=label_condition_B),
    )  # define dictionary for rename columns according to given condition A and B labels
    ...
    df_result = pd.DataFrame(
        dict(
            ...
            condition_A_average=condition_A_average,
            condition_B_average=condition_B_average,
            condition_A_std_by_mean=condition_A_std_by_mean,
            condition_B_std_by_mean=condition_B_std_by_mean,
            std_by_mean_all=std_by_mean_all,
        ),
        index=Gene_IDs,
    )
    if drop_avg:  # drop unneccesary columns
        df_result = df_result.drop(
            columns=["average_all", "condition_A_average", "condition_B_average"]
        )
    if drop_std:
        df_result = df_result.drop(
            columns=[
                "condition_A_std_by_mean",
                "condition_B_std_by_mean",
                "std_by_mean_all",
            ]
        )
    if (
        len(sample_list_condition_A) == 1 or len(sample_list_condition_B) == 1
    ):  # if number of samples of one condition is zero, drop p_value columns
        df_result = df_result.drop(columns=["p_value", "adjusted_p_value"])
    df_result = df_result.rename(columns=dict_rename).sort_values(
        "Log2_Fold_Change"
    )  # build a sorted result dataframe with labeled columns
    if add_raw_data:
        df_result = df_result.join(df[all_samples])
    if (
        type(Gene_IDs[0]) is tuple
    ):  # if index is a multi-index (tuple type when returned by df.index.values), convert index of df into MultiIndex
...
def Calculate_Log2FC_p_value__dict_Gene_Sets__A_vs_B(
    df,
    dict_Gene_Sets,
    Gene_IDs=None,
    sample_list_condition_A=None,
    ...
    All_valid_Gene_IDs = set(
        df.loc[Gene_IDs].dropna().index.values
    )  # retrieve Gene_IDs without NaN from the given dataframe
    ...
    for Gene_Set_name in dict_Gene_Sets:  # for each Gene_Set_name
        Gene_Set = All_valid_Gene_IDs.intersection(
            dict_Gene_Sets[Gene_Set_name]
        )  # retrive valid Gene_IDs
        if len(Gene_Set) == 0:  # if number of valid Genes is zero, skip the gene_set
            continue
        df_Gene_Set = df.loc[Gene_Set]  # retrive data of Gene_Set
        condition_A_data, condition_B_data, data_all = (
            df_Gene_Set[sample_list_condition_A].values,
            df_Gene_Set[sample_list_condition_B].values,
            df_Gene_Set[all_samples].values,
        )
        condition_A_average, condition_B_average, average_all = (
            np.average(condition_A_data, axis=1),
            np.average(condition_B_data, axis=1),
            np.average(data_all, axis=1),
        )  # calculate average of condition A samples and condition B samples and all samples for each gene
        condition_A_std_by_mean, condition_B_std_by_mean = (
            np.std(condition_A_data, axis=1) / condition_A_average
        ).mean(), (
            np.std(condition_B_data, axis=1) / condition_B_average
        ).mean()  # calculate std in condition A and condition B
        coondition_A_B_std_by_mean_avg = (
            condition_A_std_by_mean + condition_B_std_by_mean
        ) / 2  # average std_by_mean in condition A and std in condition B, which can be used in an interactive plotting
        Log2FC = np.log2(
            condition_B_average / condition_A_average
        )  # calculate log2 fold change values
        p_value = stats.ttest_rel(
            condition_A_average, condition_B_average
        ).pvalue  # perform the t-test (pairwise t-test, t-test of related samples)
        dict_Gene_Set_result[Gene_Set_name] = dict(
            Log2_Fold_Change=Log2FC.mean(),
            Log2_Fold_Change_std=Log2FC.std(),
            p_value=p_value,
            condition_A_average=condition_A_average.mean(),
            condition_B_average=condition_B_average.mean(),
            number_valid_genes=len(Gene_Set),
            condition_A_std_by_mean=condition_A_std_by_mean,
            condition_B_std_by_mean=condition_B_std_by_mean,
            coondition_A_B_std_by_mean_avg=coondition_A_B_std_by_mean_avg,
        )
    df_result = pd.DataFrame(dict_Gene_Set_result).T  # convert result to DataFrame
    df_result["adjusted_p_value"] = MULTIPLE_TEST_p_value(
        df_result.p_value.values
    )  # calculate adjusted p_values
    return df_result.sort_values(
        "p_value"
    )  # sort results according to p_values and return result dataframe
...
def Calculate_Log2FC_p_value___dict_Gene_Sets_Name__Gene_Sets___A_vs_B(
    df,
    dict_Gene_Sets_Name__Gene_Sets=None,
    Gene_IDs=None,
    filter_b4_cal__p_value=0.05,
    filter_b4_cal__avg_all=0,
    sample_list_condition_A=None,
    sample_list_condition_B=None,
    label_condition_A="condition_A",
    label_condition_B="condition_B",
    ...
    set_all_samples = set(df.columns.values)
    sample_list_condition_A, sample_list_condition_B = list(
        set_all_samples.intersection(sample_list_condition_A)
    ), list(
        set_all_samples.intersection(sample_list_condition_B)
    )  # retrive only valid samples
    all_samples = list(sample_list_condition_A) + list(
        sample_list_condition_B
    )  # create a sample list with all samples
    if (len(sample_list_condition_A) > 1 and len(sample_list_condition_B) > 1) and (
        filter_b4_cal__p_value is not None or filter_b4_cal__avg_all is not None
    ):  # if number of samples for each condition > 1, filter entries of df by p_value of Log2FC before Gene-Set based calculation, since the analysis assumes Log2FC of the genes to be significant
        df_result = Calculate_Log2FC_p_value__A_vs_B(
            df,
            sample_list_condition_A=sample_list_condition_A,
            sample_list_condition_B=sample_list_condition_B,
            drop_avg=False,
        )
        df = df.loc[
            RESULT_filter(
                df_result,
                thres_p_value=filter_b4_cal__p_value,
                thres_avg_all=filter_b4_cal__avg_all,
            ).index.values
        ]
        print("Number of Remaining Entries after filter :", len(df))
    All_valid_Gene_IDs = set(
        PD_Subset(df, Gene_IDs).dropna().index.values
    )  # retrieve Gene_IDs without NaN from the given dataframe
    df = df.loc[All_valid_Gene_IDs]  # retrive data of a given Gene_IDs
    ma_data = np.ma.masked_array(
        df.values, df.values == 0
    )  # mask values with zero data values
    df.iloc[:, :] = (
        ma_data.T / np.power(2, np.log2(ma_data).mean(axis=1))
    ).T * 100  # normalize data by log-mean of each entry
    list_df_result = list()
    for Gene_Sets_name, dict_Gene_Sets in dict_Gene_Sets_Name__Gene_Sets.items():
        ...
        for Gene_Set_name in dict_Gene_Sets:  # for each Gene_Set_name
            ...
            if (
                len(Gene_Set) == 0
            ):  # if number of valid Genes is zero, skip the gene_set
                continue
            ...
            condition_A_std_by_mean_all_genes, condition_B_std_by_mean_all_genes = (
                np.std(condition_A_data, axis=1) / condition_A_average
            ), (
                np.std(condition_B_data, axis=1) / condition_B_average
            )  # calculate std in condition A and condition B
            condition_A_std_by_mean, condition_B_std_by_mean = (
                condition_A_std_by_mean_all_genes[condition_A_average != 0].mean(),
                condition_B_std_by_mean_all_genes[condition_B_average != 0].mean(),
            )  # calculate average of std/mean for all genes (except when average values across samples are zero)
            all_std_by_mean = (
                np.std(data_all, axis=1) / average_all
            ).mean()  # average std_by_mean in condition A and std in condition B, which can be used in an interactive plotting
            log2FC = np.log2(
                condition_B_average / condition_A_average
            )  # calculate log2 fold change values
            log2FC = np.ma.masked_array(
                log2FC, np.isinf(log2FC)
            )  # mask infinite values in log2FC (caused by 0 count values)
            p_value = stats.ttest_rel(
                condition_A_average, condition_B_average
            ).pvalue  # perform the t-test (pairwise t-test, t-test of related samples)
            dict_Gene_Set_result[Gene_Set_name] = dict(
                ...
                condition_A_average=condition_A_average.mean(),
                condition_B_average=condition_B_average.mean(),
                number_valid_genes=len(df_Gene_Set),
                condition_A_std_by_mean=condition_A_std_by_mean,
                condition_B_std_by_mean=condition_B_std_by_mean,
                all_std_by_mean=all_std_by_mean,
            )
            if (
                add_Gene_Set_2_df
            ):  # if 'add_Gene_Set_2_df' is set True, add used Gene_Set to df for subsequent use
    ...
    df_result = list_df_result[
        0
    ]  # if only one Gene_Sets is given, take the df_result out of the one-element list
    if (
        len(list_df_result) > 1
    ):  # if more than one Gene_Sets were given, join the results in list_df_result into one DataFrame
    ...
    df_result["adjusted_p_value"] = MULTIPLE_TEST_p_value(
        df_result.p_value.values.astype(float)
    )  # calculate adjusted p_values
    return (
        df_result.sort_values("p_value").rename(columns=dict_rename),
        df,
    )  # sort results according to p_values and return result dataframe
...
def GSEA_Calculate_Average___dict_Gene_Sets(
    series, dict_Gene_Sets, Gene_IDs=None, log10_transform=True, name_of_series="data"
):
    ...
    if Gene_IDs is None:  # if Gene_IDs was not given, set indices of df as Gene_IDs
        ...
        Gene_IDs = set(series.index.values).intersection(
            Gene_IDs
        )  # consider Gene_IDs that exist in df
    Gene_IDs = set(
        series.loc[Gene_IDs].dropna().index.values
    )  # retrieve Gene_IDs without NaN from the given dataframe
    series = series.loc[Gene_IDs]  # retrive data of a given Gene_IDs
    dict_Gene_Set_result = dict()
    for Gene_Set_name in dict_Gene_Sets:  # for each Gene_Set_name
        Gene_Set = Gene_IDs.intersection(
            dict_Gene_Sets[Gene_Set_name]
        )  # retrive valid Gene_IDs
        if len(Gene_Set) == 0:  # if number of valid Genes is zero, skip the gene_set
            continue
        ...
        if log10_transform:
            data = np.log10(data)
        dict_Gene_Set_result[Gene_Set_name] = dict(
            Average=data.mean(), Standard_dev=data.std(), number_valid_genes=len(data)
        )
    if series.name is not None:
        name_of_series = series.name
    Average_label, Std_label = (
        "Log10_" * log10_transform + "Average_" + name_of_series,
        "Standard_dev_of_" + "Log10_" * log10_transform + name_of_series,
    )
    df_result = pd.DataFrame(dict_Gene_Set_result).T.rename(
        columns=dict(Average=Average_label, Standard_dev=Std_label)
...
def GSEA_Calculate_Average___dict_Gene_Sets_Name__Gene_Sets(
    series,
    dict_Gene_Sets_Name__Gene_Sets=None,
    Gene_IDs=None,
    ...
    series = series.loc[Gene_IDs]  # retrive data of a given Gene_IDs
    if (
        series.name is not None
    ):  # use name of the series to label the columns of result dataframe
        name_of_series = series.name
    Average_label, Std_label = (
        "Log10_" * log10_transform + "Average_" + name_of_series,
        "Standard_dev_of_" + "Log10_" * log10_transform + name_of_series,
...
def CALCULATE_Factor_Analysis__df(df, sample_list, n_factors, Gene_Set=None):
    ...
    column_labels = "Factor_" + np.array(
        np.array(np.arange(1, n_factors + 1), dtype=str), dtype=object
    )
    FA = FactorAnalysis(n_components=n_factors)
    return pd.DataFrame(
        FA.fit_transform(df.values), index=df.index.values, columns=column_labels
    )
...
def GENE_Annotate_df_for_plot(df, gene_list_annotation):
    ...
    if (
        ...
        dict_ID_positions = dict(
            (Gene_ID, position) for Gene_ID, position in zip(Gene_IDs, list_positions)
        )  # build dictionary ( key = Gene_ID, value = position )
        if (
            gene_list_annotation == "all"
        ):  # If gene_list_annotation = 'all', annotat all genes in a given df
            gene_list_annotation = Gene_IDs
        elif type(gene_list_annotation) is int:
            gene_list_annotation = Gene_IDs[::gene_list_annotation]
        elif (
            type(gene_list_annotation) is list
            and type(gene_list_annotation[0]) is int
            and len(gene_list_annotation) < 4
        ):
            if len(gene_list_annotation) == 2:
                gene_list_annotation.append(
                    1
                )  # by default gene_list_annotation[ 2 ] = 1
            gene_list_annotation = Gene_IDs[
                gene_list_annotation[0] : gene_list_annotation[
                    1
                ] : gene_list_annotation[2]
            ]
        if isinstance(
            Gene_IDs[0], (int, float, np.float64, np.int64)
        ):  # if a given dataframe contains Entrez Gene_IDs
            Gene_IDs_annotation = LIST_intersection_with_set(
                List_Gene__2__List_Gene_ID(gene_list_annotation), Gene_IDs
            )  # retrive valid Gene_IDs in a given list of genes for annotation
            Symbols_annotation = List_Gene_ID__2__List_Gene_Symbol(
                Gene_IDs_annotation
            )  # retrive valid Gene_Symbols from Gene_IDs
        ...
        list_positions_annotation = list(
            dict_ID_positions[Gene_ID] for Gene_ID in Gene_IDs_annotation
        )  # retrive positions of the valid genes in the DataFrame
...
def PLOT_elbow__find_optimal_n_cluster_from_linkage_map(L, n_max_clusters=10):
    ...
    plt.plot(
        idxs[:-2] + 1,
        acceleration_rev,
        label='second derivative of "distance from root for each cluster"',
    )
    plt.show()
...
def Clustering(
    df,
    axis=1,
    cluster_name="unnamedData_",
    clustering_type="optimal",
    is_relation_matrix=True,
    n_recursion_lim=10,
    thres_skewed_ratio=5,
    cluster_start=0,
    min_num_genes=None,
    n_max_cluster=10,
    linkage_method=None,
    ...
    if type(df) is not pd.DataFrame:
        ...
        df = df.join(
            other=list_df[1], how="left", lsuffix="_1", rsuffix="_2"
        )  # join the first and second DataFrame, while adding suffix to column labels
        for index_df, a_df in zip(np.arange(3, len(list_df) + 1), list_df[2:]):
            df = df.join(
                other=a_df, how="left", rsuffix="_" + str(index_df)
            )  # join the thrid DataFrames and more DataFrames
    if (
        axis == 0
    ):  # For asymmetric data (with different columns and indices), If 'axis' is 0, cluster columns. if 'axis' is 1, cluster indices.
        df = df.T
    if clustering_type == "optimal" or clustering_type == "hc":
        dict_tree = hierarchical_clustering(
            df,
            cluster_name=cluster_name,
            is_relation_matrix=is_relation_matrix,
        ...
        if min_num_genes is None:
            min_num_genes = (
                int(len(df) * 0.01) if len(df) > 2000 else 20
            )  # set an optimal threshold for number of genes of a cluster to be subclustered (1% of all genes, minimum = 20 genes)
        dict_tree = recursive_clustering(
            df,
            cluster_name=cluster_name,
            n_recursion_lim=n_recursion_lim,
            thres_skewed_ratio=thres_skewed_ratio,
            is_relation_matrix=is_relation_matrix,
            cluster_start=cluster_start,
            min_num_genes=min_num_genes,
            n_max_cluster=n_max_cluster,
    ...
    if list_df is not None:
        if cluster_column_too:
            list_df_clustered = list(
                df.loc[clustered_IDs, clustered_IDs] for df in list_df
            )
        else:
        ...
        return (
            df.loc[clustered_IDs, clustered_IDs],
            dict_tree,
        )  # return one Clustered relation_matrix dataFrame
    else:
        if (
            axis == 0
        ):  # For asymmetric data (with different columns and indices), If 'axis' is 0, cluster columns. if 'axis' is 1, cluster indices.
...
def CALCULATE_Correlation_Matrix(
    ...
    df_for_correl_matrix = (
        df.loc[valid_Gene_Set, sample_list].dropna().transpose()
    )  # prepare a matrix for correlation matrix
    ...
    if (
        n_std_for_outliers is not None
    ):  # if 'n_std_for_outliers' has been given, create a mask for outliers for the use in subsequent calculation
        df_for_correl_matrix[
            OUTLIERS_GET_mask_for_outliers(
                data.T, n_std_for_outliers=n_std_for_outliers
            ).T
        ] = np.nan  # create a mask for outliers and replace outliers with np.nan
    if (
        correlation_type == "spearman"
    ):  # if correlation_type == 's', compute Spearman's correl coefficients
        if n_std_for_outliers is not None:
            df_correl_mat = df_for_correl_matrix.corr(
                method=correlation_type, min_periods=min_observation
            )
        else:
            df_correl_mat = pd.DataFrame(
                stats.spearmanr(data).correlation, index=Gene_IDs, columns=Gene_IDs
            )  # create DataFrame of correlation matrix
    elif (
        correlation_type == "pearson"
    ):  # if correlation_type == 'p', compute Perason's correl coefficients
        if n_std_for_outliers is not None:
            df_correl_mat = df_for_correl_matrix.corr(
                method=correlation_type, min_periods=min_observation
            )
            ...
            df_correl_mat = pd.DataFrame(
                np.corrcoef(data.T), index=Gene_IDs, columns=Gene_IDs
            )  # create DataFrame of correlation matrix
    else:
        df_correl_mat = df_for_correl_matrix.corr(
            method=correlation_type, min_periods=min_observation
...
def Correlation_Clustering(
    ...
    default Gene_Set : indices of df (df.index.values)"""
    df_correl_mat = CALCULATE_Correlation_Matrix(
        df=df,
        sample_list=sample_list,
        Gene_Set=Gene_Set,
        correlation_type=correlation_type,
        n_std_for_outliers=n_std_for_outliers,
        min_observation=min_observation,
    ).dropna()  # calculate correlation matrix
    _, dict_tree_clusters = Clustering(
        df_correl_mat,
        cluster_name=cluster_name,
        min_num_genes=min_num_genes,
        n_max_cluster=n_max_cluster,
    ...
    )  # perform a recursive clustering of df
    df_clustered_correl_mat = df_correl_mat.loc[
        dict_tree_clusters["Gene_IDs"], dict_tree_clusters["Gene_IDs"]
    ]  # rearrange indices and columns of df to cluster genes according to the result
...
def ORANGE_CLUSTERING_explore_tree_node(tree_node, list_left_to_right):
    ...
    if tree_node.is_leaf:
        ...
        list_left_to_right = ORANGE_CLUSTERING_explore_tree_node(
            tree_node.left, list_left_to_right
        )
        list_left_to_right = ORANGE_CLUSTERING_explore_tree_node(
            tree_node.right, list_left_to_right
        )
        return list_left_to_right
...
def print_cluster_tree(cluster_tree, print_cluster_location=True, threshold=50, d=0):
    ...
    """
    if (
        len(cluster_tree["Gene_IDs"]) < threshold
    ):  # if the number of gene of current cluster is less then a threshold, skip printing
        return
    if print_cluster_location:  # print information about current cluster
        print(
            "\t" * d,
            cluster_tree["cluster_name"],
            "number of genes :",
            len(cluster_tree["Gene_IDs"]),
            "\tstart and end position =",
            cluster_tree["cluster_start"],
            ":",
            cluster_tree["cluster_start"] + len(cluster_tree["Gene_IDs"]),
        )
    else:
        print(
            "\t" * d,
    ...
    if cluster_tree["subcluster_tree"] is not None:  # recursively plot subclusters
        for key, val in cluster_tree["subcluster_tree"].items():
            print_cluster_tree(
                val,
                print_cluster_location=print_cluster_location,
                threshold=threshold,
                d=d + 1,
            )  # recursion
...
bbox_props_white = dict(
    boxstyle="round", fc="w", ec="0.5", alpha=0.7
)  # define box properties that will be used in a function below for annotation
...
def Plot_corr_matrix__Gene_IDs(
    ...
    df_data=None,
    color_limit="auto",
    color_limit_infobar=None,
    color_map_diversing=True,
    show_inforbar_separately=True,
    color_map="bwr",
    Gene_IDs_interest=None,
    mask_thres=None,
    sample_list_min_by_max=None,
    add_Log2FC=False,
    n_factors=None,
    save_fig=False,
    show_grid=False,
    show_colorbar=True,
    Genes_of_interest_name="unknown_gene_set",
    show_title=True,
    mask_alpha=0.4,
    image_dpi=200,
    df_name="Data",
    ...
    title="",
    folder=None,
    gene_list_annotation=None,
    gene_annotation_text_size=10,
):
    ...
    if (
        (add_Log2FC or n_factors is not None) or sample_list_min_by_max is not None
    ) and df_data is None:  # set a defualt dataframe to create infobar and use them for ploting
        df_data = df_proteome_unshared
    Symbols_annotation, list_positions_annotation = GENE_Annotate_df_for_plot(
        df, gene_list_annotation
    )  # retrive symbols and positions of genes that will be annotated on the plot
    if type(df) is list:
        ...
        for a_ax, a_df, a_df_name, a_color_map in zip(ax, df, df_name, color_map):
            a_ax = subplot_relation_matrix__Gene_IDs(
                fig,
                a_ax,
                a_df,
                df_data=df_data,
                color_limit=color_limit,
                color_limit_infobar=color_limit_infobar,
                color_map_diversing=color_map_diversing,
                color_map=a_color_map,
                Gene_IDs_interest=Gene_IDs_interest,
                mask_thres=mask_thres,
                sample_list_min_by_max=sample_list_min_by_max,
                add_Log2FC=add_Log2FC,
                n_factors=n_factors,
                show_grid=show_grid,
                show_colorbar=show_colorbar,
                Genes_of_interest_name=Genes_of_interest_name,
        ...
        if (
            show_inforbar_separately and add_Log2FC
        ):  # if 'show_inforbar_separately' is true, create two axes and plot matrix and inforbar separately
            fig, axes = plt.subplots(1, 2, sharex=True, sharey=True)
            ax_matrix, ax_infobar = axes
            ax = subplot_relation_matrix__Gene_IDs(
                fig,
                ax_matrix,
                df,
                ax_infobar=ax_infobar,
                df_data=df_data,
                color_limit=color_limit,
                color_limit_infobar=color_limit_infobar,
                color_map_diversing=color_map_diversing,
        ...
        if (
            gene_list_annotation is not None
        ):  # if 'gene_list_annotation' is given, annotated each Gene_Symbol on the plot
            for Symbol, position in zip(Symbols_annotation, list_positions_annotation):
                ax.text(
                    ...
                    ha="right",
                    va="center",
                    size=gene_annotation_text_size,
                    bbox=bbox_props_white,
                )
    if save_fig:  # save and close figure if save_fig = True
        plt.savefig(
            folder + To_window_path_compatible_str(title) + ".png", dpi=image_dpi
        )
        plt.close()
    else:
        return fig, ax
...
def subplot_relation_matrix__Gene_IDs(
    fig,
    ax,
    df,
    ax_infobar=None,
    ...
    * a correlation matrix means index and columns have same labels, and element at i, j indicates some sort of relationship between i and j)
    ...
    if (
        Gene_IDs_interest is not None
    ):  # if 'Gene_IDs_interest' is given, plot matrix and infobar together (plotting separately is currently not supported)
        ax_infobar = None
    if (
        color_limit == "auto"
    ):  #  if 'color_limit' = 'auto', automatically set color_limit as mean + std * 1.2
        abs_data = np.abs(df.values)
        color_limit = abs_data.mean() + abs_data.std() * 1.2
    if (
        color_limit_infobar is None
    ):  # set color_limit as color_limit_infobar if its value has not been given
        color_limit_infobar = color_limit
    if (
        add_Log2FC and df_data is None
    ):  # set defualt dataframe for create Log2FC and use them for ploting
        df_data = df_proteome_unshared
    ...
    if add_Log2FC:  # add Log2FC inforbar if 'add_Log2FC' is set to True
        ...
        if (
            sample_list_min_by_max is not None
        ):  # 'sample_list_min_by_max' is not None, override Log2FC infobar with min_by_max values
            arr_Log2FC[gap_width:, :] = Calculate_min_by_max(
                df_data, sample_list=sample_list_min_by_max, Gene_Set=df.index.values
            ).values  # Calculate and broadcast min_by_max to the 2-D array for better visualization
        else:
            arr_Log2FC[gap_width:, :] = (
                Calculate_Log2FC_p_value__A_vs_B(df_data, Gene_IDs=df.index.values)
                .loc[df.index.values, "Log2_Fold_Change"]
                .values
            )  # Calculate and broadcast Log2FC to the 2-D array for better visualization
    ...
    if n_factors is not None:  # add Factor Analysis inforbar if 'n_factors' is not None
        ...
        df_FA = CALCULATE_Factor_Analysis__df(
            df_data, N_samples, n_factors=5, Gene_Set=df.index.values
        ).loc[
            df.index.values
        ]  # Perform Factor Analysis and rearrange the result according to the given relation_matrix
        for index_factor, data_of_a_factor in zip(np.arange(n_factors), df_FA.values.T):
            arr_FA[
                gap_width
                + index_factor
                * dislay_length_for_a_factor : (index_factor + 1)
                * dislay_length_for_a_factor,
                :,
            ] = data_of_a_factor  # broadcast data of a factor to the array that will be displayed
        ...
        if ax_infobar is None:
            corr = np.vstack(
                (corr.T, arr_FA)
            ).T  # add FA result to correlation matrix and Log2FC (if added)
            ax.annotate(
                label_Factor_Analysis,
                xy=(n + int(dislay_length_for_a_factor * n_factors / 2), 0),
                fontsize=10,
            )  # annotate FA result
        else:  # when inforbar is plotted separately
    ...
    if ax_infobar is not None or (
        not add_Log2FC and n_factors is None
    ...
    if color_map_diversing:  # set color_limit according to the type of a colormap
        im.set_clim(
            -color_limit, color_limit
        )  # set range of a colormap with color_limit value
    else:
    ...
    if (
        Gene_IDs_interest is not None
    ):  # if a list of Gene_IDs has been given, locate the positions of genes by overlaying a mask
        valid_Gene_IDs = list(
            set(df.index.values).intersection(Gene_IDs_interest)
        )  # retrive valid Gene_IDs (Gene_IDs that can be found in the dataframe)
        print(
            "number (percentage) of valid Gene_IDs :",
            len(valid_Gene_IDs),
            "(",
            len(valid_Gene_IDs) / len(Gene_IDs_interest) * 100,
            ")",
        )
        mask = deepcopy(
            df
        ...
        ax.imshow(
            mask, cmap="binary", alpha=mask_alpha
        )  # overlay a mask onto the correltion matrix
        title = title + "\n" + Genes_of_interest_name + " marked by black crosses"
    elif (
        mask_thres is not None
    ):  # if a threshold level has been given to locate the positions that has higher value than the threshold.
        ...
        title = (
            title
            + "\nvalues > "
            + str(round(mask_thres, 2))
            + " marked by black shades"
        )
...
def Find_position_in_correl_mat__Gene(
    df=None, Gene_of_interest="CRBN"
):  # set gene of interest. for example, ACAT1 as a mitochondrial marker
    """
    ...
    if gene_id == -1:
        ...
        return
    Gene_position = np.where(df.columns.values == gene_id)[0][
        0
    ]  # retrive a position of the gene
...
def Fine_cluster__Gene_or_Position_from_tree(
    cluster_tree, gene=None, position=None, d=0
):  # set gene of interest. for example, ACAT1 as a mitochondrial marker
    """
    ...
    if (gene is not None and gene in cluster_tree["Gene_IDs"]) or (
        position is not None and start_position <= position and position < end_position
    ):
        print(
            "\t" * d,
            cluster_tree["cluster_name"],
        ...
        if cluster_tree["subcluster_tree"] is not None:
            for key, val in cluster_tree["subcluster_tree"].items():
                Tree__find_cluster__Gene_or_Position(
                    val, gene=gene, position=position, d=d + 1
                )  # recursion
        elif (
            gene is not None
        ):  # if this is the last leaf node and gene is given, print the exact position of the gene
            print(
                "\t" * (d + 1),
                dict_ID_Symbol_simple[gene],
                "position :",
                start_position
                + np.where(np.array(cluster_tree["Gene_IDs"]) == gene)[0][0],
            )  # retrive a position of the gene and print the position of a given gene


# In[ ]:


def PLOT__A_Gene_Data_Correl_Mat(
    df,
    Gene_of_interest,
    df_name="unnamed",
    save_fig=False,
    ...
    plt.plot(
        range(len(data)), data, ".", alpha=alpha
    )  # plot the clustered correlation scores of the given gene with the other genes
    Gene_position = np.where(df.columns.values == Gene_ID)[0][
        0
    ]  # retrive a position of the gene
    plt.plot(
        Gene_position, 1, ".", label=Gene_Symbol
    )  # locate the given gene on a plot
    ...
    if save_fig:  # save figure if save_fig is True
        plt.savefig(
            graph_folder + Gene_Symbol + "clustered_corr_matrix__" + df_name + ".png",
            dpi=200,
        )
        plt.close()
...
def EXPLORE_Cluster(
    ...
    while True:
        ...
        Plot_corr_matrix__Gene_IDs(
            df=df_clus,
            df_data=df_data,
            show_inforbar_separately=False,
            color_map=color_map,
            add_Log2FC=add_Log2FC,
            Gene_IDs_interest=current_cluster_Gene_IDs,
            Genes_of_interest_name=current_cluster_name,
            df_name=df_name,
        )  # plot current cluster on relation matrix
        plt.show()
        subclusters = current_tree_node["subcluster_tree"]
        if (
            subclusters is None
        ):  # if subcluster_tree is None, skip displaying subcluster_tree
            print("current node is leaf node. No subclusters exists")
        else:
            dict_index__clus_name__num_genes = (
                dict()
            )  # build a dataframe to display subcluster_tree
            cluster_index = 0
            for subcluster_name, subcluster_node in subclusters.items():
                dict_index__clus_name__num_genes[cluster_index] = dict(
                    subcluster_name=subcluster_name,
                    num_genes=len(subcluster_node["Gene_IDs"]),
                )
                cluster_index += 1
        ...
        while decision == "":  # get an input
            print(
                "press 'q' and enter to quit exploring. press 'b' and enter to go back to parent cluster. press 's' to name of current cluster"
            )
            decision = input()
            if decision == "s":  # if 's' is entered, save current name to the list
                saved_cluster_names.append(
                    current_cluster_name
                )  # save current cluster name to the list, which will be returned once this interactive method ends
        ...
        if decision == "q":
            ...
            if (
                len(list_parent_tree_node) == 0
            ):  # if a stack list_parent_tree_node is empty, then current node is root_node, and print message and receive another input
                print(
                    "current cluster is the biggest cluster and do not have parent cluster"
                )
                continue
            else:
                current_tree_node = (
                    list_parent_tree_node.pop()
                )  # go back to immediate parent_tree_node
                continue
        elif (subclusters is not None) and int(
            decision
        ) in dict_index__clus_name__num_genes:  # if subcluster_tree is not empty, change current cluster to a subcluster according to the given index
            next_cluster_name = dict_index__clus_name__num_genes[int(decision)][
                "subcluster_name"
            ]  # retrive name of next cluster
            list_parent_tree_node.append(
                current_tree_node
            )  # push current node to the the parent node stack
            current_tree_node = subclusters[
                next_cluster_name
            ]  # set current cluster node
...
def PLOT_magnigfied_correl_matrix(
    ...
    Log2FC = (
        Calculate_Log2FC_p_value__A_vs_B(df_data, Gene_IDs=Gene_IDs)
        .loc[Gene_IDs, "Log2_Fold_Change"]
        .values
    )  # calculate Log2FC
    cax = ax.matshow(
        np.vstack((df.loc[Gene_IDs, Gene_IDs].values.T, Log2FC)).T,
        cmap="bwr",
        vmin=-color_limit,
        vmax=color_limit,
    )
    plt.xticks(
        range(len(Gene_Name_Symbols) + 1),
        Gene_Name_Symbols + ["Log2FC T/N"],
        rotation=270,
    )
    # add column labels
    ...
    plt.grid(show_grid)
    return pd.DataFrame(
        dict(Gene_Name_Symbols=Gene_Name_Symbols, Log2FC_T_vs_N=Log2FC),
...
def CLUSTER_GET_Gene_IDs_by_cluster_name(dict_tree_clus, cluster_name):
    ...
    if (
        dict_tree_clus["cluster_name"] == cluster_name
    ):  # if current cluster_name match a given cluster_name, return Gene_IDs of it
        return dict_tree_clus["Gene_IDs"]
    if (
        dict_tree_clus["subcluster_tree"] is None
    ):  # if current cluster_name is not matched with a given name and a leaf node, return -1
        return -1
    else:
        for subcluster_node in dict_tree_clus[
            "subcluster_tree"
        ].values():  # for each subcluster, recursively search the cluster name
            search_result = CLUSTER_GET_Gene_IDs_by_cluster_name(
                subcluster_node, cluster_name=cluster_name
            )  # recursion
            if search_result != -1:
...
def SETTING_MPL_Default_Property_Cycle(**dict_property):
    ...
    mpl.rcParams["axes.prop_cycle"] = (
        cycler(color=l_default_color, **dict_property)
        if dict_property.get("color", None) is None
        else cycler(**dict_property)
    )
...
def MPL_Scatter_Align_Two_Series(
    s_1,
    s_2,
    ls="",
    marker="o",
    alpha=0.5,
    show_xy_axis=(True, True),
    annotate_xy_axis=(False, False),
    annotate_color="blue",
    annotate_labels=None,
    annotate_outlier_in_both_x_and_y=True,
    annotate_distance_factor=1,
    n_std_for_outliers=3,
    label_split_char="_",
    figsize=(5, 5),
    **dict_mpl_basic_config,
    ...
    arr_1, arr_2, arr_labels = (
        s_1.values.astype(float),
        s_2.values.astype(float),
        s_1.index.values,
    )
    if label_split_char is not None and type(arr_labels[0]) is str:
        arr_labels = np.array(
            list(label.split("_")[0] for label in arr_labels)
        )  # split labels to reduce complexity of annotations
    ...
    min_1, max_1 = MPL_Internal_Util_get_min_max_for_plotting_an_axis(
        arr_1
    )  # retrive min, max, range of x and y values for plotting
    ...
    if show_xy_axis is not None:  # plot x and y axis
        ...
        arr_x_range, arr_y_range = np.arange(
            min_1 - step_x, max_1 + step_x + range_x * 0.01, step_x
        ), np.arange(min_2 - step_y, max_2 + step_y + range_y * 0.01, step_y)
        if show_xy_axis[0]:
    ...
    ax.plot(
        arr_1, arr_2, ls=ls, alpha=alpha, marker=marker
    )  # plot all data points in the two aligned series
    if (
        annotate_xy_axis is not None and annotate_xy_axis[0] or annotate_xy_axis[1]
    ):  # annotate points if valid 'annotate_xy_axis' is given
        if (
            annotate_labels is None
        ):  # if labels for annotations are not given, retrive annotate outliers by default
            mask_arr_1, mask_arr_2 = OUTLIERS_GET_mask_for_outliers(
                np.vstack((arr_1, arr_2)), n_std_for_outliers=n_std_for_outliers
            )  # retrive mask for outliers
            if (
                annotate_xy_axis[0] and annotate_xy_axis[1]
            ):  # retrive a mask for annotation of outliers for both x and y axis
                mask_arr = (
                    mask_arr_1 & mask_arr_2
                    if annotate_outlier_in_both_x_and_y
                    else mask_arr_1 | mask_arr_2
                )
        ...
        ax.plot(
            arr_1[mask_arr], arr_2[mask_arr], "o", color=annotate_color, alpha=0.7
        )  # annotate outliers
        for x, y, label in zip(arr_1[mask_arr], arr_2[mask_arr], arr_labels[mask_arr]):
            ax.annotate(
                ...
                (x, y),
                xytext=(
                    x + range_x * 0.01 * annotate_distance_factor,
                    y + range_y * 0.01 * annotate_distance_factor,
                ),
            )
...
def MPL_2D_Hist(
    ...
    **dict_mpl_basic_config,
    ...
    """Align two series and plot a scatter plots. Annotate outliers by default. Annotations of list of points are also availble via 'annotate_labels' arguments."""
    if isinstance(s_1, (pd.Series)) and isinstance(
        s_2, (pd.Series)
    ):  # align two series if series is given
        ...
        arr_1, arr_2 = s_1, s_2  # if numpy arrays were given
    fig, ax = plt.subplots(1, 1, figsize=figsize)
    colors_norm = (
        colors.LogNorm() if logscale else colors.Normalize()
    )  # set normalization method based on 'logscale' argument
    h, xedges, yedges, im = ax.hist2d(
        arr_1, arr_2, bins=bins, norm=colors_norm, cmap=cmap
    )
    if colorbar:
        plt.colorbar(im, ax=ax)  # show colorbar if 'colorbar' is true
    MATPLOTLIB_basic_configuration(**dict_mpl_basic_config)
...
def PLOT_Scatter_Annotation(
    ...
    **dict_matplotlib_basic_setting,
    ...
    l_colors = (
        ...
        if cmap is None
        else COLORMAP_GET_list_of_colors_from_list_of_values(
            np.arange(len(s_label)), return_RGB_hex_str=True, cmap=cmap
        )
    )  # set color map for plotting
    for label, count, color in zip(
        s_label.index, s_label.values, l_colors
    ):  # draw markers for samples in each cluster
        x_coordinates, y_coordinates = arr_coordinates[arr_labels == label].T
        ax.scatter(
            x_coordinates,
            y_coordinates,
            label="{Label} (N={N_labels})".format(Label=label, N_labels=int(count)),
            alpha=alpha,
            color=color,
            marker=marker,
            **dict_ax_scatter,
        )  # for each cluster, plot markers with different colors
    plt.legend()
...
def IMSHOW_expand_and_visualiz_narrow_arr(
    df,
    label_columns=None,
    width_column=None,
    cmap="bwr",
    ...
    if width_column == 1:
        ...
        arr_expanded = NUMPY_expand_narrow_arr(
            df, width_column=width_column
        )  # expand df to help visualization of DataFrame
    mask_nan = np.isnan(arr_expanded)  # retrive mask of NaN values
    width_column = int(
        arr_expanded.shape[1] / df.values.shape[1]
    )  # retrive the value of width_columns used to expand array
    fig, ax = plt.subplots(1, 1)
    im = ax.imshow(
        np.ma.masked_array(arr_expanded, mask_nan), cmap=cmap
    )  # show heatmap of data
    im.set_clim(color_limits[0], color_limits[1])
    im_nan = ax.imshow(
        np.ma.masked_array(np.zeros_like(arr_expanded), ~mask_nan), cmap="Greys_r"
    )  # show invalid values using a black color
    im_nan.set_clim(0, 1)
    ax.grid(show_grid)
    Symbols_annotation, list_positions_annotation = GENE_Annotate_df_for_plot(
        df, gene_list_annotation
    )  # retrive position and symbols of genes (indices) that will be annotated
...
def OUTLIERS_GET_data_values_without_outliers(arr_values, n_std_for_outliers=2):
    ...
    mask_outliers = OUTLIERS_GET_mask_for_outliers(
        np.array([arr_values], dtype=float), n_std_for_outliers=n_std_for_outliers
    )[
        0
    ]  # remove outliers from list_values to map scalar values to color effectively
...
def OUTLIERS_GET_min_max_after_outlier_removal(arr_values, n_std_for_outliers=2):
    ...
    arr_values_without_outliers = OUTLIERS_GET_data_values_without_outliers(
        arr_values, n_std_for_outliers=n_std_for_outliers
    )  # remove outliers from list_values to map scalar values to color effectively
    return (
        arr_values_without_outliers.min(),
        arr_values_without_outliers.max(),
    )  # retrive min and max of values without outliers


# In[ ]:


def BOKEH_transform_arr_values_into_a_range_from_0_to_1(
    arr_values, n_std_for_outliers=2, log_transform=False, inverse=False
):
    ...
    if (
        inverse
    ):  # if inverse is True, set the maximum value to 0 and change the sign so that ranks of data_values becomes the opposite while all data_values remain positive
        arr_values = arr_values.max() - arr_values
    else:
        arr_values = (
            arr_values - arr_values.min()
        )  # set the minimun value to 0 so that all data_values become positive
    if (
        n_std_for_outliers is None
    ):  # if None is given through 'n_std_for_outliers' argument, do not exclude outliers when retriving min and max values
        arr_values_without_outliers_min, arr_values_without_outliers_max = (
            arr_values.min(),
            arr_values.max(),
        )
    else:
        (
            arr_values_without_outliers_min,
            arr_values_without_outliers_max,
        ) = OUTLIERS_GET_min_max_after_outlier_removal(
            arr_values, n_std_for_outliers=n_std_for_outliers
        )  # retrive min and max of values without outliers
...
def COLORMAP_GET_list_of_colors_from_list_of_values(
    list_values,
    cmap="viridis",
    n_std_for_outliers=2,
    return_RGB_hex_str=False,
    log_transform=False,
    ...
    if (
        n_std_for_outliers is None
    ):  # if 'n_std_for_outliers' is None, do not exclude outliers when retriving min and max values
        arr_values_without_outliers_min, arr_values_without_outliers_max = (
            list_values.min(),
            list_values.max(),
        )
        ...
        (
            ...
            list_values, n_std_for_outliers=n_std_for_outliers
        )  # retrive min and max of values without outliers
    if (
        arr_values_without_outliers_max == 0
    ):  # if resulting 'arr_values_without_outliers_max' is zero, retrive maximum value without considering zero values
        (
            arr_values_without_outliers_min,
            arr_values_without_outliers_max,
        ) = OUTLIERS_GET_min_max_after_outlier_removal(
            list_values[list_values != 0], n_std_for_outliers=n_std_for_outliers
        )
    if (
        cmap in cm.__dict__
    ):  # if given color_map is valid, use the scalar color mapper of a given color_map name
    ...
    if (
        arr_values_without_outliers_min < 0
    ):  # if minimum value is negative (regardless of the sign of arr_values_without_outliers_max), increase arr_values_without_outliers_max by arr_values_without_outliers_min so that arr_values_without_outliers_max becomes 1 after transformation
    ...
    arr_color = np.array(
        list(
            colormapper(
                int(
                    255
                    * float(value - arr_values_without_outliers_min)
                    / arr_values_without_outliers_max
                )
            )[:3]
            for value in list_values
        )
    )  # map a given list of values with color_map of a given color_map name to colors and return the list of colors as a np.array
    if (
        return_RGB_hex_str
    ):  # if 'return_RGB_hex_str' is set to True, convert RGB color tuple into RGB hex string and return list of RGB hex strings
        arr_color_0_to_255 = np.array(
            arr_color * 255, dtype=int
        )  # convert 0 ~ 1 RGB values into 0 ~ 255 RGB integer values
        return np.array(
            list(
                "#%02x%02x%02x"
                % (color_0_to_255[0], color_0_to_255[1], color_0_to_255[2])
                for color_0_to_255 in arr_color_0_to_255
            )
        )  # convert RGB color tuple into RGB hex string
...
def VERIFICATION_boxplot_distribution_of_ranks_of_genes_for_each_sample(
    df,
    sample_list,
    Gene_Set=None,
    save_fig=False,
    ...
    sample_list_sorted, _ = ARRAY_sort_one_list_based_on_another(
        sample_list, arr_sample_ranks_median
    )
    if show_plot:  # if show_plot is True, draw a plot of boxplots
        plt.figure(figsize=(18, 12))
        plt.boxplot(
            arr_sample_ranks.T[indices_sorting_rank_median].T,
            widths=0.85,
            showmeans=True,
            notch=True,
            showfliers=False,
        ...
        title = (
            dataset_name
            + " Distribution of Ranks of '{}' Genes in {} Among {} Samples".format(
                name_Gene_Set, name_df, name_sample_list
            )
        )
        plt.title(title)
        plt.ylabel("Distribution of Ranks of Given Genes for Each Sample.")
        plt.xticks(
            np.arange(1, len(sample_list) + 1),
            sample_list_sorted,
            rotation=90,
            fontsize=9,
        )
...
def PLOT_GEL_BLOTs(
    ...
    Gene_Set,
    df=None,
    method_data_heatmap="max",
    sample_labels=None,
    add_Gene_Name=True,
    width_blot=0.8,
    cmap="Greys_r",
    textcolors=["black", "white"],
    text_color_threshold=50,
    threshold_bothends=None,
    title=None,
    show_colorbar=False,
    valfmt="{x:.2e}",
    aspect=0.3,
    ...
    df = df.replace(
        0, np.nan
    ).dropna()  # if mask_nan_color is None else df.replace( 0, np.nan ) # drop entries with zero values if None values are not masked. Mask zero values with np.nan values
    if isinstance(
        df.index.values[0], (int, float, np.float64, np.int64)
    ):  # if the given df's indices are gene_ids
        Gene_IDs = List_Gene__2__List_Gene_ID(Gene_Set)
        valid_Gene_IDs = Gene_IDs[
            GET_MASK_of_intersection(Gene_IDs, df.index.values)
        ]  # retrive valid Gene_IDs while preserving the order of a given list of genes
        ...
        valid_Gene_IDs, Gene_Symbols, add_Gene_Name, width_blot, fontsize_index = (
            Gene_Set,
            Gene_Set,
            False,
            width_blot / 2,
            17,
        )  # set values for plotting df whose indices are not gene_id
    data = df.loc[valid_Gene_IDs].values
    if (
        np.sum(data < 0) > 0
    ):  # if data contain negative values, set color limit automatically
        method_data_heatmap, valfmt = (-1, 0), "{x:.2f}"
    if method_data_heatmap == "max":
        data_heatmap, color_limits, text_color_threshold = (
            (data.T / data.max(axis=1)).T * 100,
            (0, 100),
            50,
        )  # Normalize data with its maximum values, set color limits, and threshold for text color.
    elif method_data_heatmap == "std_positive":
        data_heatmap, color_limits, text_color_threshold = (
            (data.T / np.power(2, np.log2(data).mean(axis=1)) + data.std(axis=1) * 2).T
            * 100,
            (0, 100),
            50,
        )  # Normalize data with its log-avg + 2 std, set color limits, and threshold for text color.
    elif method_data_heatmap == "std_zero":
        data_heatmap, color_limits, text_color_threshold = (
            (data.T / data.std(axis=1) * 2).T * 100,
            (-100, 100),
            0,
        )  # Normalize data with 2 * std, set color limits, and threshold for text color.
    elif type(method_data_heatmap) is tuple:
        data_heatmap, color_limits, text_color_threshold = (
            data,
            method_data_heatmap,
            np.mean(method_data_heatmap),
        )  # do not normalize data, set color limits, and threshold for text color.
    else:
        return -1
    fig, ax = plt.subplots(
        figsize=(
            5 + int(len(valid_sample_list) * width_blot) + 2 * int(add_Gene_Name),
            2 + int(len(valid_Gene_IDs) / 2) + len(valid_sample_list[0]) / 5,
        )
    )
    ax.grid(False)  # Turn off grid
    if (
        sample_labels is None
    ):  # draw heatmap # if sample_labels has not be separately given, use sample names to annotate each sample
        im, cbar = heatmap(
            data_heatmap,
            row_labels=Gene_Symbols,
            col_labels=valid_sample_list,
            ax=ax,
            show_colorbar=show_colorbar,
            cmap=cmap,
            cbarlabel=data_label,
            color_limit=color_limits,
            grid_color=grid_color,
            grid_width=grid_width,
            cbar_kw=dict(aspect=3),
            fontsize_x=fontsize_col,
            fontsize_y=fontsize_index,
        )
    else:
        im, cbar = heatmap(
            data_heatmap,
            row_labels=Gene_Symbols,
            col_labels=sample_labels,
            ax=ax,
            show_colorbar=show_colorbar,
            cmap=cmap,
            cbarlabel=data_label,
    ...
    if threshold_bothends is not None:
        texts = annotate_heatmap(
            ...
            data,
            valfmt=valfmt,
            textcolors=textcolors[::-1],
            threshold_bothends=threshold_bothends,
            fontsize=fontsize_blot,
        )
    else:
        texts = annotate_heatmap(
            im,
...
basic_bokeh_label_kw = dict(
    x_offset=5, y_offset=5, text_font_size="10pt", render_mode="canvas"
)
...
def Bokeh_scatter(
    ...
    y_axis_type="linear",
    color=None,
    color_default="#2326EE",
    color_log_transform=False,
    color_n_std_for_outliers=None,
    cmap="viridis_r",
    colormap_diverging=False,
    size=None,
    size_factor=30,
    size_log_transform=True,
    size_inverse=True,
    size_n_std_for_outliers=2,
    annotation=None,
    df_filtered_for_annotation=None,
    show_annotation_kw=basic_bokeh_label_kw,
    title="Scatter Plot",
    save_html=False,
    plot_size=700,
    URL="https://www.google.com/search?q=@",
    alpha_fill_quadrants=0.2,
    l_column_for_hover_tool=None,
    graph_folder=None,
):
    """General Function to draw interactive Bokeh plot"""
    if (
        color is not None and "p_value" in color
    ):  # optimize setting if 'p_value' was given as one of the columns
    ...
    df = deepcopy(
        df
    ).reset_index()  # reset index so that index can be also used to plot graphs
    dict_result = df.to_dict(
        orient="list"
    )  # build bokeh source data dictionary from a dataframe
    dict_result["size"] = (
        list(np.full(len(df), size_factor / 3))
        if size is None
        else BOKEH_transform_arr_values_into_a_range_from_0_to_1(
            dict_result[size],
            n_std_for_outliers=size_n_std_for_outliers,
            log_transform=size_log_transform,
            inverse=size_inverse,
        )
        * size_factor
    )  # set data for sizes of markers
    dict_result["color"] = (
        list(np.full(len(df), color_default))
        if color is None
        else COLORMAP_GET_list_of_colors_from_list_of_values(
            dict_result[color],
            n_std_for_outliers=color_n_std_for_outliers,
            cmap=cmap,
            return_RGB_hex_str=True,
            log_transform=color_log_transform,
            diverging_colormap=colormap_diverging,
        )
    )  # set data for colors of markers
    # create source data
    ...
    TOOLTIPS = [
        ("{x}".format(x=x_axis), "@{x}".format(x=x_axis)),
        ("{y}".format(y=y_axis), "@{y}".format(y=y_axis)),
    ]
    if size is not None:
        TOOLTIPS.append(
            ("Size ({size})".format(size=size), "@{size}".format(size=size))
        )
    if color is not None:
        TOOLTIPS.append(
            ("Color ({color})".format(color=color), "@{color}".format(color=color))
        )
    if annotation is not None:
        TOOLTIPS.append(
            (
                "Annotation ({annotation})".format(annotation=annotation),
                "@{annotation}".format(annotation=annotation),
            )
        )
    if (
        l_column_for_hover_tool is not None
    ):  # if 'l_column_for_hover_tool' is given, add give list of columns to TOOLTIPs for hover tool visualization
        for col in l_column_for_hover_tool:
            TOOLTIPS.append((col, "@" + col))
    #     title += '   color ({}) and size ({})'.format( color, size ) # add color and size annotation to the title
    p1 = figure(
        tools="box_select,lasso_select,tap,pan,box_zoom,wheel_zoom,reset,save,hover",
        tooltips=TOOLTIPS,
        title=title,
        x_axis_label=x_axis,
        y_axis_label=y_axis,
    ...
    p1.add_layout(
        BoxAnnotation(
            right=0, bottom=0, fill_alpha=alpha_fill_quadrants, fill_color="black"
        )
    )  # add box annotations to mark boundaries
    p1.add_layout(
        BoxAnnotation(
            left=0, top=0, fill_alpha=alpha_fill_quadrants, fill_color="black"
        )
    )
    p1.circle(
        x_axis,
    ...
    if annotation is not None:
        ...
        )  # call back using column given as 'annotation' column
        if (
            df_filtered_for_annotation is not None
        ):  # if both annotation and df_filtered_for_annotation were given, show annotations on the scatter plot
            source_filtered = ColumnDataSource(
                data=df_filtered_for_annotation.to_dict(orient="list")
            )  # build bokeh source data dictionary of a dataframe after filtering
            labels = LabelSet(
                x=x_axis,
                y=y_axis,
                text=annotation,
...
def BOKEH_PLOT_interactive_scatter_plot_log2fc(
    ...
    y_axis_type="linear",
    show_symbol=True,
    show_symbol_filter_kw=basic_filter,
    show_symbol_kw=basic_bokeh_label_kw,
    title="Scatter Plot",
    save_html=False,
    plot_size=700,
    tap_callback="google",
    ...
    dict_result["size"] = (
        BOKEH_transform_arr_values_into_a_range_from_0_to_1(
            dict_result[size],
            n_std_for_outliers=size_n_std_for_outliers,
            log_transform=size_log_transform,
            inverse=size_inverse,
    ...
    dict_result["color"] = COLORMAP_GET_list_of_colors_from_list_of_values(
        dict_result[color],
        n_std_for_outliers=color_n_std_for_outliers,
        cmap=color_map,
        return_RGB_hex_str=True,
    ...
    source_filtered = ColumnDataSource(
        data=df_filtered.to_dict(orient="list")
    )  # build bokeh source data dictionary of a dataframe after filtering
    # set output type (save figure as a html file)
    if save_html:
        reset_output()  # reset output before plotting
    ...
    # set hovering tool according to the type of graph
    TOOLTIPS = [
        ("Gene Name (Symbol)", "@Approved_Name"),
        ("{x}".format(x=x_axis), "@{x}".format(x=x_axis)),
        ("{y}".format(y=y_axis), "@{y}".format(y=y_axis)),
        ("{size}".format(size=size), "@{size}".format(size=size)),
        ("{color}".format(color=color), "@{color}".format(color=color)),
    ]
    title += "   color ({}) and size ({})".format(
        color, size
    )  # add color and size annotation to the title
    p1 = figure(
        tools="box_select,lasso_select,tap,pan,box_zoom,wheel_zoom,reset,save,hover",
        tooltips=TOOLTIPS,
        title=title,
    ...
    fill_alpha = 0.3
    p1.add_layout(
        BoxAnnotation(right=0, bottom=0, fill_alpha=fill_alpha, fill_color="black")
    )  # add box annotations to mark boundaries
    p1.add_layout(
        BoxAnnotation(right=0, top=0, fill_alpha=fill_alpha, fill_color="black")
    )
    p1.add_layout(
        BoxAnnotation(left=0, top=0, fill_alpha=fill_alpha, fill_color="black")
    )
    # plot two groups with colors
    p1.circle(
        x_axis,
    ...
    if (
        tap_callback.lower() == "google"
    ):  # set URL that will be directed if marker is clicked
...
def BOKEH_PLOT_interactive_volcano_plot(
    df,
    x_axis="Log2_Fold_Change",
    y_axis="adjusted_p_value",
    x_axis_type="linear",
    y_axis_type="log",
    show_symbol=True,
    show_symbol_filter_kw=basic_filter,
    show_symbol_gene_ids=None,
    show_symbol_kw=basic_bokeh_label_kw,
    title=None,
    save_html=False,
    plot_size=700,
    ...
    df_filtered = (
        RESULT_filter(df, **show_symbol_filter_kw)
        if show_symbol_gene_ids is None
        else RESULT_filter(PD_Subset(df, show_symbol_gene_ids), **show_symbol_filter_kw)
    )
    source_filtered = ColumnDataSource(
        data=df_filtered.to_dict(orient="list")
    )  # build bokeh source data dictionary of a dataframe after filtering
    col_a, col_b = list(
        col for col in df.columns if "average" in col and col != "average_all"
    )  # retrive column names for condition A and B
    title = (
        "({}) VolcanoPlot {} vs {}".format(
            dataset_name, col_a.replace("_average", ""), col_b.replace("_average", "")
        )
        if title is None
        else title
    )  # set title based on the names of columns
    ...
    TOOLTIPS = [
        (
            "{x_axis_short}, {y_axis_short}, p val".format(
                x_axis_short=dict_shorthand[x_axis], y_axis_short=dict_shorthand[y_axis]
            ),
            "@{x_axis}, @{y_axis} @p_value".format(x_axis=x_axis, y_axis=y_axis),
        ),
        ("Gene Name (Symbol)", "@Approved_Name"),
        ("Gene_ID", "@Gene_ID"),
        ("Cond. A vs B avg.", "@{} vs @{}".format(col_a, col_b)),
    ]
    p1 = figure(
        tools="box_select,lasso_select,tap,pan,box_zoom,wheel_zoom,reset,save,hover",
        tooltips=TOOLTIPS,
    ...
    p1.circle(
        x_axis, y_axis, size=8, source=source, color="navy", alpha=0.3
    )  # plot two groups with colors
    p1.circle(x_axis, y_axis, size=10, source=source_filtered, color="navy", alpha=1)
    decreased_box = BoxAnnotation(
        right=0, fill_alpha=0.1, fill_color="green"
    )  # add box annotations # annotate upregulated genes on green background
    increased_box = BoxAnnotation(
        left=0, fill_alpha=0.1, fill_color="red"
    )  # annotate upregulated genes on red background
    p1.add_layout(decreased_box)
    p1.add_layout(increased_box)
    taptool = p1.select(
        type=TapTool
    )  # open NCBI page or Google search result with Entrez Gene ID when clicked
    if (
        tap_callback.lower() == "google"
    ):  # set URL that will be directed if marker is clicked
...
basic_filter_geneset = dict(
    thres_abs_log2fc=0.5, thres_p_value=0.05, label_p_value="adjusted_p_value"
)


def BOKEH_PLOT_interactive_volcano_plot__Gene_Sets(
    df,
    x_axis="Log2_Fold_Change",
    y_axis="adjusted_p_value",
    x_axis_type="linear",
    y_axis_type="log",
    color="Log2_Fold_Change_std",
    color_n_std_for_outliers=2,
    color_log_transform=False,
    colormap_diverging=False,
    size="number_valid_genes",
    size_log_transform=False,
    size_inverse=False,
    size_n_std_for_outliers=2,
    show_symbol=True,
    show_symbol_filter_kw=basic_filter,
    ...
    df = df.dropna()
    dict_shorthand = dict(
        Log2_Fold_Change="Log2FC", adjusted_p_value="adj p", p_value="p val"
    )  # define dictionary of shorthand annotation of possible x and y axes
    dict_result = df.to_dict(
        orient="list"
    )  # build bokeh source data dictionary from a dataframe
    ...
    # create source data
    source = ColumnDataSource(data=dict_result)
    df_filtered = RESULT_filter(
        df, **show_symbol_filter_kw
    )  # build bokeh source data dictionary of a dataframe after filtering
    dict_result_filtered = df_filtered.to_dict(orient="list")
    dict_result_filtered["Gene_Set_Name"] = list(
        df_filtered.index.values
    )  # retrive Gene_Set_names
    ...
    TOOLTIPS = [
        (
            "{x_axis_short}, {y_axis_short}".format(
                x_axis_short=dict_shorthand[x_axis], y_axis_short=dict_shorthand[y_axis]
            ),
            "@{x_axis}, @{y_axis}".format(x_axis=x_axis, y_axis=y_axis),
        ),
        ("Cond. A vs B avg.", "@condition_A_average vs @condition_B_average"),
        ("Name", "@Gene_Set_Name"),
        ("n_genes", "@number_valid_genes"),
    ]
    p1 = figure(
        tools="box_select,lasso_select,tap,pan,box_zoom,wheel_zoom,reset,save,hover",
        tooltips=TOOLTIPS,
    ...
    decreased_box = BoxAnnotation(
        right=0, fill_alpha=0.1, fill_color="green"
    )  # annotate upregulated genes on green background
    increased_box = BoxAnnotation(
        left=0, fill_alpha=0.1, fill_color="red"
    )  # annotate upregulated genes on red background
    p1.add_layout(decreased_box)
...
def BOKEH_PLOT_Scatter_Annotation(
    arr_coordinates,
    arr_labels,
    ...
    **dict_attributes,
    ...
    l_colors = (
        ...
        if color_map is None
            ...
            np.arange(len(s_label)), return_RGB_hex_str=True, color_map=color_map
        )
    )  # set color map for plotting
    arr_colors = np.zeros_like(arr_labels)
    for label, color in zip(
        s_label.index, l_colors
    ):  # prepare an array of colors matching labels
        arr_colors[arr_labels == label] = color
    l_columns = ["x", "y", "label", "color"]
    arr_data = np.vstack(
        (arr_coordinates.T, arr_labels, arr_colors)
    )  # stack all data_values (coordinates, color, and additional attributes) into one array to make a ColumnDataSource
    ...
    source = ColumnDataSource(
        data=dict((col, data) for col, data in zip(l_columns, arr_data))
    )
    if save_html:
        reset_output()  # reset output before plotting
        output_file(graph_folder + To_window_path_compatible_str(title) + ".html")
        ...
        output_notebook()
    TOOLTIPS = [("x", "@x"), ("y", "@y"), ("label", "@label")] + list(
        ("{}".format(col_label), "@{}".format(col_label)) for col_label in l_columns[4:]
    )
    p = figure(
        tools="box_select,lasso_select,tap,pan,box_zoom,wheel_zoom,reset,save,hover",
        tooltips=TOOLTIPS,
...
def subplot_two_genes(
    ...
    alpha=0.5,
    n_std_for_outliers=3,
    add_regression_line_normal=True,
    regression_type="intercept zero",
    show_correl="N",
    external_gene_1_data=None,
):
    """
    ...
    if (
        ...
        series_gene_1_data, series_gene_2_data = (
            df[All_samples].loc[gene_id_1],
            df[All_samples].loc[gene_id_2],
        )
    # perform correlation analysis
    if (
        show_correl is not None
    ):  # set sample_set according to a given 'show_correl' argument
        if show_correl in dict_argument__samples_name:
            sample_set, name_samples = (
                dict_type_sample[dict_argument__samples_name[show_correl][0]],
                dict_argument__samples_name[show_correl][1],
            )  # set sample_set and name_samples
        else:  # by default if other than None is given, sample_set is normal and tumor samples
            sample_set, name_samples = (
                dict_type_sample["normal_or_tumor"],
                "All Samples",
            )
        gene_1_data, gene_2_data = GET_non_NaN_without_outliers(
            series_gene_1_data[sample_set].values,
            series_gene_2_data[sample_set].values,
            n_std_for_outliers=n_std_for_outliers,
        )  # retrive data for two genes and remove outliers and NaN values
        correl_p = stats.pearsonr(gene_1_data, gene_2_data)[
            0
        ]  # calculate Pearson's correlation
        correl_s = stats.spearmanr(
            gene_1_data, gene_2_data
        ).correlation  # calculate Spearman's correlation
    # retrive data and plot the samples of each type
    if normal_type:
        gene_1_normal = series_gene_1_data[
            N_samples
        ].values  # retrive data for normal samples
        gene_2_normal = series_gene_2_data[N_samples].values
        if (
            add_regression_line_normal
        ):  # if 'add_regression_line_normal' is set to True, draw a regression line for normal samples
            label = "(normal samples)"
            gene_1_data, gene_2_data = GET_non_NaN_without_outliers(
                gene_1_normal, gene_2_normal, n_std_for_outliers=n_std_for_outliers
            )  # remove outliers and NaN values
            x_range = np.arange(
                0, gene_1_normal.max(), 0.05
            )  # define x_range of a regression line
            if regression_type == 0:
                slope = np.linalg.lstsq(
                    np.array([gene_1_data]).T, gene_2_data, rcond=None
                )[0][
                    0
                ]  # solving an equation a x = b to find the best slop for the data
                line = slope * x_range  # calculate a line from x_range
                label += " Least Square Through Origin"
            elif regression_type == "th":
                slope, intercept, lower, higher = stats.theilslopes(
                    gene_2_data, gene_1_data
                )  # calculate theilslopes and plot regression line
                line, line_lower, line_higher = (
                    (slope * x_range) + intercept,
                    (lower * x_range) + intercept,
                    (higher * x_range) + intercept,
                )  # calculate lines from x_range
                label += " Theil-Sen"
                ax.plot(
                    x_range, line_lower, "--", color="brown", alpha=0.7
                )  # draw a regression of lower and upper estimation of slopes
                ...
                (
                    slope,
                    intercept,
                ) = LINREGRESS_Projection_Based_Least_Square_Linear_Regression(
                    gene_1_data, gene_2_data, np.vstack((gene_1_data, gene_2_data))
                )  # perform projection-based linear regression to calculate slope and intercept
                line = (slope * x_range) + intercept  # calculate a line from x_range
        ...
        ax.plot(
            ...
            label="normal",
            alpha=alpha,
        )  # plot normal samples after regression line (if it has been drawn)
    if tumor_type:
        if grade_color:
            categories = ["G1", "G2", "G3", "G4"]  # set categories
            colors = ["burlywood", "Coral", "red", "magenta"]
            data_gene_1 = get_data__tumor_grades(
                gene_id_1, df
            )  # retrive tumor_grade-wise data
            data_gene_2 = get_data__tumor_grades(gene_id_2, df)
            for category, color in zip(categories, colors):
                ax.plot(
                    data_gene_1[category],
                    data_gene_2[category],
                    "o",
                    color=color,
                    linewidth=10,
                    label=category,
    ...
    if (
        show_correl is not None
    ):  # if 'show_correl' is not None, show correlation coefficient data
        title += "\n({name}, Correl. Pearson = {c_p}, Spearman = {c_s})".format(
            name=name_samples, c_p=round(correl_p, 3), c_s=round(correl_s, 3)
        )
    ax.set_title(title)
    ax.legend()  # add legends to the plots
    sample_set = (
        tumor_type * T_samples + normal_type * N_samples
    )  # define sample_set that is currently shown
    gene_1_data, gene_2_data = GET_non_NaN_without_outliers(
        series_gene_1_data[sample_set].values,
        series_gene_2_data[sample_set].values,
        n_std_for_outliers=n_std_for_outliers,
    )  # retrive data that is shown and remove outliers and NaN values
    x_lim = (
        gene_1_data.mean() + n_std_for_outliers * gene_1_data.std()
    )  # set x_lim and y_lim by using mean and std of data
...
def Plot_two_genes__normal_vs_tumor(
    gene_1="CRBN",
    gene_2="ACAT1",
    data_phospho=False,
    switch_genes=False,
    ...
    save_fig=False,
    n_std_for_outliers=3,
    add_regression_line_normal_1=True,
    add_regression_line_normal_2=False,
    regression_type="proj",
    show_correl_1="N",
    show_correl_2=None,
    graph_folder=None,
):
    ...
    if type(gene_1) is pd.Series:
        ...
        if data_phospho:
            ...
            Gene_1_Symbol, Gene_2_Symbol = (
                df_phosphoproteome.loc[gene_1].Gene_Symbol,
                df_phosphoproteome.loc[gene_2].Gene_Symbol,
            )
        else:
            gene_id_1, gene_id_2 = Gene_2_Gene_ID(gene_1), Gene_2_Gene_ID(gene_2)
            if (
                gene_id_1 not in df.index.values or gene_id_2 not in df.index.values
            ):  # if gene_id_1 or gene_id_2 do not exist in df, return an error value
                return -1
            Gene_1_Symbol, Gene_2_Symbol = (
                dict_ID_Symbol_simple[gene_id_1],
                dict_ID_Symbol_simple[gene_id_2],
            )
    df_name = "{dataset_name} {data_name}".format(
        dataset_name=dataset_name, data_name=df_name
    )
    if (
        df_2 is not None or switch_genes
    ):  # if second dataFrame has been given or genes are switched, draw two subplots
        fig, ax = plt.subplots(1, 2, figsize=(15, 6), sharey=True, sharex=True)
        _, x_lim_1, y_lim_1 = subplot_two_genes(
            ax[0],
            gene_id_1=gene_id_1,
            Gene_1_Symbol=Gene_1_Symbol,
            gene_id_2=gene_id_2,
            Gene_2_Symbol=Gene_2_Symbol,
            df=df,
            df_name=df_name,
            ...
            alpha=alpha,
            n_std_for_outliers=n_std_for_outliers,
            add_regression_line_normal=add_regression_line_normal_1,
            regression_type=regression_type,
            show_correl=show_correl_1,
            external_gene_1_data=gene_1,
        )
        if df_2 is not None:
            _, x_lim_2, y_lim_2 = subplot_two_genes(
                ax[1],
                gene_id_1=gene_id_1,
                Gene_1_Symbol=Gene_1_Symbol,
                gene_id_2=gene_id_2,
                ...
                alpha=alpha,
                n_std_for_outliers=n_std_for_outliers,
                add_regression_line_normal=add_regression_line_normal_2,
                regression_type=regression_type,
                show_correl=show_correl_2,
                external_gene_1_data=gene_1,
            )
        ...
        x_lim, y_lim = max(x_lim_1, x_lim_2), max(
            y_lim_1, y_lim_2
        )  # set an overall x_lim and y_lim by selecting larger values of x_lim and y_lim
    else:
        fig, ax = plt.subplots(1, 1, sharey=True)
        _, x_lim, y_lim = subplot_two_genes(
            ax,
            gene_id_1=gene_id_1,
            Gene_1_Symbol=Gene_1_Symbol,
            gene_id_2=gene_id_2,
    ...
    if save_fig:
        if data_phospho:
            plt.savefig(
                graph_folder
                + To_window_path_compatible_str(
                    df_name + "_two_gene_" + gene_1 + "_" + gene2 + ".png"
                ),
                dpi=200,
            )
        else:
...
def Change__normal_vs_tumor__Pathway_or_Gene_Set(
    Pathway=None, datatype="std", process_type="average", Gene_Set=None
):
    """
    ...
    # calculate number of valid genes and total number of genes of the pathway or set and print the values
    valid_gene_set = set(
        df_proteome[list(dict_type_sample["normal_or_tumor"])]
        .loc[existing_gene_set]
        .dropna()
        .index
    )
    num_valid_genes = len(valid_gene_set)
    if num_valid_genes == 0:  # if there is no valid genes, end the method
        ...
        return -1
    coverage = round(
        num_valid_genes / num_pathway_genes * 100, 1
    )  # calculate pathway or gene set coverage
    # data values
    if (
        process_type == "average"
    ):  # compute average or sum of values to assess pathway activity
        normal_data = np.average(
            df[list(dict_type_sample["normal"])].loc[valid_gene_set].iloc[:, 1:].values,
            axis=0,
        )
        tumor_data = np.average(
            df[list(dict_type_sample["tumor"])].loc[valid_gene_set].iloc[:, 1:].values,
            axis=0,
        )
    else:
        normal_data = np.sum(
    ...
    if datatype == "std":
        ...
        change_in_tumor = (
            "percent",
            np.average(tumor_data) / np.average(normal_data) * 100 - 100,
        )
        change_in_tumor__type = "percent"
    return dict(
        num_valid_genes=num_valid_genes,
        num_pathway_genes=num_pathway_genes,
        datatype=datatype,
        p_value=p_value,
        change_in_tumor=change_in_tumor,
        change_in_tumor__type=change_in_tumor__type,
...
def PLOT_box_plot__heatmap__normal_vs_tumor__Pathway_or_Gene_Set(
    df=None,
    Pathway=None,
    datatype="std",
    process_type="average",
    ...
    if Pathway is not None:
        ...
        Gene_List = dict_pathway[Pathway]["genes"]
        num_pathway_genes = len(Gene_List)  # calculate the number of genes in a pathway
        Gene_Set_name = Pathway.split("  ", 1)[
            1
        ]  # handle name of pathway that will be used in the plots
    elif Gene_Set is not None:
        Gene_List = list(Gene_Set)
        num_pathway_genes = len(Gene_List)
    else:
    ...
    valid_gene_set = set(
        df[list(dict_type_sample["normal_or_tumor"])]
        .loc[existing_gene_set]
        .dropna()
        .index
    )
    ...
    if displaying:  # display the result only when displaying = True
        print(
            num_valid_genes, " / ", num_pathway_genes, " genes exist in proteome data"
        )
    ...
    plt.savefig(
        graph_folder + title.replace("/", "_").replace("\n", " ") + ".png", dpi=200
    )  # save figure at graph folder
    plt.close()
    ################################################ Ploting gene-wise heatmap ####################################
    # convert the list of valid Gene_ID to list of Gene_Symbols
    list_valid_gene = sorted(
        list(valid_gene_set)
    )  # convert gene set to an ordered data structure, a list
    ...
    fig, ax = plt.subplots(
        figsize=(8 + int(len(column_labels) / 2.5), 3 + 3 * int(add_Gene_Name))
    )
    ...
    if (
        ...
        normal_data = np.average(
            df[list(dict_type_sample["normal"])]
            .loc[list_valid_gene]
            .iloc[:, 1:]
            .values,
            axis=1,
        ...
        tumor_data = np.average(
            df[list(dict_type_sample["tumor"])].loc[list_valid_gene].iloc[:, 1:].values,
            axis=1,
        )
    else:
        normal_data = np.sum(
    ...
    # draw heatmap
    im, cbar = heatmap(
        np.vstack((normal_data, tumor_data)),
        labels,
        column_labels,
        ax=ax,
        cmap=cmap,
    ...
    if len(list_valid_gene) < 15:
        texts = annotate_heatmap(
            im, valfmt="{x:.2f}", threshold_bothends=threshold_bothends
        )
    else:
        texts = annotate_heatmap(
            im, valfmt="{x:.1f}", threshold_bothends=threshold_bothends
        )
    fig.tight_layout()
    # save fiqure
    fig.savefig(
...
def PLOT_box_plot__normal_vs_tumor__Gene(
    Gene,
    datatype="std",
    displaying=True,
    p_val_thres=1e-8,
    df=None,
    name_of_df="protein level",
    graph_folder=None,
):
    """
    ...
    title = (
        dataset_name
        + "\n"
        + dict_ID_Symbol[Gene_ID][1]
        + " ( "
        + dict_ID_Symbol_simple[Gene_ID]
        + " )\n "
        + name_of_df
        + " ("
        + datatype
...
def PLOT_whisker_plot__tumor_grades__Gene(Gene, df=None):
    ...
    title = (
        ...
        + " "
        + dict_ID_Symbol_simple[Gene_ID]
        + " protein level with tumor progression"
    )
    # define categories
    ...
    source_normal = ColumnDataSource(
        data=dict(data=normal_data, label=["normal" for i in range(len(normal_data))])
    )
    source_G1 = ColumnDataSource(
        data=dict(data=G1_data, label=["G1" for i in range(len(G1_data))])
    )
    source_G2 = ColumnDataSource(
        data=dict(data=G2_data, label=["G2" for i in range(len(G2_data))])
    )
    source_G3 = ColumnDataSource(
        data=dict(data=G3_data, label=["G3" for i in range(len(G3_data))])
    )
    source_G4 = ColumnDataSource(
        data=dict(data=G4_data, label=["G4" for i in range(len(G4_data))])
    )
    # Create a Bokeh plot
    output_notebook()
    ...
    p.circle(
        y="data",
        x=jitter("label", width=0.4, range=p.x_range),
    ...
    source_error = ColumnDataSource(
        data=dict(base=categories, lower=lower, upper=upper)
    )
...
def PLOT_whisker_plot__tumor_stages__Gene(Gene, df=None):
    ...
    title = (
        ...
        + " "
        + dict_ID_Symbol_simple[Gene_ID]
        + " protein level with tumor invasions (tumor stages)"
    )
    # define categories
    ...
    source_T1a = ColumnDataSource(
        data=dict(data=T1a_data, label=["T1a" for i in range(len(T1a_data))])
    )
    source_T1b = ColumnDataSource(
        data=dict(data=T1b_data, label=["T1b" for i in range(len(T1b_data))])
    )
    source_T2a = ColumnDataSource(
        data=dict(data=T2a_data, label=["T2a" for i in range(len(T2a_data))])
    )
    source_T2b = ColumnDataSource(
        data=dict(data=T2b_data, label=["T2b" for i in range(len(T2b_data))])
    )
    source_T3 = ColumnDataSource(
        data=dict(data=T3_data, label=["T3" for i in range(len(T3_data))])
    )
    source_T3a = ColumnDataSource(
        data=dict(data=T3a_data, label=["T3a" for i in range(len(T3a_data))])
    )
    source_T3b = ColumnDataSource(
        data=dict(data=T3b_data, label=["T3b" for i in range(len(T3b_data))])
    )
    source_T4 = ColumnDataSource(
        data=dict(data=T4_data, label=["T4" for i in range(len(T4_data))])
    )
    # Create a Bokeh plot
    output_notebook()
    p = figure(plot_width=650, x_range=categories, title=title)
...
def Matplotlib_change_box_plot_color(ax, edge_color, fill_color=None, alpha=None):
    ...
    if fill_color == "same":
        ...
        for patch in ax[
            "boxes"
        ]:  # if fill_color has not been given, remove the color of box
...
def PLOT_PCA_plot(df, annotate_labels=None):
    ...
    if annotate_labels is not None:
        x_step, y_step = (x_coord.max() - x_coord.min()) / 100, (
            y_coord.max() - y_coord.min()
        ) / 100  # define x and y step for annotation
...
def PLOT_Embedding_2D(
    df,
    arr_2D_embedding=None,
    n_features_pca=100,
    method_embedding="UMAP",
    Kmean_clusters=None,
    perplexity=50,
    metric="correlation",
    dbscan_eps_min_samples=(0.5, 7),
    plot_samples_not_clustered=True,
    annotate_labels=None,
    individual_annotation=False,
    figsize=(8, 6),
    **dict_embedding,
):
    """data : ( Genes = index, samples = column )
    ...
    if (
        arr_2D_embedding is None
    ):  # if previously transformed t-SNE data is not given, by using PCA and t-SNE, perform t-SNE transformation
        data = data.T
        reducer_pca = PCA(
            n_components=np.min([n_features_pca, data.shape[0] - 1, data.shape[1] - 1])
        )  # n_component of PCA transformation is minimum between 'n_features_pca', number of samples, or number of features in data
        arr_pca = reducer_pca.fit_transform(data)
        if "sne" in method_embedding.lower():
            reducer = TSNE(
                n_components=2, perplexity=perplexity, **dict_embedding
            )  # perform t-SNE transformation with 2 components
        elif "umap" == method_embedding.lower():
            reducer = umap.UMAP(metric=metric, **dict_embedding)
        arr_2D_embedding = reducer.fit_transform(
            arr_pca
        )  # columns = Genes, indices = Cell_IDs
    fig, ax = plt.subplots(figsize=figsize)
    if (
        Kmean_clusters is not None
    ):  # if parameter for Kmean cluster has been given, use Kmean to cluster TSNE result
        kmeans = KMeans(n_clusters=Kmean_clusters, random_state=0).fit(
            arr_2D_embedding
        )  # Kmean cluster data using Kmean_cluster argument
        ...
        db = DBSCAN(
            eps=dbscan_eps_min_samples[0], min_samples=dbscan_eps_min_samples[1]
        ).fit(arr_2D_embedding)
        n_clusters, cluster_labels = (
            len(set(db.labels_)) - (1 if -1 in db.labels_ else 0),
            db.labels_,
        )
    for index_cluster in range(
        -1 * plot_samples_not_clustered, n_clusters
    ):  # draw markers for samples in each cluster
        x_coord, y_coord = arr_2D_embedding[cluster_labels == index_cluster].T
        ax.scatter(
            x_coord,
            y_coord,
            label="cluster {index_cluster}".format(index_cluster=index_cluster),
            alpha=0.7,
        )  # for each cluster, plot markers with different colors
    if annotate_labels is not None:  # if list of samples for annotation has been given
        ...
        ):  # if individial_annotation is set to True, annotate individual samples with unique labels
            annotate_labels = np.array(annotate_labels)[
                GET_MASK_of_intersection(annotate_labels, samples)
            ]  # retrive valid samples from the given list while preserving the order
            for sample in annotate_labels:
                index = np.where(samples == sample)[0][0]
                x_coord_selected, y_coord_selected = TSNE_transformed[index]
                ax.scatter(
                    x_coord_selected, y_coord_selected, label=sample, linewidths=10
                )  # annotate individual samples with unique label and (possibly) color
        else:  # individial_annotation is set to False, annotate all valid samples in annotate_labels as a whole
            x_coord_selected, y_coord_selected = arr_2D_embedding[
                GET_MASK_of_intersection(samples, annotate_labels)
            ].T
            ax.scatter(
                x_coord_selected,
                y_coord_selected,
    ...
    return (
        arr_2D_embedding,
        cluster_labels,
    )  # return TSNE-Transformed data for further optimization of cluster number
...
def PLOT_3D_tSNE_plot(
    df,
    TSNE_transformed=None,
    Kmean_clusters=None,
    dbscan_eps_min_samples=(0.5, 7),
    TSNE_learning_rate=100,
    plot_samples_not_clustered=True,
    sample_list_for_annotation=None,
    individual_annotation=False,
):
    """data : ( Genes = index, samples = column )
    Draw 3D PCA plot of samples"""
    data = df.values  # retrive a numpy array data
    if (
        TSNE_transformed is None
    ):  # if previously transformed t-SNE data is not given, by using PCA and t-SNE, perform t-SNE transformation
        data = data.T
        PCA_for_tSNE = PCA(
            n_components=np.min([50, len(data)])
        )  # n_component of PCA transformation is minimum between 50 and number of samples in data
        PCA_transformed = PCA_for_tSNE.fit_transform(data)
        for_3D_Plot = TSNE(
            n_components=3, learning_rate=TSNE_learning_rate
        )  # perform t-SNE transformation with 3 components
    ...
    if (
        sample_list_for_annotation is not None
    ):  # if list of samples for annotation has been given
        samples = df.columns.values  # retrive list of samples
        if (
            individual_annotation
        ):  # if individial_annotation is set to True, annotate individual samples with unique labels
            sample_list_for_annotation = np.array(sample_list_for_annotation)[
                GET_MASK_of_intersection(sample_list_for_annotation, samples)
            ]  # retrive valid samples from the given list while preserving the order
            for sample in sample_list_for_annotation:
                index = np.where(samples == sample)[0][0]
                x_coord, y_coord, z_coord = TSNE_transformed[index]
                ax.scatter(
                    x_coord, y_coord, z_coord, label=sample, linewidths=10
                )  # annotate individual samples with unique label and (possibly) color
        else:  # individial_annotation is set to False, annotate all valid samples in sample_list_for_annotation as a whole
            x_coord, y_coord, z_coord = TSNE_transformed[
                GET_MASK_of_intersection(samples, sample_list_for_annotation)
            ].T
            ax.scatter(
                x_coord,
                y_coord,
...
def PLOT_DimRed_Gene(
    ...
    color_map="viridis",
    figsize=(8, 6),
    geneset_name="Collection of Genes",
    **dict_circle_or_plot,
):
    ...
    s_data, name_gene = (
        (PD_Locate_gene(df, gene), gene)
        if isinstance(gene, (str, int, float))
        else (PD_Subset(df, gene).mean(axis=0), geneset_name)
    )  # retrive a data of a gene or avarage of a given list of genes
    data = np.vstack(
        [
            arr_2d.T.astype(object),
            COLORMAP_GET_list_of_colors_from_list_of_values(
                s_data.values, return_RGB_hex_str=True, color_map=color_map
            ).astype(object),
        ]
    )
    fig, ax = plt.subplots(1, figsize=figsize)
    flag_plot_circle = dict_circle_or_plot.pop(
        "circle", False
    )  # if circle = True argument was given, set 'flag_plot_circle' to True
    if flag_plot_circle:
        ...
        ax.scatter(
            data[0], data[1], linewidth=linewidth, color=data[2], **dict_circle_or_plot
        )
    x_lim, y_lim = (
        np.vstack([np.min(arr_2d, axis=0), np.max(arr_2d, axis=0)]).T * 1.1
    )  # retrive x and y limits of the plot
    MATPLOTLIB_basic_configuration(
        x_lim=tuple(x_lim), y_lim=tuple(y_lim), title=name_gene
    )
...
def PLOT_NORMALIZE_boxplots_organelle_amount_along_with_Gene_data(
    df,
    Organelle_Gene_Set,
    Gene=None,
    ...
    df_normalized, series_organelle_amount = NORMALIZE_Relative_Log_Expression(
        ...
        scale_all_genes=True,
    )  # retrive normalizaed data and estimated mitochondria amount from RLE
    df = PANDAS_Subset(df, Organelle_Gene_Set).dropna()  # set a dataframe to display
    num_Organelle_Genes = len(
        df
    ...
    list_tumor_samples_sorted = list(
        sample for sample in list_selected_sorted_samples if sample in T_samples
    )
    list_normal_samples_sorted = list(
        sample for sample in list_selected_sorted_samples if sample in N_samples
    )  # retrive sorted list of tumor samples and normal samples
    list_ref_samples_sorted = list(
        sample for sample in list_selected_sorted_samples if sample in Ref_samples
    )  # retrive sorted list of tumor samples and normal samples
    if (
        plot_normalized
    ):  # if 'plot_normalized' is set to True, use normalized organelle data to plot the data
        df_all_input_genes = (
            df_normalized  # set a dataframe that will be used for plotting
        )
        df = PANDAS_Subset(
            df_normalized, Organelle_Gene_Set
        )  # subset 'Organelle_Gene_Set' fron normalized data
    tumor_data = df[
        list_tumor_samples_sorted
    ].values  # retrive data that will be used for boxplots
    ...
    if Gene is not None:  # set colors for plots if Gene is given
        (
            ...
            T_bp_color,
            T_gene_color,
        ) = (None, "chartreuse", "g", "cyan", "b", "darkorange", "r")
    ...
    tumor_pos = series_organelle_amount[
        list_tumor_samples_sorted
    ].values  # retrive the positions of boxplots of tumor, normals, and ref
    ...
    bp_normal = ax1.boxplot(
        ...
        showfliers=False,
        patch_artist=True,
        medianprops=dict(alpha=alpha, color=N_bp_color),
    )
    ...
    if (
        Gene is not None
    ):  # if Gene is given, retrive data of gene and plot the dataalong with boxplots representing organelles
        if (
            type(Gene) is pd.Series
        ):  # if external data was given through 'Gene' argument, use them to plot a data
            ...
            if data_gene.mean() > 1:
                ...
                type(data_gene.values[0]) is np.bool_
                or type(data_gene.values[0]) is bool
            ):
                data_gene = (
                    data_gene * series_organelle_amount - 0.05
                ) * 1.05  # if data_type of external data is boolean, hide data points where data = 0 by subtracting 0.1
        else:
        ...
        ax1.plot(
            ...
            "o",
            color=T_gene_color,
            label="(Tumor) " + Gene_Symbol + is_proteome_data * " Amount",
        )
        ax1.plot(
            normal_pos,
            normal_data_gene,
            "o",
            color=N_gene_color,
            label="(Normal) " + Gene_Symbol + is_proteome_data * " Amount",
        )
        if (
            show_ref
        ):  # show scatter plots of data a given gene of reference samples if 'show_ref' is set to True
            ref_data_gene = data_gene[list_ref_samples_sorted].values
            ax1.plot(
                ref_pos,
                ref_data_gene,
                "o",
                color=ref_gene_color,
                label="(Ref) " + Gene_Symbol + is_proteome_data * " Amount",
            )
    organelle_min_amount, organelle_max_amount = (
        series_organelle_amount.values.min(),
        series_organelle_amount.values.max(),
    )
    plt.xticks(
        np.arange(0, organelle_max_amount, 0.1),
        np.round(np.arange(0, organelle_max_amount, 0.1), 2),
    )  # set x_ticks
    if is_normalized:
        plt.ylabel(Gene_Symbol)  # set label of gene_data as a y_label
        plt.yticks(
            np.arange(0, 3, 0.2), np.round(np.arange(0, 3, 0.2) * scaleing_factor, 2)
        )  # set y_ticks if gene_data has been normalized (priority to gene_data)
    else:
        plt.ylabel(
            "Distributions of "
            + Gene_Set_description
            + " Proteins"
            + plot_normalized * " (Relative Amount to Pseudo Ref.)"
        )
    plt.xlim(
        left=series_organelle_amount.min() - 0.025,
        right=series_organelle_amount.max() + 0.025,
    )  # set x_range automatically
    plt.ylim(bottom=0)  # set y_limit
    plt.xlabel(
        Gene_Set_description
        + " Amount From Relative Log Expression (RLE) Methods"
        + plot_normalized * " Before Normalization"
    )  # annotate the plot
    plt.title(
        "{dataset_name} Proteomic data\n{description} Proteins Distribution (Total {n_genes} Proteins) {normalized}\n(Red = Tumor, Blue = Normal)".format(
            dataset_name=dataset_name,
            description=Gene_Set_description,
            n_genes=str(num_Organelle_Genes),
            normalized=plot_normalized * " After RLE Normalization",
        )
    )
    plt.legend()
    plt.grid(False)
...
def PLOT_paired_T_vs_N(
    ...
    arr_paired_samples = df_paired_samples.loc[
        list_patients
    ].values  # retrive sample_IDs of each patient
    categories = ["Normal", "Tumor"]  # define categories for plotting
    fig, ax = plt.subplots()
    for (
        paired_samples_of_a_patient
    ) in arr_paired_samples:  # plot tumor and normal data for each patient
        data_patient = series_data[paired_samples_of_a_patient]
        if (
            color_by_change
        ):  # if 'color_by_change' is set to True, set color of plot according to Log2FC of Tumor to Normal
            color = cm.coolwarm(
                int(np.log2(data_patient[1] / data_patient[0]) * 128 + 128)
            )
        else:
            color = "k"
        ax.plot(
            categories, data_patient, ".-", lw=linewidth, color=color, alpha=alpha
        )  # overload alpha in color with 'alpha' argument
...
def CORREL_s_df(
    ...
    df,
    print_message=True,
    correl_method="pearsonr",
    return_n_sample_with_zero_values=True,
):
    """# 2021-04-21 15:43:22
    ...
    if correl_method == "pearsonr":
        df_result = pd.DataFrame(
            list(list(stats.pearsonr(data_s, data)) for data in df.values),
            index=df.index.values,
            columns=["correl_coeffi", "p_value"],
        )
    elif correl_method == "kendalltau":
        df_result = pd.DataFrame(
            list(list(stats.kendalltau(data_s, data)) for data in df.values),
            index=df.index.values,
            columns=["correl_coeffi", "p_value"],
        )
    else:
        df_result = pd.DataFrame(
            list(list(stats.spearmanr(data_s, data)) for data in df.values),
            index=df.index.values,
            columns=["correl_coeffi", "p_value"],
        )
...
def CORREL_gene_df(gene, df, df_target=None, print_message=True):
    ...
    geneid, genesymbol = (
        ...
        else GET_Gene_ID_and_Symbol_from_input_gene(gene)
    )  # if a given label as a gene exist in df, assume df do not cotain genes.
    ...
    s_gene, df_target_T = s_gene.align(
        df_target.T, "inner"
    )  # retrive data of valid samples
    ...
    s = pd.Series(
        dict(
            (geneid, NUMPY_spearman_correl(data_a_gene, data_gene))
            for data_a_gene, geneid in zip(data, geneids)
        )
    )
...
def CORREL_INTERNAL_Retrive_correl_coeff_from_scipy_modules(correl_result):
    ...
    if isinstance(correl_result, (np.float64, float)):
        ...
        correl_result, (stats.stats.SpearmanrResult, stats.stats.KendalltauResult)
    ):
...
def CORREL_df(
    ...
    if (
        axis == 0
    ):  # if columns of the two dataframes become rows and columns of correlation matrix, transpose the two matrix
        df, df_target = df.T, df_target.T
    df, df_target = df.align(
        df_target, join="inner", axis=1
    )  # align two dataframe before correlation analysis
    data_1, data_2 = (
        df.values,
        df_target.values,
    )  # retrive data of Normal ans Tumor samples
    mask_1, mask_2 = np.isnan(data_1), np.isnan(data_2)  # get mask for NaN values
    l_var_1, n_var_1, l_var_2, n_var_2 = (
        df.index.values,
        len(df),
        df_target.index.values,
        len(df_target),
    )
    arr_corr = np.zeros(
        (n_var_1, n_var_2)
    )  # create 11 empty ( var x var ) numpy arrays filled zeros that will store computation results that can measure deviation from normal correlation
    Function_correl = stats.__dict__.get(
        method, NUMPY_spearman_correl
    )  # default correlation method is the default built-in custum function 'NUMPY_spearman_correl'
    if mask_1.sum() > 0 or mask_2.sum() > 0:
        print(
            "NaN values detected in aligned dataframes. Use mask to calculate correlation coefficient"
        )
        for index_var_1, data_var_1, mask_var_1 in zip(
            np.arange(len(data_1)), data_1, mask_1
        ):
            for index_var_2, data_var_2, mask_var_2 in zip(
                np.arange(len(data_2)), data_2, mask_2
            ):
                if flag_symmetric and index_var_2 > index_var_1:
                    continue
                mask_var_1_2 = ~(
                    mask_var_1 | mask_var_2
                )  # retrive mask for valid samples
                arr_corr[index_var_1, index_var_2] = (
                    CORREL_INTERNAL_Retrive_correl_coeff_from_scipy_modules(
                        Function_correl(
                            data_var_1[mask_var_1_2], data_var_2[mask_var_1_2]
                        )
                    )
                    if mask_var_1_2.sum() > thres_minimum_n_samples
                    else np.nan
                )
    else:
    ...
    return (
        PANDAS_relation_dataframe_make_symmetric(
            df_corr, symmetry_relationship="=", diagonal_data=1
        )
        if flag_symmetric
        else df_corr
    )
...
def CORREL_s_with_df(s, df, print_message=True):
    ...
    if print_message:
        print(
            "number of features used for correlation analysis: {}".format(
                len(df.columns)
            )
        )
    data_df, arr_index_df, data_s = df.values, df.index.values, s.values
    s_correl_result = pd.Series(
        dict(
            (index, NUMPY_spearman_correl(data_of_an_index, data_s))
            for data_of_an_index, index in zip(data_df, arr_index_df)
        )
    )
...
def CACULATE_correlation__sample_list__Gene(
    df, sample_list, gene=None, correl_type="pearson", drop_na=True
):
    """perform correlation with a given gene in a given set of samples.    return DataFrame of correlation result with a given gene."""
    if (
        drop_na
    ):  # drop NaN values in df_proteome to increase the accuracy of correlation analysis
        df_selected = df_proteome[
            sample_list
        ].dropna()  # select only given samples in proteome dataset without NaN values
    else:
        df_selected = df_proteome[
            sample_list
        ]  # select only given samples in proteome dataset
    gene_id = Gene_2_Gene_ID(gene)  # retrive gene_id
    if gene_id == -1:  # if gene is invalid, end the method
        return
    target_data = df_selected.loc[
        gene_id
    ]  # retrive data of target gene, or "bait" gene
    correlation = dict(
        correl_coef=[], Gene_Symbol=[], p_value=[], Gene_ID=[], Gene_Name=[]
    )
    if correl_type == "pearson":  # perform correlation analysis
        for Gene_ID in df_selected.index:
            data = df_selected.loc[Gene_ID]
...
def Shrink_by_p_value(p_val, p_value_filter=0.25):
    ...
    if np.isnan(p_val):
        return np.nan
    ratio = np.log(p_value_filter * 1.5) / np.log(
        p_val
    )  # calculate the ratio of the log of p-value filter to p-value
    return (
        0 if ratio > 1 else (1 - ratio)
    )  # return the weight of this entry calcluated from p-value
...
def Export_Log2FC_tumor_vs_normal_Pathview(df, name_of_df=""):
    ...
    normal_data = np.ma.masked_array(
        normal_data, np.isnan(normal_data)
    )  # apply mask on np.nan values of tumor and normal data
    tumor_data = np.ma.masked_array(tumor_data, np.isnan(tumor_data))

    p_values = (
        []
    )  # calculate p_values and adjust Log2FC values according to the p_value
    ...
    for intIndex in range(len(Gene_IDs)):  # for each entry
        ...
        p_value = stats.ttest_ind(
            normal, tumor, nan_policy="omit"
        ).pvalue  # perform t-test (with ignoring np.nan values) and add p_value to the list
        p_values.append(p_value)
        # calculate log2 fold change and multiply weight based on p_value, and add Log2FC to the list
        Log2FC_tumor_by_normal.append(
            np.log2(np.average(tumor) / np.average(normal))
            * p_value_scoring_ratio_large_slope(p_value, p_value_filter=0.001)
        )

    # create dataframe from t-test, log2FC values
    dict_normal_vs_tumor_FC_proteome = dict(
        Gene_ID=Gene_IDs,
        Log2FC_tumor_by_normal=Log2FC_tumor_by_normal,
        p_value=p_values,
    )
    df = pd.DataFrame(dict_normal_vs_tumor_FC_proteome)
    df = df.set_index("Gene_ID")
    df = df.sort_values(
        ["Log2FC_tumor_by_normal"], axis="index"
    ...
    df_to_export = df.drop(
        labels="p_value", axis="columns"
    )  # drop p_value colume for the use in Pathview R package
    df_to_export.to_csv(
        result_folder + "normal_vs_tumor_FC_proteome" + name_of_df + ".csv"
...
def LINREGRESS_make_dataframe_symmetric(
    df,
...
def ELBOW_Find(l, sort_array=True, return_index_of_an_elbow_point=False):
    ...
    if not isinstance(l, (np.ndarray)):
        l = np.array(
            l, dtype=float
        )  # convert data to array with floating point numbers
    LIST_COUNT
    if sort_array:
        l = np.sort(l)  # sort the given data
    width_y = (
        l[-1] - l[0]
    )  # get the difference between max and min values of the given data values (y)
    x = (
        np.arange(len(l)) / len(l) * width_y
    )  # get scaled x-coordinates of the data (scaled floating point indices of the sorted data array) (scaled to the difference between min and max values in the data array)
    #     sine_theta = width_y / ( ( l[ - 1 ] - l[ 0 ] ) ** 2 + ( x[ - 1 ] - x[ 0 ] ) ** 2 ) ** 0.5
    sin_45_degree = 1 / 2**0.5
    arr_rotation = np.array(
        [[sin_45_degree, sin_45_degree], [-sin_45_degree, sin_45_degree]]
    )
    x_rotated, y_rotated = np.matmul(arr_rotation, np.vstack([x, l]))
    index_elbow_point = y_rotated.argmin()

    return (
        index_elbow_point if return_index_of_an_elbow_point else l[index_elbow_point]
    )  # return the index or the value of an elbow point
...
def Parse_Line(str_line, l_type, delimiter="\t", set_str_representing_nan=set(["nan"])):
    ...
    for val, f_for_converting_type in zip(l_val, l_type):
        if (
            len(val) == 0
        ):  # if a value is an empty string, interpret it as a np.nan value
            val_converted = np.nan
        else:
            if f_for_converting_type == str:  # if current dtype is string
                if (
                    val in set_str_representing_nan
                ):  # if current value belongs to a set of string values representing np.nan values, interpret it as a np.nan value
                    val_converted = np.nan
                else:
                    val_converted = val
...
def NUMPY_Calculate_Average(arr):  # 2020-07-30 16:17:33
    ...
    avg = (
        arr_valid.sum() / n_non_nan_values
        if n_non_nan_values > 10
        else (sum(arr_valid) / n_non_nan_values if n_non_nan_values > 0 else np.nan)
    )
...
def LISTNUMPY_List_comprehension_using_numpy(
    arr, f_map_return, dtype, f_map_if=None, f_map_else_return=None, f_if=None
):
    ...
    if (
        f_if is not None
    ):  #  if 'f_if' is not None, filter out entries that makes 'f_if' return False
    ...
    if f_map_if is None:
        ...
        if (
            f_map_else_return is None
        ):  # if f_map_else_return is None, return np.nan if 'f_map_if' returns False for the entry
            for index in np.arange(len(arr)):
                arr_mapped[index] = (
                    f_map_return(arr[index]) if f_map_if(arr[index]) else np.nan
                )
        else:
            for index in np.arange(len(arr)):
                arr_mapped[index] = (
                    f_map_return(arr[index])
                    if f_map_if(arr[index])
                    else f_map_else_return(arr[index])
                )
    return arr_mapped
...
def NUMPY_UTIL_Does_contain_NaN(arr, print_message=True):
    ...
    if n_NaN > 0:
        if print_message:
            print(
                "[NUMPY_UTIL_Does_contain_NaN] The given array contains {} n_NaN values".format(
                    n_NaN
                )
            )
        return True
...
def NUMPY_least_square_linear_regress(x, y):
    ...
    sum_x, sum_y, sum_x2, sum_xy = (
        ...
        np.dot(x, x),
        np.dot(x, y),
    )  # calculate sum of x, y, x^2, xy that will be used to calculate the slope
    slope = (N * sum_xy - sum_x * sum_y) / (N * sum_x2 - sum_x**2)
    return (
        slope,
        (sum_y - slope * sum_x) / N,
    )  # return slope and intercept (intercept calculated using mean of x and y)
...
def NUMPY_theil_sen_regression(data_1, data_2):
    ...
    N = data_1.size
    arr_x, arr_y = np.zeros(
        (2, N, N)
    )  # create arrays to which x and y coordinates are broadcasted (and duplicated N times)
    arr_x[:, :], arr_y[:, :] = data_1, data_2  # broad cast x and y data to N x N matrix
    slope = np.median(
        skip_diag_masking(arr_y.T - data_2) / skip_diag_masking(arr_x.T - data_1)
    )  # calculate median of slopes of all pairs of distinct points (diagonals are removed since they are zeros (slope between identical points) and do not have slopes)
    return slope, np.median(data_2) - slope * np.median(
        data_1
    )  # return slope and an intercept
...
arr_halfs = (
    np.ones(2) / 2
)  # define an object that is required for a method that uses below method
...
def LINREGRESS_OPTIMIZE_calculate_std_of_rejections(slope, x_y_centered):
    ...
    return NUMPY_GET_mean_std(
        np.matmul(
            np.array([[1, slope], [-slope, 1]]) / np.sqrt(1 + slope**2), x_y_centered
        )[1]
    )[
        1
    ]  # create a rotation matrix, perform rotation, select y values, and return std of the values
...
an_intercept_vector = np.array(
    [[0.0], [0.0]]
)  # an empty 2D vector then will store the value of an intercept and become an intercept vector


def LINREGRESS_Projection_Based_Least_Square_Linear_Regression(
    gene_1_data, gene_2_data, gene_1_2
):
    ...
    minus_b_by_2a = -(S_y**2 - S_x**2 + n * (S_x2 - S_y2)) / (
        (n * S_x_y - S_x * S_y) * 2
    )
    sqrt_value = np.sqrt(
        1 + minus_b_by_2a**2
    )  # inside square root, the value is always larger than 1
    list_slopes = [
        minus_b_by_2a - sqrt_value,
        minus_b_by_2a + sqrt_value,
    ]  # calculate the two possible optimal slopes and intercepts
    list_intercepts = [
        (S_y - S_x * list_slopes[0]) / n,
        (S_y - S_x * list_slopes[1]) / n,
    ]
    an_intercept_vector[1] = list_intercepts[0]
    std_rejection_1 = LINREGRESS_OPTIMIZE_calculate_std_of_rejections(
        list_slopes[0], gene_1_2 - an_intercept_vector
    )
    an_intercept_vector[1] = list_intercepts[1]
    std_rejection_2 = LINREGRESS_OPTIMIZE_calculate_std_of_rejections(
        list_slopes[1], gene_1_2 - an_intercept_vector
    )
...
def DEVIATION_FROM_NORMAL_CORRELATION_calculation(
    ...
    T_mask, N_mask = OUTLIERS_GET_mask_for_outliers(
        T_data, n_std_for_outliers=n_std_for_outliers
    ), OUTLIERS_GET_mask_for_outliers(
        N_data, n_std_for_outliers=n_std_for_outliers
    ...
    (
        ...
        (11, n_genes, n_genes)
    )  # create 11 empty ( n_genes x n_genes ) numpy arrays filled zeros that will store computation results that can measure deviation from normal correlation

    for index_gene_1, T_data_gene_1, T_mask_gene_1, N_data_gene_1, N_mask_gene_1 in zip(
        np.arange(n_genes), T_data, T_mask, N_data, N_mask
    ):
        for (
            index_gene_2,
            T_data_gene_2,
            ...
            N_data_gene_2,
            N_mask_gene_2,
        ) in zip(np.arange(n_genes), T_data, T_mask, N_data, N_mask):
            if index_gene_2 > index_gene_1:
                continue
            T_mask_gene_1_2 = ~(
                T_mask_gene_1 | T_mask_gene_2
            )  # retrive a mask for non_outliers in the pair of datavalues of genes
            N_mask_gene_1_2 = ~(N_mask_gene_1 | N_mask_gene_2)
            T_data_gene_1_no_outliers, T_data_gene_2_no_outliers = (
                T_data_gene_1[T_mask_gene_1_2],
                T_data_gene_2[T_mask_gene_1_2],
            )  # retrive non_outliers data
            N_data_gene_1_no_outliers, N_data_gene_2_no_outliers = (
                N_data_gene_1[N_mask_gene_1_2],
                N_data_gene_2[N_mask_gene_1_2],
            )
            T_data_gene_1_2 = np.vstack(
                (T_data_gene_1_no_outliers, T_data_gene_2_no_outliers)
            )  # create an array of vectors of postitions (gene_1 = x axis, gene_2 = y axis)
            N_data_gene_1_2 = np.vstack(
                (N_data_gene_1_no_outliers, N_data_gene_2_no_outliers)
            )
            (
                slope,
                ...
                N_data_gene_1_no_outliers, N_data_gene_2_no_outliers, N_data_gene_1_2
            )  # retrive slope and intercept of the gene_pair
            (
                arr_slopes[index_gene_1, index_gene_2],
                arr_intercepts[index_gene_1, index_gene_2],
            ) = (
                slope,
                intercept,
            )  # put calculated index and slope into the array
            an_intercept_vector[1] = intercept  # define vector of position of intercept
            R = np.array([[1, slope], [-slope, 1]]) / np.sqrt(
                1 + slope**2
            )  # calculation rotation matrix that rotate linear regression line to x-axis
            T_data_gene_1_2_projected = np.matmul(
                R, T_data_gene_1_2 - an_intercept_vector
            )  # subtract intercept_vector and perform projection to the linear regression line using the rotation matrix.
            N_data_gene_1_2_projected = np.matmul(
                R, N_data_gene_1_2 - an_intercept_vector
            )
            (
                arr_T_rej_mean[index_gene_1, index_gene_2],
                arr_T_rej_std[index_gene_1, index_gene_2],
            ) = NUMPY_GET_mean_std(
                T_data_gene_1_2_projected[1]
            )  # Than, calculating mean and standard deviation of rejections of the points
            _, arr_N_rej_std[index_gene_1, index_gene_2] = NUMPY_GET_mean_std(
                N_data_gene_1_2_projected[1]
            )
            _, arr_T_proj_std[index_gene_1, index_gene_2] = NUMPY_GET_mean_std(
                T_data_gene_1_2_projected[0]
            )  # Than, calculating mean and standard deviation of rejections of the points
            _, arr_N_proj_std[index_gene_1, index_gene_2] = NUMPY_GET_mean_std(
                N_data_gene_1_2_projected[0]
            )
            arr_T_dev_corr[index_gene_1, index_gene_2] = NUMPY_spearman_correl(
                T_data_gene_1_2_projected[0], T_data_gene_1_2_projected[1]
            )  # calculate Spearman's rank correlation between projections and rejections of points
            arr_N_dev_corr[index_gene_1, index_gene_2] = NUMPY_spearman_correl(
                N_data_gene_1_2_projected[0], N_data_gene_1_2_projected[1]
            )
            arr_T_corr[index_gene_1, index_gene_2] = NUMPY_spearman_correl(
                T_data_gene_1_no_outliers, T_data_gene_2_no_outliers
            )  # calculate Spearman's rank correlation before projection
            arr_N_corr[index_gene_1, index_gene_2] = NUMPY_spearman_correl(
                N_data_gene_1_no_outliers, N_data_gene_2_no_outliers
            )
    ...
    (
        ...
        pd.DataFrame(arr_T_corr, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_N_corr, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_slopes, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_intercepts, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_T_rej_mean, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_T_rej_std, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_T_proj_std, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_T_dev_corr, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_N_rej_std, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_N_proj_std, index=Gene_IDs, columns=Gene_IDs),
        pd.DataFrame(arr_N_dev_corr, index=Gene_IDs, columns=Gene_IDs),
    )  # convert numpy arrays into DataFrames
    df_slopes = PANDAS_relation_dataframe_make_symmetric(
        df_slopes, symmetry_relationship="1/"
    )
    df_intercepts = (
        df_intercepts + (-df_intercepts / df_slopes).T
    )  # calculate half of intercept data from the other half with slope data
    df_T_rej_mean = PANDAS_relation_dataframe_make_symmetric(
        df_T_rej_mean, symmetry_relationship="-"
    )
    df_T_rej_std, df_N_rej_std = PANDAS_relation_dataframe_make_symmetric(
        df_T_rej_std, symmetry_relationship="="
    ), PANDAS_relation_dataframe_make_symmetric(df_N_rej_std, symmetry_relationship="=")
    df_T_proj_std, df_N_proj_std = PANDAS_relation_dataframe_make_symmetric(
        df_T_proj_std, symmetry_relationship="="
    ), PANDAS_relation_dataframe_make_symmetric(
        df_N_proj_std, symmetry_relationship="="
    )
    df_T_dev_corr, df_N_dev_corr = PANDAS_relation_dataframe_make_symmetric(
        df_T_dev_corr, symmetry_relationship="-", diagonal_data=0
    ), PANDAS_relation_dataframe_make_symmetric(
        df_N_dev_corr, symmetry_relationship="-", diagonal_data=0
    )
    df_T_corr, df_N_corr = PANDAS_relation_dataframe_make_symmetric(
        df_T_corr, symmetry_relationship="=", diagonal_data=1
    ), PANDAS_relation_dataframe_make_symmetric(
        df_N_corr, symmetry_relationship="=", diagonal_data=1
    )
    df_dev_origin = df_intercepts / np.sqrt(
        df_slopes.values**2 + 1
    )  # calculate deviation from origin (sign is same with intercept) from df_intercepts and df_slopes. deviation from origin = i / sqrt( s**2 + 1 )
    return dict(
        df_T_corr=df_T_corr,
        df_N_corr=df_N_corr,
        df_slopes=df_slopes,
        df_intercepts=df_intercepts,
        df_T_rej_mean=df_T_rej_mean,
        df_T_rej_std=df_T_rej_std,
        df_T_proj_std=df_T_proj_std,
        df_T_dev_corr=df_T_dev_corr,
        df_N_rej_std=df_N_rej_std,
        df_N_proj_std=df_N_proj_std,
...
def DEVIATION_NORMAL_CORR_a_gene_pair_with_switching(
    ...
    df = pd.DataFrame(
        [s_gene_pair, s_gene_pair_switched], index=["Initial", "Switched"]
    ).T
    ...
    if (
        mask_zero_with_nan
    ):  # if 'mask_zero_with_nan' is True, convert near zero values to np.nan
...
def DEVIATION_NORMAL_CORR_a_gene_pair(gene_1, gene_2, df=None, n_std_for_outliers=3.5):
    ...
    gene_id_1, gene_id_2 = Gene_2_Gene_ID(gene_1), Gene_2_Gene_ID(gene_2)
    T_data_gene_1, T_data_gene_2 = GET_non_NaN_without_outliers(
        df[T_samples].loc[gene_id_1],
        df[T_samples].loc[gene_id_2],
        n_std_for_outliers=n_std_for_outliers,
    )
    N_data_gene_1, N_data_gene_2 = GET_non_NaN_without_outliers(
        df[N_samples].loc[gene_id_1],
        df[N_samples].loc[gene_id_2],
        n_std_for_outliers=n_std_for_outliers,
    )
    T_corr = NUMPY_spearman_correl(
        T_data_gene_1, T_data_gene_2
    )  # calculate Spearman's rank correlation before projection
    ...
    slope, intercept = LINREGRESS_Projection_Based_Least_Square_Linear_Regression(
        N_data_gene_1, N_data_gene_2, N_data_gene_1_2
    )
    an_intercept_vector[1] = intercept  # define vector of position of intercept
    R = np.array([[1, slope], [-slope, 1]]) / np.sqrt(
        1 + slope**2
    ...
    T_rej_mean, T_rej_std = NUMPY_GET_mean_std(
        T_data_gene_1_2_projected[1]
    )  # Than, calculating mean and standard deviation of rejections of the points
    N_rej_mean, N_rej_std = NUMPY_GET_mean_std(N_data_gene_1_2_projected[1])
    T_proj_mean, T_proj_std = NUMPY_GET_mean_std(
        T_data_gene_1_2_projected[0]
    )  # Than, calculating mean and standard deviation of rejections of the points
    N_proj_mean, N_proj_std = NUMPY_GET_mean_std(N_data_gene_1_2_projected[0])
    T_dev_corr = NUMPY_spearman_correl(
        T_data_gene_1_2_projected[0], T_data_gene_1_2_projected[1]
    )  # calculate Spearman's rank correlation between projections and rejections of points
    N_dev_corr = NUMPY_spearman_correl(
        N_data_gene_1_2_projected[0], N_data_gene_1_2_projected[1]
    )
    T_data_gene_1_2_projected[0].sort(), N_data_gene_1_2_projected[
        0
    ].sort()  # sort the projection data in-place to retrive max and min of projection (this is one of the fastest way)
    T_proj_min, T_proj_max = (
        T_data_gene_1_2_projected[0, 0],
        T_data_gene_1_2_projected[0, -1],
    )
    N_proj_min, N_proj_max = (
        N_data_gene_1_2_projected[0, 0],
        N_data_gene_1_2_projected[0, -1],
    )
    ...
    return pd.Series(
        dict(
            slope=slope,
            intercept=intercept,
            T_rej_mean=T_rej_mean,
            T_rej_std=T_rej_std,
            N_rej_mean=N_rej_mean,
            N_rej_std=N_rej_std,
            T_proj_mean=T_proj_mean,
            T_proj_std=T_proj_std,
            N_proj_mean=N_proj_mean,
            N_proj_std=N_proj_std,
            T_dev_corr=T_dev_corr,
            N_dev_corr=N_dev_corr,
            T_corr=T_corr,
            N_corr=N_corr,
            T_proj_min=T_proj_min,
            T_proj_max=T_proj_max,
            N_proj_min=N_proj_min,
            N_proj_max=N_proj_max,
            origin_rej=origin_rej,
...
def LINREGRESS_OPTIMIZE_search_slope_with_minumun_std_of_rejections(
    ...
    stds = np.zeros(
        num_probing + 2
    )  # create an numpy array that will store search results (outputs of a function whose minimum will be searched)
    stds[0], stds[-1] = LINREGRESS_OPTIMIZE_calculate_std_of_rejections(
        lower_slope, x_y_centered
    ), LINREGRESS_OPTIMIZE_calculate_std_of_rejections(
        upper_slope, x_y_centered
    )  # store std of slopes of lower and upper boundaries of the search space
    for index_repeat in range(
        num_repeat
    ):  # repeat search with more dense search points in smaller sub-search space
        step = (upper_slope - lower_slope) / (
            num_probing + 1
        )  # calculate step between search points
        slopes = np.arange(
            lower_slope, upper_slope + 1e-5, step
        )  # build a list of search points
        for index_probing in range(
            1, num_probing + 1
        ):  # calculate output of the function (std of rejections at a given slope) for every search point (a slope in the search space)
            stds[index_probing] = LINREGRESS_OPTIMIZE_calculate_std_of_rejections(
                slopes[index_probing], x_y_centered
            )
        two_lowest_stds = stds.argsort()[
            :2
        ]  # retrive indices of two lowest function outputs (std of rejections), assuming the two indices are of adjacent points and there is only one local minimum)
        slopes = slopes[
            two_lowest_stds
        ]  # retrive slopes of two lowest std of rejections
        stds[0], stds[-1] = stds[two_lowest_stds][
            slopes.argsort()
        ]  # retrive std of rejections of lower_slope and upper_slope
        lower_slope, upper_slope = sorted(
            slopes
        )  # retrive lower and upper slope for setting search points for the next round of search
    return (
        lower_slope + upper_slope
    ) / 2  # once all repeats of search ended, return the average of slopes with two lowest function outputs


# In[ ]:


def LINREGRESS_calculate_std_rejections_and_projections(
    data_1, data_2, slope, intercept
):
    data_1_2 = np.vstack((data_1, data_2))
    intercept_vector = np.array(
        [[0], [intercept]]
    )  # define vector of position of intercept
    R = np.array([[1, slope], [-slope, 1]]) / np.sqrt(
        1 + slope**2
    )  # calculation rotation matrix that rotate linear regression line to x-axis
    data_1_2_projected = np.matmul(
        R, data_1_2 - intercept_vector
    )  # subtract intercept_vector and perform projection to the linear regression line using the rotation matrix.
...
def LINREGRESS_plot_slope_intercept(
    slope,
    intercept,
    data_1=None,
    data_2=None,
...
def TCGA_PLOT_KaplanMeierCurve(
    data_1,
    data_2,
    label_1="data_1",
    label_2="data_2",
    title="",
    data_label="Days to Death (Days)",
    alpha=0.05,
    value_invalid=5000,
    graph_folder=None,
    save_fig=False,
    ...
    data_valid_entries = data_all[
        data_all != value_invalid
    ]  # retrive data of only valid entries (that is not 9999)
    x_lim = data_valid_entries.max() * 1.05  # set x_limit for the plot
    (
        p_value,
        data_1_mean,
        data_2_mean,
        data_2_mean_by_data_1_mean,
    ) = TCGA_CALCULATE_KaplanMeierCurve_log_rank_p_value(
        data_1, data_2, alpha=alpha
    )  # calculate p_value and ratio between data_2_mean and data_1_mean
    fig, ax = plt.subplots(1, 1)
    ax = KaplanMeierFitter().fit(data_1, label=label_1).plot(ax=ax)
    ax = KaplanMeierFitter().fit(data_2, label=label_2).plot(ax=ax)
    title = "{dataset_name} Kaplan-Meier Curves\n{title}\nLog Rank p-value= {p_value}".format(
        dataset_name=dataset_name, title=title, p_value="%.2e" % p_value
    )
    ax.set_title(title)
    ...
    if save_fig:
        plt.savefig(
            graph_folder + to_window_path_compatible_str(title) + ".png", dpi=200
        )
        if not show_fig:
            plt.close()
    else:
...
def TCGA_CALCULATE_KaplanMeierCurve_log_rank_p_value_of_all_genes_in_dataframe(
    ...
    data_1_name, data_2_name = "lower_{}_percent".format(
        str_percent
    ), "upper_{}_percent".format(
        str_percent
    )  # set data_1_name and data_2_name according to 'cut_ratio'
    (
        Patient_IDs,
        _,
    ) = TCGA_SAMPLE_GET_List_Patients_IDs__List_Sample_Types__from__List_Sample_IDs(
        df.columns.values
    )  # retrive Patient_IDs from column names of a given df
    Gene_IDs, data = (
        df.index.values,
        df.values,
    )  # prepare list of Gene_IDs and data for iterations
    n_patients = len(Patient_IDs)  # retruve the number of patients
    dict_Gene_ID__analysis_result = (
        dict()
    )  # an empty dictionary that will store analysis result
    for Gene_ID, data_of_a_gene in zip(Gene_IDs, data):
        data_of_a_gene__sorted = np.sort(
            data_of_a_gene
        )  # sort the data to retrive threshold values for lower n % and upper n %
        ...
        survival_data__low = df_clinical.loc[
            Patient_IDs[data_of_a_gene < thres_the_lower_portion]
        ].days_to_death.values  # retrive survival data of two groups of patients belonging to lower n_% and upper n_% of a given data of a gene
        survival_data__up = df_clinical.loc[
            Patient_IDs[data_of_a_gene > thres_the_upper_portion]
        ].days_to_death.values
        (
            p_value,
            data_1_mean,
            data_2_mean,
            ...
            survival_data__low, survival_data__up
        )  # calculate log_rank p_value
        dict_Gene_ID__analysis_result[Gene_ID] = dict(
            p_value=p_value,
            data_1_mean=data_1_mean,
            data_2_mean=data_2_mean,
            data_2_mean_by_data_1_mean=data_2_mean_by_data_1_mean,
        )
    result = pd.DataFrame(dict_Gene_ID__analysis_result).T.sort_values(
        "p_value"
    )  # convert dictionary result into DataFrame and sort the result with p_values
    result = result.rename(
        columns=dict(
            data_1_mean=data_1_name + "__mean",
            data_2_mean=data_2_name + "__mean",
            data_2_mean_by_data_1_mean="Ratio of " + data_2_name + " to " + data_1_name,
        )
    )  # rename the column names of the result dataframe
...
def TCGA_PLOT_KaplanMeierCurve_for_a_gene(
    Gene,
    df=None,
    df_clinical=None,
    cut_ratio=0.25,
    ...
    df = df[T_samples]  # select only Tumor samples
    str_percent = str(int(round(cut_ratio * 100, 0)))
    data_1_name, data_2_name = "low_{}_percent".format(
        str_percent
    ), "up_{}_percent".format(
        str_percent
    )  # set data_1_name and data_2_name according to 'cut_ratio'
    ...
    return TCGA_PLOT_KaplanMeierCurve(
        survival_data__low,
        survival_data__up,
        label_1=data_1_name,
        label_2=data_2_name,
        ...
        save_fig=save_fig,
        show_fig=show_fig,
    )  # plotted a Kaplan Meier Curve, and return fig and ax if save_fig is False
...
def Broad_CCLE_Cell_Line_Mapper(l_cl):
    ...
    for cl in l_cl:  # map given list of cell lines to CCLE Cell line ids
        if (
            cl not in df_ccle_meta_cl.index.values
        ):  # for cell line names that is not CCLE cell line ID
            l_cl_search = CCLE_Search_Cell_Line(cl + "_")
            if len(l_cl_search) > 1:
                print(
                    "[Cell Line Error] More than one cell line was searched :",
                    l_cl_search,
    ...
    return list(
        cl if cl in df_ccle_meta_cl.index.values else CCLE_Search_Cell_Line(cl + "_")[0]
        for cl in l_cl
    )
...
def Broad_CCLE_Preprocessing_map_unmapped_cell_lines(l_cl, print_result=True):
    ...
    l_n_matched = list(
        np.array(
            list(True if cl_name == cl.split("_")[0] else False for cl in l_all_cl)
        ).sum()
        for cl_name in l_unmapped_cl_names
    )  # number of ccle_cl that have exactly the same
    df_matching_result = pd.DataFrame(
        dict(
            Unmapped_ID=l_unmapped_cl,
            Cell_Line=l_unmapped_cl_names,
            n_matched=l_n_matched,
        )
    ).set_index("Unmapped_ID")
    if print_result:
        display(df_matching_result)
    df_matching_result = df_matching_result[
        df_matching_result.n_matched == 1
    ]  # filter out invalid cell_line ids
    return dict(
        (cl_id_unmaped, list(cl for cl in l_all_cl if cl_name == cl.split("_")[0])[0])
        for cl_name, cl_id_unmaped in zip(
            df_matching_result.Cell_Line.values, df_matching_result.index.values
        )
    )
...
def CCLE_EXPORATORY_Add_snucellbank_availability(df, inplace=False):
    ...
    df["Available_in_SNU_Cell_Bank"] = list(
        True if cl in l_snu_cl else False for cl in df.index.values
    )
    return df
...
def CCLE_EXPLORATORY_CL_Genes_Mutation_Table(l_genes=None, l_cl=None):
    ...
    l_cl = (
        df_mut.Tumor_Sample_Barcode.values
        if l_cl is None
        else Broad_CCLE_Cell_Line_Mapper(l_cl)
    )
    l_genes = (
        df_gdsc_cn.index.values if l_genes is None else l_genes
    )  # default genes are genes available in the copy number data
    l_geneids = List_Gene__2__List_Gene_ID(l_genes)
    dict_res = dict()
    for geneid in l_geneids:
        df_search_res = (
            CCLE_MUTATION_Retrive_a_gene_of_cell_lines(geneid, return_compact_df=False)
            .sort_values("isTCGAhotspot", ascending=False)
            .drop_duplicates(subset=["Tumor_Sample_Barcode"])
            .set_index("Tumor_Sample_Barcode")
            .astype(object)
        )  # prioritize mutation data using TCGA_hotspot_mutation_status when more then protein_change data values are available
        arr_mut, arr_mask_hotspot, indices = (
            df_search_res.Genome_Change.values,
    ...
    for cl in set(l_cl).difference(
        df.columns.values
    ):  # fill cell_lines with no mutations with 'WT'
        df[cl] = "WT"
    df = df.T
    df_subset = PD_Subset(
        df, index=df_gdsc_cn.columns.values, columns=df_gdsc_cn.index.values
    )  # add df_gdsc_cn copy number data
    if df_subset.shape[1] != df.shape[1]:
        print(
            "[MAPPING ERROR] Following genes not present in 'df_gdsc_cn' dataset",
            List_Gene_ID__2__List_Gene_Symbol(
                set(df.columns.values).difference(df_gdsc_cn.index.values)
            ),
        )
    df_subset.values[
        (
            PD_Subset(
                df_gdsc_cn,
                index=l_geneids,
                columns=df_subset.index.values,
                preserve_order_in_df=False,
            )
            == 0
        ).T
        & (df_subset == "WT")
    ] = "DEL"  # update copy number data (Deletion, Disruption, and Loss of Heterozygousy)
    df_subset.values[
        PD_Subset(
            df_gdsc_cn_d,
            index=l_geneids,
    ...
    df = df_subset.append(
        df.loc[list(set(df.index.values).difference(df_subset.index.values))].replace(
            "WT", "WT?"
        )
    )  # add back cell lines do not exist in df_gdsc_cn (they only have point mutation data) to the final processed result dataframe
...
def CCLE_two_gene_plot(
    df, gene1, gene2=None, df_2=None, external_data=None, plot_kw=dict()
):
    if external_data is not None:
        ...
        MPL_Scatter_Align_Two_Series(
            data_1,
            external_data,
            n_std_for_outliers=0,
            annotate_xy_axis=(True, True),
            **plot_kw,
        )
    else:
        data_1 = PD_Locate_gene(df, gene1)
        data_2 = (
            PD_Locate_gene(df, gene2) if df_2 is None else PD_Locate_gene(df_2, gene2)
        )
        MPL_Scatter_Align_Two_Series(
            data_1,
            data_2,
...
def CCLE_MUTATION_Retrive_a_gene_of_cell_lines(gene, l_cl=None, return_compact_df=True):
    df = (
        PANDAS_Select(df_mut, Entrez_Gene_Id=Gene_2_Gene_ID(gene))
        if l_cl is None
        else PANDAS_Select(
            df_mut, Entrez_Gene_Id=Gene_2_Gene_ID(gene), Tumor_Sample_Barcode=l_cl
        )
    )  # if valid list of cell_lines were given, subset df with given list of cell lines
    if return_compact_df:
        df = df[
            [
                "Hugo_Symbol",
...
def CCLE_METABOLOMICS_Search_Metabolite(query):
    query = query.lower()
    return sorted(
        list(metabol for metabol in df_metabol.index.values if query in metabol.lower())
    )
...
def CCLE_Search_Datasets_A_Cell_Line(cell_line_name):
    return list(
        ...
        for df, data_name in zip(
            [df_rna, df_rnai_gd, df_crispr_gd, df_metabol],
            ["df_rna", "df_rnai_gd", "df_crispr_gd", "df_metabol"],
        )
        if cell_line_name in df.columns.values
    )
...
def CCLE_Gene_Dependency_Score_Plot_Gene(
    Gene,
    data_type="crispr",
    save_fig=False,
    ...
    if (
        Gene_ID == -1 or Gene_ID not in df.index.values
    ):  # if gene is invalid or not exist in the data, return an error value
        return -1
    df = pd.DataFrame(
        dict(Gene_Dependency=df.loc[Gene_ID].dropna(), RNA=df_rna.loc[geneid_for_rna])
    ).dropna()
    ...
    s_cell_line_set_median = pd.Series(
        dict(
            (
                cell_line_set_name,
                np.median(PANDAS_Subset(df, cell_line_set).Gene_Dependency),
            )
            for cell_line_set_name, cell_line_set in dict_cell_line_set.items()
            if len(PANDAS_Subset(df, cell_line_set)) > thres_n_cell_line
        )
    )  # retrive median viability score for each cell_line_set. exclude cell_line_set if a subset contain less than 'thres_n_cell_line' entries
    x_position = 0
    for cell_line_set_name in (
        s_cell_line_set_median.dropna().sort_values().index.values
    ):
        cell_line_set = dict_cell_line_set[cell_line_set_name]
        df_subset = PANDAS_Subset(df, cell_line_set).sort_values("Gene_Dependency")
        ax.scatter(
            x=np.arange(len(df_subset)) + x_position,
            y=df_subset.Gene_Dependency,
            s=df_subset.RNA.values * size_factor,
            label=cell_line_set_name,
            alpha=0.5,
        )
    ...
    data_description, data_method = (
        ("CRISPR KO Viability (CERES Data)", "Knock Out")
        if data_type == "crispr"
        else ("RNAi KD Viability (DEMETER2 Data)", "Knock Down")
    )  # set data_description and data_abbreviation according to 'data_type'
    MATPLOTLIB_basic_configuration(
        x_label="Ranked Cell Lines",
        y_label="Viability Score After {}".format(data_method),
        title="'{}' {}\nCell Lines By Primary Site".format(
            Gene_Symbol, data_description
        ),
        save_fig=save_fig,
        show_legend=True,
    )
...
def CCLE_CTRP_GD_Plot(cpd, gene, save_fig=False):
    gene_id, gene_symbol = GET_Gene_ID_and_Symbol_from_input_gene(gene)
    if (
        "df_ctrp_auc_aligned" not in globals()
    ):  # if aligned data is not available, align the two data
        (
            globals()["df_ctrp_auc_aligned"],
            globals()["df_crispr_gd_aligned"],
        ) = PANDAS_Align_two(df_ctrp_auc, df_crispr_gd)
    ...
    plt.plot(
        df_ctrp_auc_aligned.loc[cpd].values,
        df_crispr_gd_aligned.loc[gene_id].values,
        "o",
        alpha=0.5,
    )
    MATPLOTLIB_basic_configuration(
        y_lim=(-1.5, 0.5),
        x_label="Area Under Curve (AUC) of '{}'".format(cpd),
        y_label="Gene Dependency Data of '{}'".format(gene_symbol),
        title="{} and {}\n(CTRP Drug Response Data, CRISPR KO Gene Dependency Data)".format(
            cpd, gene_symbol
        ),
        save_fig=save_fig,
        save_file_name="{} and {}_(CTRP AUC_CRISPR GD)".format(cpd, gene_symbol),
    )
...
def CCLE_Scatter_RNA_Seq_and_Metabolite(Gene, Metabolite, query=None, log_scale=True):
    ...
    df = pd.DataFrame(
        dict(
            RNA_Seq=df_rna.loc[dict_Symbol_ID_simple[Gene]],
            Metabolomics=df_metabol.loc[Metabolite],
        )
    ).dropna()
    if (
        query == "?"
    ):  # if help is required, print out tissue types and histology subtypes
        print("Tissue Types : ")
        display(
            sorted(
                set(
                    list(
                        index.split("_")[1].capitalize()
                        for index in df_meta_ccle.index.values
                    )
                )
            )
        )
        ...
        display(
            df_meta_ccle.Hist_Subtype1.dropna().drop_duplicates().sort_values().values
        )
        return -1
    if type(query) is str:  # if only tissue is given
        ...
        df_meta_ccle_matched_tissue = PANDAS_Subset_df(
            df_meta_ccle.dropna(subset=["Hist_Subtype1"]),
            CCLE_Search_Cell_Lines_df(df.T, query[0]),
        )
        df_subtype_searched = df_meta_ccle_matched_tissue[
            list(
                True if query[1].lower() in subtype.lower() else False
                for subtype in df_meta_ccle_matched_tissue.Hist_Subtype1.values
            )
        ]
        display(
            list(
                df_subtype_searched.Hist_Subtype1.drop_duplicates().sort_values().values
            )
        )  # display searched histology subtypes
        df = PANDAS_Subset_df(df, df_subtype_searched.index.values)
    correl_p, p_p = stats.pearsonr(
        df.RNA_Seq.values, df.Metabolomics.values
    )  # calculate Pearson's correlation
    correl_s, p_s = stats.spearmanr(
        df.RNA_Seq.values, df.Metabolomics.values
    ...
    MATPLOTLIB_basic_configuration(
        x_scale=scale,
        y_scale=scale,
        x_label="RNA-Seq ({})".format(Gene),
        y_label="Metabolite Amount ({})".format(Metabolite),
        show_legend=False,
        title="RNA-Seq and Metabolomic Data",
        save_fig=False,
    )
    print(
        "\n(Correl. Pearson = {c_p}, Spearman = {c_s} \n P-value. Pearson = {p_p}, Spearman = {p_s})".format(
            c_p=round(correl_p, 3),
            c_s=round(correl_s, 3),
            p_p=round(p_p, 3),
            p_s=round(p_s, 3),
        )
    )
...
def CCLE_Transform_to_Standard_Score(
    df, l_cl=None, transform_log2=False, transform_z_score=True, transform_gd_data=True
):
...
def IMPC_CRBN_KO__BIOTYPE_add_biotype_to_transcript_dataframe(df):
    ...
    arr_gene_biotype, arr_transcript_biotype = df_transcript_biotype.loc[
        arr_tx_id
    ].values.T
...
def IMPC_CRBN_KO__COMBINE_Log2FC_results(
    df_result_liver,
    df_result_lung,
    df_result_heart,
    col_essential=[
    ...
    (
        col_essential_all,
        col_essential_renamed_liver,
        col_essential_renamed_lung,
        col_essential_renamed_heart,
    ) = (
        list(col + "_all" for col in col_essential),
        list(col + "_liver" for col in col_essential),
        list(col + "_lung" for col in col_essential),
        list(col + "_heart" for col in col_essential),
    )
    df_result_liver, df_result_lung, df_result_heart = (
        df_result_liver[col_essential],
        df_result_lung[col_essential],
        df_result_heart[col_essential],
    )
    df_result_liver, df_result_lung, df_result_heart = (
        df_result_liver.rename(
            columns=dict((col, col + "_liver") for col in col_essential)
        ),
        df_result_lung.rename(
            columns=dict((col, col + "_lung") for col in col_essential)
        ),
        df_result_heart.rename(
            columns=dict((col, col + "_heart") for col in col_essential)
        ),
    )
    df_result_all = df_result_liver.join(df_result_lung, how="outer").join(
        df_result_heart, how="outer"
    )
    arr_cols = np.array(
        [
            col_essential_renamed_liver,
    ...
    for col_all, cols_tissues in zip(col_essential_all, arr_cols):
        if "p_value" in col_all:
            df_result_all[col_all + "_log_avg"] = np.power(
                df_result_all[cols_tissues].prod(axis=1), 1 / 3
            )  # calculate geometric mean of p_values
            df_result_all[col_all] = df_result_all[cols_tissues].mean(
                axis=1
            )  # calculate linear mean of p_values
            cols_summary.append(col_all + "_log_avg")
        else:
            df_result_all[col_all] = df_result_all[cols_tissues].mean(
                axis=1
            )  # calculate linear average for all columns other than p_values
...
def UBIQUITINATION_DATASET_barplot_a_protein(
    Gene, save_fig=False, df=None, data_use_log2=False, CRBN_only=True
):
    ...
    MATPLOTLIB_basic_configuration(
        x_label="Ubiquitination Signal" + " (Log2)" * data_use_log2,
        title="Ubi. Signal of ( {} )".format(Gene_Symbol)
        + " by CRBN" * CRBN_only
        + "\n(2014 Nature Fischer Dataset)",
        save_fig=save_fig,
    )
...
def UBIQUITINATION_DATASET_barplot_proteins_for_pub(
    Genes,
    background_method="background_divide",
    save_fig=False,
    adjust_aspect=1,
    log_scale=False,
):
    ...
    Genes = LIST_intersection_with_set(
        List_Gene__2__List_Gene_ID(Genes), df_ubi.index.values
    )
    print(len(Genes))
    if (
        background_method == "background_divide"
    ):  # divide df_ubi data with its background data (E1_E2)
    ...
    for Gene, ax in zip(Genes, arr_ax):
        ...
        Gene_ID, Gene_Symbol = Gene_ID_Symbol
        data = df.loc[Gene_ID]
        mean_E1_E2_CRBN, err_E1_E2_CRBN = NUMPY_GET_mean_std(
            data.loc[cols_E1_E2_CRBN].values
        )
        mean_E1_E2_CRBN_Len, err_E1_E2_CRBN_Len = NUMPY_GET_mean_std(
            data.loc[cols_E1_E2_CRBN_Len].values
        )
        ...
        ax.barh(
            labels, data_values, xerr=error_values, capsize=3
        )  # draw bar plots with error values
...
def CRBNPROT_Decompress_metadata(df=None, data_labels=None):
    ...
    )  # set default data_labels
    df = df_crbnprot if df is None else df  # default df is df_crbnprot
    return pd.DataFrame(
        list(col.split("__") for col in df_crbnprot.columns.values),
        index=df_crbnprot.columns.values,
        columns=data_labels,
    )
...
def CRBNPROT_Compress_metadata(df=None, data_labels=None):
    ...
    df = (
        df_crbnprot_meta_assays if df is None else df
    )  # default df is df_crbnprot_meta_assays
...
def CRBNPROT_Locate_gene(gene, add_decompressed_metadata=True):
    ...
    l_labels_log2fc, l_labels_p_values = Search_list_of_strings(
        labels, "log2fc"
    ), Search_list_of_strings(labels, "value")
    df_meta = PD_Subset(
        PD_Subset(df_crbnprot_meta_assays, l_labels_log2fc),
        ["Data_Type", "Experiment_System", "Condition_A", "Condition_B"],
        axis=1,
    )  # subset metadata dataframe
    if add_decompressed_metadata:
        df = pd.DataFrame(
            dict(
                Log2_Fold_Change=s_data.loc[l_labels_log2fc].values,
                p_value=s_data.loc[l_labels_p_values].values,
            ),
            index=l_labels_log2fc,
        ).join(df_meta)
        ...
        df = pd.DataFrame(
            ...
            index=CRBNPROT_Compress_metadata(df_meta),
        )
    df.index.name = "Data for {}".format(
        genesymbol
    )  # set the name of the dataframe as the name of its index
    return df
...
def CRBNPROT_Retrive_list_of_log2fc_and_p_value_cols(df=None):
    ...
    return Search_list_of_strings(cols, "log2fc"), Search_list_of_strings(
        cols, "value"
    )  # and 'Ubi_P' not in col and 'Author' not in col )
...
def ONLINE_TOLLS_Enrichr(df, col="Log2_Fold_Change"):
    ...
    for symbol, weight in zip(
        df.Approved_Symbol.values,
        BOKEH_transform_arr_values_into_a_range_from_0_to_1(df[col].values),
    ):
...
def heatmap(
    ...
    **kwargs,
    ...
    if (
        cbarlabel is not None and show_colorbar
    ):  # label color bar if name has been given
        cbar.ax.set_ylabel(cbarlabel, rotation=-90, va="bottom")
    ax.set_xticks(np.arange(data.shape[1]))  # We want to show all ticks...
    ax.set_yticks(np.arange(data.shape[0]))
    ax.set_xticklabels(
        col_labels, fontsize=fontsize_x
...
def annotate_heatmap(
    ...
    **textkw,
    ...
    data_heatmap = (
        im.get_array()
    )  # retrive data used to draw heatmap from the given image
    if threshold_bothends is None:  # Normalize the threshold to the images color range.
        if threshold is not None:
            threshold = im.norm(threshold)
        else:
            threshold = im.norm(data_heatmap.max()) / 2.0
    else:
        threshold_low = im.norm(data_heatmap.max()) * (1.0 - threshold_bothends)
    ...
    if threshold_bothends is None:
        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                kw.update(
                    color=textcolors[
                        im.norm(data_heatmap[i, j])
                        > threshold_high | im.norm(data_heatmap[i, j])
                        < threshold_low
                    ]
                )
                text = im.axes.text(
...
def ANNDATA_From_DF(df):
    ...
    return sc.AnnData(
        X=df,
        obs=pd.DataFrame(df.index.values).rename(columns={0: "obs"}).set_index("obs"),
        var=pd.DataFrame(df.columns.values).rename(columns={0: "var"}).set_index("var"),
    )
    ...
    for dict_it in l_dict_it:
        for key in dict_it:
            ...
            dict_it_combined[key].update(
                dict_it[key]
            )  # combine interval trees of each key into a single interval tree
    return dict_it_combined


def GTF_Search_Overlap(
    df_gtf1, df_gtf2, col_name_indicating_overlap="flag_overlap", inplace=False
):
    """# 2022-04-16 23:19:32
    ...
    df_gtf1[col_name_indicating_overlap] = list(
        seqname in dict_it_gtf2 and len(dict_it_gtf2[seqname][start - 1 : end]) > 0
        for seqname, start, end in df_gtf1[["seqname", "start", "end"]].values
    )
    return df_gtf1
...
def PARSER_EMBL_Format(path_file):
    ...
    for record in l_record[:-1]:
        ...
        for line in record.split("\n"):
            str_field = (
                "Sequence" if line[:2] == "  " else line[:2]
            )  # if field is empty, assign its field to 'Sequence' field
            str_content = (
                line.split("{}   ".format(line[:2]))[1] if "   " in line else ""
            )
            dict_field_to_content[str_field] = (
                dict_field_to_content[str_field] + " " + str_content
                if str_field in dict_field_to_content
                else str_content
            )
    ...
    if start1 <= start2:
        if end1 >= end2:
            ...
            if end1 < start2:
                ...
                return (
                    overlap_length / (end1 - start1) * 100,
                    overlap_length / (end2 - start2) * 100,
                )
    else:
        if end2 >= end1:
...
def OS_CPU_and_Memory_Usage(
    flag_ignore_processes_using_neglible_amount_of_resources=True,
):
    """# 2022-06-17 13:12:35
...
def OS_PIPELINE_Multiple_Running(
    ...
    path_data=None,
    title="",
    excute_cmd_line=False,
    wait_n_seconds_between_splited_jobs=5,
    split=True,
    l_cmd_line_initialize=["sleep 1", 'echo "Bookshelves PIPELINE Started"', "date"],
    l_server=None,
    server_replace_home_dir=False,
    shellscript_use_relative_path=False,
    dict_id_server_to_path_home={
        "node210": "/node210data/",
        "node01": "/node01data/",
        "node200": "/home/",
    },
    ...
    if len(arr_cmd_line) == 0:
        ...
        return -1
    try:
        arr_cmd_line.dtype  # preprocess the format of arr_cmd_line. 1) make sure arr_cmd_line is np.array datatype, and 2) dtype of the np.array is Object
    except:
        arr_cmd_line = np.array(arr_cmd_line, dtype=str).astype(object)
    str_time_stamp = (
        TIME_GET_timestamp()
    )  # get time stamp of current time to uniquely name the shellscript
    arr_cmd_line = (
        arr_cmd_line.astype(object)
        if arr_cmd_line.dtype is not np.dtype("O")
        else arr_cmd_line
    )  # convert arr_cmd_line to object dtype array
    n_lines_at_a_time = min(
        n_lines_at_a_time, len(arr_cmd_line)
    )  # set n_lines_at_a_time according to the given number of command lines
    if (
        l_server is None
    ):  # if cmd_line is run in only one server # l_server = [ os.environ[ 'SERVER_NAME' ] ] # use current id_server by default
        ...
        for index_shell_script in range(n_lines_at_a_time):
            with open(
                f"{path_data}{name_file}__splitted_{index_shell_script + 1}.sh", "w"
            ) as file:
                arr_cmd_line_for_a_server_splitted = arr_cmd_line_for_a_server[
                    index_shell_script::n_lines_at_a_time
                ]
                n_line_splitted = len(
                    arr_cmd_line_for_a_server_splitted
                )  # add echo command line that report the progress of the given jobs for each splitted file
                arr_percents = np.round(
                    np.arange(0, 100, 100 / n_line_splitted) + 100 / n_line_splitted, 1
                )
                arr_percents = arr_percents[
                    arr_percents <= 100
                ]  # remove occasional percent values above 100%
                arr_cmd_line_for_a_server_splitted = (
                    arr_cmd_line_for_a_server_splitted
                    + '\necho "Bookshelves PIPELINE: '
                    + arr_percents.astype(str).astype(object)
                    + " % completed (line "
                    + np.arange(1, n_line_splitted + 1).astype(str).astype(object)
                    + ' at splitted file {})"\ndate\necho "Completed Command : '.format(
                        index_shell_script + 1
                    )
                    + arr_cmd_line_for_a_server_splitted
                    + '"'
                )  # echo the complete percentage of given jobs and echo the given command line name
                str_shell_script = (
                    "\n".join(l_cmd_line_initialize)
                    + "\n"
                    + "\n".join(arr_cmd_line_for_a_server_splitted)
                )
                if wait_n_seconds_between_splited_jobs:
                    file.write(
                        f"sleep {int( ( index_shell_script + 1 ) * wait_n_seconds_between_splited_jobs )}\n{str_shell_script}"
                    )  # if 'wait_n_seconds_between_splited_jobs' is given, wait n_seconds between initiating each job, so that resources can be evenly distributed and avoid bottle neck.
                else:
                    file.write(
                        str_shell_script
                    )  # add shell command lines in 'l_cmd_line_initialize' at the start of each splitted file.
        if n_lines_at_a_time > 1:
            with open(f"{path_data}{name_file}.sh", "w") as file:
                ...
                file.write(
                    " & ".join(
                        list(
                            f"bash {name_file if shellscript_use_relative_path else path_data + name_file}__splitted_{index_shell_script + 1}.sh"
                            for index_shell_script in range(n_lines_at_a_time)
                        )
                    )
                    + "\nwait"
            ...
            os.rename(
                f"{path_data}{name_file}__splitted_{index_shell_script + 1}.sh",
                path_data + f"{name_file}.sh",
            )
        ...
        for index_server, id_server in enumerate(l_server):  # for each server
            arr_cmd_line_for_a_server = arr_cmd_line[
                index_server::n_servers
            ]  # retrieve cmd_line_for_each_server
            path_home_current_server = dict_id_server_to_path_home[id_server]
            if (
                server_replace_home_dir
            ):  # replace home directory in 'cmd_line' to the current server's home directory
                arr_cmd_line_for_a_server = np.array(
                    list(
                        cmd_line.replace(
                            "/__current_server_home_path__/", path_home_current_server
                        )
                        for cmd_line in arr_cmd_line_for_a_server
                    ),
                    dtype=object,
                )  # modify the absolute path to match the path_folder_data_server for each server
            n_lines = len(arr_cmd_line_for_a_server)
            n_lines_at_a_time_server = (
                n_lines_at_a_time if n_lines >= n_lines_at_a_time else n_lines
            )  # if number of command lines is larger than current 'n_lines_at_a_time', set 'n_lines' as 'n_lines_at_a_time_server' for the current server
            ...
            for index_shell_script in range(n_lines_at_a_time_server):
                with open(
                    ...
                    arr_cmd_line_for_a_server_splitted = arr_cmd_line_for_a_server[
                        index_shell_script::n_lines_at_a_time_server
                    ]
                    n_line_splitted = len(
                        arr_cmd_line_for_a_server_splitted
                    )  # add echo command line that report the progress of the given jobs for each splitted file
                    arr_percents = np.round(
                        np.arange(0, 100, 100 / n_line_splitted)
                        + 100 / n_line_splitted,
                        1,
                    )
                    arr_percents = arr_percents[
            ...
            if n_lines_at_a_time_server > 1:
                with open(
                    path_data + "{name_file}.sh".format(name_file=name_file), "w"
                ) as file:
                    if not shellscript_use_relative_path:
                        file.write(f"cd {path_data}\n")
                    file.write(
                        " & ".join(
                            list(
                                f"bash {name_file if shellscript_use_relative_path else path_data + name_file}__splitted_{index_shell_script + 1}.sh"
                                for index_shell_script in range(
                                    n_lines_at_a_time_server
                                )
                            )
                        )
                ...
                os.rename(
                    f"{path_data}{name_file}__splitted_1.sh",
                    f"{path_data}{name_file}.sh",
                )
...
def OS_PIPELINE_Run_Until_Passed(
    l_popenargs, Func_check_passed, dict_args_Func_check_passed=dict()
):  # 2020-11-12 19:42:32
    ...
    path_file_stdout = "/tmp/{uuid}.out.txt".format(
        uuid=str_id
    )  # define stdout and stdin files
    ...
    while True:
        with open(
            ...
            with open(path_file_stderr, "w+") as ferr:
                ...
                ferr.seek(0)
                stderr = ferr.read()
        if Func_check_passed(stdout, stderr, **dict_args_Func_check_passed):
            break
...
def OS_Memory():
    ...
    for line in l_line:
        ...
        str_name_field = l[0][
            :-1
        ]  # retrieve the name of the field # remove ':' character at the end
...
def OS_Download(url_remote, path_destination, download_file=True):
    ...
    """
    if (
        url_remote[-1] == "/" or path_destination[-1] == "/"
    ):  # automatically set 'download_file' FLAG to False if given 'path_destination' or 'url_remote' ends with '/'
        download_file = False
    ...
    OS_Run(
        l_args + [path_destination, url_remote],
        path_file_stdout=f"{path_destination}.wget.stdout.out",
        path_file_stderr=f"{path_destination}.wget.stderr.out",
    )
...
def TE_eQTL_Swarm_Plot(df_expr, df_genotype, index_entry, index_genotype):
    ...
    df_eqtl_query = pd.DataFrame(
        dict(
            expr=df_expr.loc[index_entry],
            n_alt_alleles=df_genotype.loc[int(index_genotype.rsplit("_", 1)[1])],
        )
    )
    sns.swarmplot(data=df_eqtl_query, x="n_alt_alleles", y="expr")
    MATPLOTLIB_basic_configuration(
        title="eQTL SwamPlot",
        x_label="number of alternative alleles of {}".format(index_genotype),
        y_label="detected expression of {}".format(index_entry),
    )
            ...
            while True:
                ...
                if str_tag != "XX" and len(str_content) > 0:
                    ...
                    if str_tag == "  ":
                        ...
                        str_content = STR.Replace_a_character_every_n_characters(
                            str_content[:65], n_characters=11, replacement_characters=""
                        ).strip()
                    if str_content[-1] == "." or str_content[-1] == ";":
                        str_content = str_content[
                            :-1
                        ]  # if content ends with ';' or '.', remove the characters.
                    if str_tag in dict_record:
                        dict_record[str_tag] = (
                            dict_record[str_tag] + character_for_joining + str_content
                        )  # combine content with same str_tag together
                    else:
                        dict_record[str_tag] = str_content
            if len(line) == 0:
                break
...
def SEQ_Trim_Invalid_Residue(seq, invalid_residues="JOUXZB"):  # 2020-07-14 16:25:34
    ...
    for char_invalid_residue in l_invalid_residues[1:]:
        seq = seq.replace(
            char_invalid_residue, char_representing_all_invalid_residue
        )  # replace all invalid_residues to a character representing all invalid residues
    n_invalid_residue_at_5_prime, n_invalid_residue_at_3_prime = (
        0,
        0,
    )  # initialize number of N bases at either ends
    while (
        n_invalid_residue_at_5_prime < length_seq
        and seq[n_invalid_residue_at_5_prime] == char_representing_all_invalid_residue
    ):
        n_invalid_residue_at_5_prime += 1  # find out where N base ends at either end
    if n_invalid_residue_at_5_prime == length_seq:
        return ""  # if a sequence contains only char_representing_all_invalid_residue base, return an empty fastq record
    while (
        seq[length_seq - n_invalid_residue_at_3_prime - 1]
        == char_representing_all_invalid_residue
    ):
        n_invalid_residue_at_3_prime += 1
    seq_trimmed = seq[
        n_invalid_residue_at_5_prime : length_seq - n_invalid_residue_at_3_prime
    ]  # trim a stretch of N bases at either end
    if (
        char_representing_all_invalid_residue in seq_trimmed
    ):  # if char_representing_all_invalid_residue base exist inside a trimmed sequence, retrieve either 3' non-N base stretch or 5' non-N base stretch depending on the length of the non-N base stretchs.
        n_non_invalid_residue_at_5_prime = seq_trimmed.find(
            char_representing_all_invalid_residue
        )
        n_non_invalid_residue_at_3_prime = seq_trimmed[::-1].find(
            char_representing_all_invalid_residue
        )
        if n_non_invalid_residue_at_5_prime >= n_non_invalid_residue_at_3_prime:
            seq_trimmed = seq_trimmed[:n_non_invalid_residue_at_5_prime]
        else:
            seq_trimmed = seq_trimmed[-n_non_invalid_residue_at_3_prime:]
    return seq_trimmed
...
def SAM_Header_to_Chrom_Length_DataFrame(l_header):
    ...
    df = pd.DataFrame(
        np.array(
            list(
                list(entry[3:] for entry in sorted(header[4:].split("\t"))[::-1])
                for header in l_header
                if "@SQ" in header
            )
        ),
        columns=["seqname", "seqlength"],
    )
...
def NGS_CIGAR_Retrive_List_of_Mapped_Segments(cigar, pos_global):
    ...
    l_seg, start, int_aligned_length, int_total_aligned_length = (
        list(),
        pos_global,
        0,
        0,
    ...
    for length, operation in NGS_CIGAR_Iterate_a_CIGAR_String(cigar):
        if operation in "MD=X":
            ...
            operation == "N"
        ):  # if splice junction appears, split the region and make a record
            l_seg.append(
                (start, start + int_aligned_length - 1)
            )  # set the end position
            start = start + int_aligned_length + length  # set the next start position
            int_total_aligned_length += (
                int_aligned_length  # increase total aligned length
            )
...
def NGS_GTF_Parse_Attribute(record):
    ...
    return ast.literal_eval(
        ...
        + ", ".join(
            list(
                '"' + entry.replace(' "', '" : "')
                for entry in record.strip()[:-1].split("; ")
            )
        )
        + "}"
    )
...
for aa in dict_NGS__aa_to_codon:
    ...
    for codon in l_codon:
        dict_NGS__codon_to_aa[codon] = aa


def NGS_SEQ_Translate(
    seq,
    start=0,
    ...
    while True:
        if start + 3 > length_seq:
            ...
            if return_error_value_if_stop_codon_not_found:
                return np.nan
            else:
                break
        try:
            aa = dict_NGS__codon_to_aa[
                seq[start : start + 3]
            ]  # try to translat the sequence
        except:
            if return_error_value_if_stop_codon_not_found:
                return (
                    np.nan
                )  # if an error occur during translating and stop codon was not found, return np.nan if 'return_error_value_if_stop_codon_not_found'
            else:
                break
        if aa == "*":
            if append_stop_codon_to_returned_seq:
                l_aa.append(aa)
            break
        else:
            l_aa.append(aa)
...
def NGS_SEQ_ORF_Find_All_Methionine_ORFs(seq, return_list=False):
    ...
    for int_atg_start_pos in STR.Find_all(
        seq, "ATG"
    ):  # find all positions of subsequences starting with ATG (methionine start codon)
        prot_seq = NGS_SEQ_Translate(
            seq,
            int_atg_start_pos,
            append_stop_codon_to_returned_seq=True,
            return_error_value_if_stop_codon_not_found=False,
            print_message=False,
        )
        end_pos = int_atg_start_pos + 3 * len(
            prot_seq
        )  # retrive end position of CDS, including a stop codon if detected.
        if l_l_value[-1][1] == end_pos:
            continue  # if current ORF shared the same stop position (and with downstream start position), do not include the ORF in the output, since current ORF is a fragment of the previous ORF
        else:
            l_l_value.append(
                [int_atg_start_pos, end_pos, prot_seq]
            )  # use 0-based coordinate
    return (
        l_l_value[1:]
        if return_list
        else pd.DataFrame(
            l_l_value[1:], columns=["start", "end", "translated_protein_seq"]
        )
...
def NGS_SEQ_ORF_Find_All_Methionine_ORFs_on_Both_Strands(
    ...
    int_length_seq = len(
        seq
    )  # convert coordinate of reverse complement of a sequence into that of the sequence
    ...
    df_orf = pd.concat(
        [df_plus, df_minus], ignore_index=True
    )  # combine ORFs from plus strand and the minus strand
    df_orf.sort_values("start", ignore_index=True, inplace=True)
    if use_1_based_coordinate:
        df_orf.start += 1  # use 1-based-coordinate for start and end positions if 'use_1_based_coordinate' is set to True
    return df_orf
...
def NGS_SEQ_Trim_PolyA(
    ...
    for index, str_base in enumerate(seq_polyA_at_5_prime_end):
        if str_base != str_repeated_base:
            if (
                seq_polyA_at_5_prime_end[index + 1 : index + 1 + int_lookup_window]
                == str_repeated_bases_in_lookup_window
            ):
                continue  # even though sequence is not 'A', look up the next bases, and if the next consequtive three bases are 'A', consider the character as heterogenity of PolyA and countinue counting number of 'A's in PolyA
            else:
                break
    if return_length_of_polyA:
        return index
    else:
        return (seq[:-index] if index > 0 else seq) if from_3_prime_end else seq[index:]
...
def NGS_SEQ_Encode_to_integer(seq, reverse_order_during_encoding=True):
    ...
    int_encoded_seq = 0
    if not reverse_order_during_encoding:
        seq = seq[::-1]
...
def NGS_SEQ_Calculate_Simple_Repeat_Proportion_in_a_read(
    seq, int_len_kmer=4, int_kmer_count_threshold_for_repeat=5
):
    ...
    dict_kmer_count = COUNTER(
        seq[index : index + int_len_kmer] for index in range(n_kmers)
    )  # count kmers of the given length
    int_kmer_count_repeat = (
        0  # count kmers that have count values above the given threshold
    )
    for kmer in dict_kmer_count:
        int_kmer_count = dict_kmer_count[kmer]
        if int_kmer_count > int_kmer_count_threshold_for_repeat:
            int_kmer_count_repeat += int_kmer_count
...
def SAM_Retrive_List_of_Mapped_Segments(
    cigartuples,
    pos_start,
...
def FASTA_Iterate(
    path_file_fasta, remove_space_in_header=False, header_split_at_space=False
):  # 2020-08-21 14:38:09
    """# 2021-12-28 20:27:27
    ...
    while True:
        ...
        while True:
            ...
            if not line or line[:1] == ">":
                break  # if encounter header line or the end of file, break the loop
            l_seq.append(line)  # append sequence
        ...
        yield {"header": header, "sequence": "".join(l_seq)}
        if not line:
            break
...
def FASTA_Assembly_Stats(
    ...
    if not flag_arr_length_is_sorted:
        ...
        arr_length = arr_length[
            ::-1
        ]  # reverse the order so that largest contigs are at the front
    ...
    for index, percentage in enumerate(arr_length_percentage[1:], 1):
        arr_length_percentage_cumulative[index] = (
            arr_length_percentage_cumulative[index - 1] + percentage
        )  # calculate cumulative length
    l_values = [
        arr_length.sum(),
        arr_length[0],
    ]  # retrive total_length and length of the longest contig
    for int_thres_percentage in np.arange(10, 100, 10):
        mask = (
            arr_length_percentage_cumulative < int_thres_percentage
        )  # mask for contigs below thres_percentage when cumulated percentage is calculated by using larger contigs first.
        l_values.extend(
            [arr_length[np.where(np.diff(mask))[0][0]], mask.sum()]
        )  # calculate assembly-stats
    l_values.extend([arr_length.min(), len(mask)])  # append values for N100
    s_assembly_stats = pd.Series(
        l_values,
        index=[
            "total_length",
            "longest",
...
def FASTA_Filter_by_Accession(
    path_fasta_input,
    path_fasta_output,
    set_accession,
    header_split_at_space=True,
    flag_insert_characters_every_n_characters=True,
):
    """
    2022-04-29
    ...
    new_file = (
        gzip.open(path_fasta_output, "wb")
        if bool_flag_output_gzipped
        else open(path_fasta_output, "w")
    )  # open file of path_file_fasta depending on the detected gzipped statu
    for r in FASTA_Iterate(
        path_fasta_input, header_split_at_space=header_split_at_space
    ):
            ...
            str_contents = (
                ...
                + (
                    STR.Insert_characters_every_n_characters(
                        r["sequence"], n_characters=60, insert_characters="\n"
                    )
                    if flag_insert_characters_every_n_characters
                    else r["sequence"]
                )
                + "\n"
            )
            new_file.write(
                str_contents.encode() if bool_flag_output_gzipped else str_contents
            )
    new_file.close()
...
def FASTA_DNA_break_scaffolds(dict_fasta, remove_space_in_header=True):
    ...
    if remove_space_in_header:
        dict_fasta = dict(
            (header.replace(" ", "_"), dict_fasta[header]) for header in dict_fasta
        )  # if 'remove_space_in_header' was set to True, replace space characers in the fasta header with '_'
    dict_fasta_scaffolds_broken = dict()
    for header in dict_fasta:
        seq = dict_fasta[header].upper()  # convert lower character 'n' with 'N'
        if (
            "N" in seq
        ):  # if 'N' is contained in the sequence, break scaffolds into contigs and modify fasta header
            for index, seq_broken_scaffolds in enumerate(seq.replace("N", " ").split()):
                dict_fasta_scaffolds_broken[
                    header + "_broken_scaffold_{index}".format(index=index)
                ] = seq_broken_scaffolds
        else:
            dict_fasta_scaffolds_broken[header] = seq
...
def FASTA_Filter(dict_fasta, int_thres_length):
    ...
    for header in set(
        header for header in dict_fasta if int_thres_length > len(dict_fasta[header])
    ):
        dict_fasta.pop(
            header
        )  # retrive set of headers with sequence length smaller than 'int_thres_length' and remove them from the dictionary
    return dict_fasta
...
def GENOME_Add_Global_Position(df, col_chrom, col_start, col_end):
    ...
    if "dict_chr_to_chr_start_global" not in globals():
        globals()["dict_chr_to_chr_start_global"] = (
            DB_Load_a_data_object(
                "df_anno_GRCh38_Ensembl__chrom_length", dataset="TE_eQTL"
            )
            .set_index("seqname")
            .start_position_global.to_dict()
        )  # load 'dict_chr_to_chr_start_global' if it has not been loaded.
...
def MSA_Calculate_Base_Frequency(l_seq, show_heatmap=False, cmap="viridis"):
    ...
    arr_seq = np.array(
        list(list(str_aligned_seq) for str_aligned_seq in l_seq), dtype=object
    )  # build an array of bases
    df_base_frequency = (
        pd.DataFrame(
            list(
                LIST_COUNT(arr_base_at_position, duplicate_filter=None)
                for arr_base_at_position in arr_seq.T
            )
        ).T.fillna(0)
        / len(arr_seq)
        * 100
    ...
    if show_heatmap:  # show heatmap of base frequency if 'show_heatmap' is True
        ...
        plt.yticks(
            np.arange(len(df_base_frequency)), labels=df_base_frequency.index.values
        )
        MATPLOTLIB_basic_configuration(
            show_grid=False,
            y_lim=dict(top=-0.5, bottom=len(df_base_frequency) - 0.5),
            show_colorbar=True,
        )
...
def NEWICK_Parse_by_Recursion(str_newick_tree, default_branch_length=1, verbose=False):
    ...
    while pos < len_str_newick_tree:
        ...
        if count_layer_parentheses == 0 and (
            character == "," or pos == len_str_newick_tree - 1
        ):  # parse a node
            ...
            if (
                len(str_node) > 0 and str_node[0] == "("
            ):  # if node not empty and is a nested newick tree
                (
                    str_nested_newick_tree,
                    str_name_and_branch_length_of_node,
                ) = str_node.rsplit(")", 1)
                str_nested_newick_tree = str_nested_newick_tree[
                    1:
                ]  # remove the initial '(', too, to remove it from the nested structure.
                if verbose:
                    print("nested tree is " + str_nested_newick_tree[:10])
                dict_subtree = NEWICK_Parse_by_Recursion(
                    str_nested_newick_tree,
            ...
            if ":" in str_name_and_branch_length_of_node:
                (
                    str_name_node,
                    str_float_branch_length,
                ) = str_name_and_branch_length_of_node.split(":")
                float_branch_length = (
                    default_branch_length
                    if len(str_float_branch_length) == 0
                    else float(str_float_branch_length)
                )  # assign default branch length if the entry is empty
            else:
            ...
            if len(str_name_node) == 0:
                str_name_node = (
                    UUID()
                )  # assign random node name if no node name was given.
            if str_name_node in dict_tree:
                str_name_node += (
                    "_" + UUID()
                )  # if current node name is not unique, add unique identifier (random barcode) to the node name
            dict_tree[str_name_node] = dict(
                name_node=str_name_node,
                branch_length=float_branch_length,
                dict_subtree=dict_subtree,
...
def NEWICK_Get_Coordinates_of_Nodes_and_Edges(
    dict_newick_tree, float_space_between_neighbors=0.01
):
    """# 2020-12-10 20:56:09
     ===== Returns =====
    ...
    """
    if isinstance(
        dict_newick_tree, (str)
    ):  # if a newick tree is given as a string, parse the string first
        dict_newick_tree = NEWICK_Parse_by_Recursion(dict_newick_tree)
    float_positon_top_of_tree_previous = (
        0  # previous position of the top of all tree (y coordinates)
    )
    float_positon_top_of_tree = (
        0  # current position of the top of all tree (y coordinates)
    )
    ...
    for name_node in dict_newick_tree:
        ...
        if len(dict_subtree) > 0:  # if current node contains a subtree
            (
                arr_coordinates_subtree,
                arr_edges_subtree,
            ) = NEWICK_Get_Coordinates_of_Nodes_and_Edges(dict_subtree)
            n_nodes = len(arr_coordinates_subtree)  # retrieve the number of nodes
            float_branch_length = record["branch_length"]
            arr_coordinates_subtree[
                :, 0
            ] += float_branch_length  # push subtree to right by 'float_branch_length'
            arr_coordinates_subtree[:, 1] += (
                float_positon_top_of_tree + float_space_between_neighbors
            )  # lift subtree to the top with a given space between subtrees
            float_positon_top_of_tree_previous = float_positon_top_of_tree_previous  # update 'float_positon_top_of_tree_previous'
            float_positon_top_of_tree = (
                np.max(arr_coordinates_subtree[:, 1])
                if n_nodes > 20
                else max(arr_coordinates_subtree[:, 1])
            )  # update the topest position of the tree (max is faster than np.max when number of nodes is small due to minimal overhead)
            l_arr_coordinates.append(arr_coordinates_subtree)
            if (
                len(arr_edges_subtree) > 0
            ):  # if edges of subtree is not empty (there is another subtree inside the subtree), add the edges to the list of edges
                l_arr_edges.append(arr_edges_subtree)
            l_edges = list(
                [name_node_child, name_node] for name_node_child in dict_subtree
            )
            l_arr_edges.append(
                np.array(l_edges, dtype=object)
            )  # add current node to the list of coordinates
        else:  # if there is no subtree
            float_positon_top_of_tree_previous = (
                float_positon_top_of_tree  # update the topest position of the tree
            )
            float_positon_top_of_tree += float_space_between_neighbors
        l_arr_coordinates.append(
            np.array(
                [
                    0,
                    (float_positon_top_of_tree + float_positon_top_of_tree_previous)
                    / 2,
                    name_node,
                ],
                dtype=object,
    ...
    return (
        np.vstack(l_arr_coordinates)
        if len(l_arr_coordinates) > 1
        else l_arr_coordinates
    ), (np.vstack(l_arr_edges) if len(l_arr_edges) > 0 else l_arr_edges)
...
def Kmer_Calculater_Kmer_Frequency(dict_fasta, int_length_kmer=6):
    ...
    s = (
        pd.Series(dict_seq_id_to_length) - int_length_kmer + 1
    )  # calculate total number of kmer
    return (
        df / s
    )  # calculate Kmer frequency by dividing count with total number of kmer
...
def INTERVALTREE_Visualize_Intervals(arr_intervals, sort_by_start=True, marker="o-"):
    ...
    if type(arr_intervals) is not np.ndarray:
        arr_intervals = np.array(arr_intervals, dtype=object)
    if sort_by_start:
        arr_intervals = arr_intervals[
            np.argsort(list(interval[0] for interval in arr_intervals))
        ]
...
def GTF_Visualize_Intervals_with_Exons_and_Introns(
    start,
    end,
    arr_intervals_of_interest=None,
    arr_pileup_count=None,
    figsize=(22, 6),
    show_legend=True,
    show_pileup_count_in_log_scale=False,
):
    ...
    if arr_pileup_count is None:
        fig, ax_annotation = plt.subplots(1, 1, figsize=figsize)
    else:
    ...
    if (
        len(arr_repeats) > 0
    ):  # if there is valid repeat element to visualize in a given window
        set_type_repeat = set()
        for interval in arr_repeats:
            id_repeat, family_repeat, type_repeat, strand_repeat = interval[2]
            if type_repeat == "SINE":
                alpha, lw, marker, color = (
                    0.7,
                    4.5,
                    "x-",
                    "red",
                )  # graphic setting for each repeat element
            elif type_repeat == "ERV":
                alpha, lw, marker, color = 0.7, 4.5, "x-", "orange"
            elif type_repeat == "LINE":
                alpha, lw, marker, color = 0.7, 4.5, "x-", "green"
            elif type_repeat == "DNA":
                alpha, lw, marker, color = 0.7, 4.5, "x-", "blue"
            elif type_repeat == "Satellite":
                alpha, lw, marker, color = 0.7, 4.5, "x-", "purple"
            else:
                alpha, lw, marker, color = 0.7, 4.5, "x-", "grey"
            if (
                type_repeat not in set_type_repeat
            ):  # if current type_repeat does not have legend, add annotation to the legend
                l_legend_line.append(
                    mpl.lines.Line2D([0], [0], marker="x", color=color, lw=lw)
                ), l_legend_label.append(
                    type_repeat
                )  # add label to legend for annotation
            ...
            ax_annotation.arrow(
                ...
                - direction_arrow * int_length_interval / 10,
                y=index,
                dx=direction_arrow * int_length_interval / 5,
                dy=0,
                fc=color,
                ec=color,
                length_includes_head=True,
                head_width=0.8,
                head_length=int_length_interval / 5,
                shape="full",
                alpha=alpha,
            )
        index += 1
    ...
    if (
        len(arr_intervals) > 0
    ):  # if there is valid component to visualize in a given window
        set_tx_id = set()
        for interval in arr_intervals:
            for comp_info in dict_comp_id_to_l_gene_id__tx_id__type__strand[
                interval[2]
            ]:
                set_tx_id.add(comp_info[1])
        df_gene_tx = pd.DataFrame(
            list([dict_tx_id_to_gene_id[tx_id], tx_id] for tx_id in set_tx_id),
            columns=["gene_id", "tx_id"],
        ).sort_values(["gene_id", "tx_id"])
        l_tx_id, l_gene_id = df_gene_tx.tx_id.values, df_gene_tx.gene_id.values
        l_l_interval_for_a_tx = list(
            list(
                [
                    dict_comp_id_to_start[comp_id],
                    dict_comp_id_to_end[comp_id],
                    comp_id,
                    comp_type,
                    dict_tx_id_to_strand[tx_id],
                ]
                for comp_id, comp_type in dict_tx_id_to_l_comp_id__type[tx_id]
            )
            for tx_id in l_tx_id
        )
        gene_id_previous, color = l_gene_id[0], next(cycler_color)
        l_legend_line.append(
            mpl.lines.Line2D([0], [0], color=color, lw=3)
        ), l_legend_label.append(gene_id_previous)
        for l_interval_for_a_tx, tx_id, gene_id in zip(
            l_l_interval_for_a_tx, l_tx_id, l_gene_id
        ):
            ...
            arr_interval_for_a_tx = arr_interval_for_a_tx[
                arr_interval_for_a_tx[:, 0].argsort()
            ]  # sort intervals by its start position
            for interval in arr_interval_for_a_tx:
                if interval[3] == "exon":
                    alpha, lw, marker = (
                        ...
                        6,
                        "-",
                    )  # graphic setting for exon and introns
                else:
                    alpha, lw, marker = 0.5, 3, "--"
                int_length_interval = interval[1] - interval[0]
            ...
            index += 1
    if (
        arr_intervals_of_interest is not None
    ):  # visualize given 'arr_intervals_of_interest'
        for interval in arr_intervals_of_interest:
            alpha, lw, marker, color = (
                ...
                "-",
                next(cycler_color),
            )  # graphic setting for intervals_of_interest
            l_legend_line.append(
                mpl.lines.Line2D([0], [0], color=color, lw=3)
            ), l_legend_label.append(interval[2])
            int_length_interval = interval[1] - interval[0]
    ...
    if show_legend:
        ax_annotation.legend(l_legend_line, l_legend_label)

    if arr_pileup_count is not None:
        n_bases = end - start + 1
        int_sampling_a_base_every_n_bases = max(1, int(n_bases / 10000))
        x_range, data = (
            np.arange(start, end + 1, int_sampling_a_base_every_n_bases),
            arr_pileup_count[start - 1 : end : int_sampling_a_base_every_n_bases],
        )
        x_axis = np.zeros(len(x_range))
        ax_pileup.plot(x_range, data, color="black")
        ax_pileup.fill_between(
            x_range, data, x_axis, facecolor="black", interpolate=True, alpha=0.5
        )
        ax_pileup.set_xlim(start, end + 1)
        if show_pileup_count_in_log_scale:
            ax_pileup.set_yscale("log")
...
def STAR_Read_Log(path_glob):
    ...
    for path_file in glob.glob(path_glob):
        ...
        for key, value in list(
            line.strip().split(" |\t")
            for line in Search_list_of_strings_with_multiple_query(l_line, "\t")
        ):
            if "%" in value:
                value = value[:-1]
            try:
...
def STAR_Parse_final_out(path_file, return_numeric=False):
    ...
    if return_numeric:
        s.iloc[:] = list(
            value if ":" in value else float(value.replace("%", ""))
...
def FASTQ_Read_Generator(path_file, return_only_at_index=None):  # 2020-08-18 22:31:31
    ...
    while True:
        ...
        if return_only_at_index is not None:
            ...
            yield [record[0], record[1], record[3]]
    file.close()
...
def FASTQ_Subsample(
    ...
    int_num_reads_to_subsample : int = 200000, # once this number of reads were written, the reads will be no longer written to the output file
    """
    int_count = 0
...
def FASTQ_Split_with_adaptor(path_file, path_file_new, str_seq_adaptor):
    ...
    """
    flag_newfile_gzipped = (
        path_file_new.rsplit(".", 1)[1] == "gz"
    )  # retrieve flag for gzipped status of the output file
    newfile = (
        gzip.open(path_file_new, "wb")
        if flag_newfile_gzipped
        else open(path_file_new, "w")
    )
    for record in FASTQ_Read(
        path_file, return_generator=True, return_only_at_index=None
    ):
        ...
        for index in range(len(l_pos) - 1):
            str_record = (
                "\n".join(
                    [
                        name_read + "_" + str(index),
                        str_seq[l_pos[index] : l_pos[index + 1]],
                        "+",
                        str_qual[l_pos[index] : l_pos[index + 1]],
                    ]
                )
                + "\n"
            )
...
def XML_Read(
    path_file, encoding="utf-8", character_for_replacing_invalid_characters=" "
):  # 2020-08-18 18:07:59
    ...
    with open(path_file, "r", encoding=encoding) as file:
        ...
        try:
            ...
            for int_char in np.arange(32):
                if int_char in [9, 10, 13]:
                    continue  # ignore white space characters
                data = data.replace(
                    chr(int_char), character_for_replacing_invalid_characters
                )  # remove special characters from xml content before parsing
            dict_xml = xmltodict.parse(data)
    return dict_xml
...
def HMMER_Map_Positions_Between_Pairwise_Alignment(
    query_alignment, target_alignment, query_start, target_start, char_gap="-"
):  # 2020-07-03 13:56:35
    ...
    for residue_query_alignment, residue_target_alignment in zip(
        query_alignment, target_alignment
    ):
        if (
            residue_query_alignment != char_gap and residue_target_alignment != char_gap
        ):  # no gap in either target or query sequences
            int_start_query += 1
            int_start_target += 1
            dict_target_pos_to_query_pos[target_start + int_start_target - 1] = (
                query_start + int_start_query - 1
            )
            ...
            dict_target_pos_to_query_pos[target_start + int_start_target - 1] = (
                query_start + int_start_query - 1
            )  # position in target sequence inside insertion will be mapped to the position where insertion begins
...
def HMMER_HMMSEARCH_Read_output(path_file):
    ...
    l_l_values = (
        list()
    )  # list-based 2D array of values that will store alignment outputs for all hmmsearch output records
    with open(path_file) as file:
        while True:  # iterate line in the file
            ...
            if len(line) == 0:
                ...
                content = line[:-1]
            if len(content) == 0:
                continue  # skip black line
            elif "#" == content[0]:
                continue  # skip comments
            elif "Query:" == content[:6]:
                acc_query = content.split()[
                    1
                ]  # retrive accession of the current query profile (human sequence's uniprot accession)
            elif ">>" == content[:2]:  # for each alignment record
                acc_target = content[2:].split()[
                    0
                ]  # retrive an identifier for the target protein (identifier is the first entry of white-space splitted fasta header of the target protein)
                line_header = file.readline()
                if " [No individual domains that" in line_header:
                    continue  # if current alignment records does not contain domain-level alignment, continue on next record.
                file.readline()  # discard header lines  # '   score  bias  c-Evalue  i-Evalue hmmfrom  hmm to    alifrom  ali to    envfrom  env to     acc', ' ---   ------ ----- --------- --------- ------- -------    ------- -------    ------- -------    ----',
                l_l_values_of_a_record = (
                    list()
                )  # list-based 2D array of values that will store an alignment output for a hmmsearch output record of a given query profile and a target protein
                while True:
                    line = file.readline()
                    if len(line) == 1:
                        break  # if line contain only newline character, finish retriving alignment records in tabular format
                    else:
                        content = line[:-1]
                    l_l_values_of_a_record.append(
                        [acc_query, acc_target] + content.split()
                    )
                file.readline()  # discard one line #  'Alignments for each domain:'
                for index in range(len(l_l_values_of_a_record)):
                    cc = (
                        file.readline()
                    )  # discard one line # == domain 1  score: 9.0 bits;  conditional E-value: 0.019',
                    line_alignment = (
                        file.readline()
                    )  # '                                     xxxxxxxxxxxxxxxxxxxxxxxxxxxxx....xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx RF' for jackhmmer HMM model after first iteration. HMM model from the first iteration does not contain this field, and it should be skipped.
                    str_query_alignment = (
                        line_alignment.split()[2]
                        if acc_query in line_alignment
                        else file.readline().split()[2]
                    )  # '           sp|P10155|RO60_HUMAN-i4 372 vllvDvSGSMsasvsgsrltaakaaaalaaallrrgdrvglvaFdtr.vvdlpltprdsvlallaalaalpgGgTdigaalraalrrkar..rdvvvliTDgeendgpehptealae 485',
                    dd = (
                        file.readline()
                    )  # discard one line # '                                       ++l+DvS S ++       + a+ +++ a+  l++g ++g++aF+ +  vd p++++  +  + a ++     g d+  alr+a +  +   +  v +i+Dg +  ++++  +++a+',
                    ...
                    l_l_values_of_a_record[index].extend(
                        [str_query_alignment, str_target_alignment]
                    )
                l_l_values.extend(
                    l_l_values_of_a_record
                )  # add alignment information for current record to the 2D array
    ...
    df.columns = [
        ...
        "target_end",
        "target_status",
        "target_envelope_start",
        "target_envelope_end",
        "target_envelope_status",
        "accuracy",
        "query_alignment",
        "target_alignment",
    ]
    l_col_interger_datatype = [
        "index_match",
        "query_start",
        "query_end",
        "target_start",
        ...
        "target_envelope_start",
        "target_envelope_end",
    ]  # column names with float datatype
    l_col_float_datatype = [
        "score",
        "bias",
        "conditional_Evalue",
        "independent_Evalue",
        "accuracy",
    ]  # column names with integer datatype
    df.loc[:, l_col_float_datatype] = df.loc[:, l_col_float_datatype].astype(
        float
    )  # convert values in columns for numeric datatype to float datatype
    df.loc[:, l_col_interger_datatype] = df.loc[:, l_col_interger_datatype].astype(
        int
    )  # convert values in columns for numeric datatype to integer datatype
    return df
...
def HMMER_Add_CIGAR_String(
    df, dict_hmm_model_name_to_consensus_length
):  # 2020-05-20 17:02:41
    ...
    for (
        ...
        int_hmmfrom,
        int_hmm_to,
        str_consensus_alignment,
        str_target_alignment,
    ) in df[["model_name", "hmmfrom", "hmm_to", "seq_consensus", "seq_target"]].values:
        len_seq_consensus = dict_hmm_model_name_to_consensus_length[model_name]
        int_length_seq = len(str_consensus_alignment)
        l_cigar_comp = (
            list()
        )  # list of strings that will yield cigar string when joined
        l_cigar_comp.append(
            [0, "START"]
        )  # put a component indicating the start of cigar string
        if int_hmmfrom > 1:
            l_cigar_comp.append(
                [(int_hmmfrom - 1), "S"]
            )  # introduce soft-cliping record to cigar string where concensus sequence was not mapped
        bool_flag_insertion, bool_flag_deletion, bool_flag_match, bool_flag_mismatch = (
            False,
            False,
            False,
            False,
        ...
        for index in range(int_length_seq):
            char_consensus, char_target = (
                str_consensus_alignment[index],
                str_target_alignment[index],
            )
            str_operation = l_cigar_comp[-1][1]
            if (
                char_consensus == "."
            ):  # when deletion (respect to genome, deletion in the consensus sequence) occured at current position
                if str_operation == "D":
                    l_cigar_comp[-1][0] += 1
                else:
                    l_cigar_comp.append([1, "D"])
            elif (
                char_target == "-"
            ):  # when insertion (respect to genome, insertion in the consensus sequence) occured at current position
                if str_operation == "I":
                    l_cigar_comp[-1][0] += 1
                else:
                    l_cigar_comp.append([1, "I"])
            elif (
                char_consensus == char_target
            ):  # when match occured at current position
                if str_operation == "=":
                    l_cigar_comp[-1][0] += 1
                else:
                    l_cigar_comp.append([1, "="])
            else:  # when mismatch occured at current position
                if str_operation == "X":
                    l_cigar_comp[-1][0] += 1
                else:
                    l_cigar_comp.append([1, "X"])
        #     else : # when match or mismatch occured at current position
        #         if str_operation == 'M' : l_cigar_comp[ - 1 ][ 0 ] += 1
        #         else : l_cigar_comp.append( [ 1, 'M' ] )
        if int_hmm_to < len_seq_consensus:
            l_cigar_comp.append(
                [len_seq_consensus - int_hmm_to, "S"]
            )  # # introduce soft-cliping record to cigar string where concensus sequence was not mapped
        l_cigar_comp = l_cigar_comp[
            1:
        ]  # exclude the component that was used to indicate the start of cigar string
...
def HMMER_Read_nhmmscan_output_with_alignment(path_file):  # 2020-05-20 17:02:41
    ...
    with open(path_file) as file:
        ...
        while (
            ...
            if len(line) == 0:
                ...
                content = line[:-1]
            if (
                "# target HMM database:" in content
            ):  # check whether current line containing HMM db directory
                path_file_hmm_db = content.split("# target HMM database: ")[1].strip()
                dict_fasta = FASTA_Read(
                    os.popen("hmmemit -C {}".format(path_file_hmm_db))
                )
                dict_hmm_model_name_to_consensus_length = dict(
                    (
                        hmm_model_name.rsplit("-consensus", 1)[0],
                        len(dict_fasta[hmm_model_name]),
                    )
                    for hmm_model_name in dict_fasta
                )  # retrive length of consensus sequences in a given HMM db, while removing '-consensus' tag from the hmm_model_name
                break
        while True:
            while True:
                line = file.readline()
                if len(line) == 0:
                    ...
                    content = line[:-1]
                if (
                    len(content) > 0 and content[0] != "#"
                ):  # ignore empty content and comments
                    if "Query:" in content:
                        str_query_sequence_name = (
                            content.split("Query:")[1].split("[L=")[0].strip()
                        )
                        int_query_sequence_length = int(
                            content.split("[L=")[1].split("]", 1)[0]
                        )
                        bool_flag_new_query_sequence_start = True
                    elif "-------" in content and bool_flag_new_query_sequence_start:
                        str_field_locations = content
                        l_l_field_annotation_start_end = STR.Read_Field_Locations(
                            str_field_locations
                        )
                        slice_field_e_value = slice(
                            l_l_field_annotation_start_end[0][0],
                            l_l_field_annotation_start_end[0][1],
                        )
                        slice_field_score = slice(
                            l_l_field_annotation_start_end[1][0],
                            l_l_field_annotation_start_end[1][1],
                        )
                        slice_field_bias = slice(
                            l_l_field_annotation_start_end[2][0],
                            l_l_field_annotation_start_end[2][1],
                        )
                        int_start_position__field_start = (
                            l_l_field_annotation_start_end[4][1]
                            - len(str(int_query_sequence_length))
                        )
                        slice_field_start = slice(
                            int_start_position__field_start,
                            l_l_field_annotation_start_end[4][1],
                        )
                        slice_field_end = slice(
                            l_l_field_annotation_start_end[5][1]
                            - len(str(int_query_sequence_length)),
                            l_l_field_annotation_start_end[5][1],
                        )
                        slice_field_model = slice(
                            l_l_field_annotation_start_end[3][0],
                            int_start_position__field_start - 1,
                        )
                        ...
                        l_l_aligned_section_field_annotation_start_end = (
                            STR.Read_Field_Locations(
                                str_aligned_section_field_locations
                            )
                        )
                        content = file.readline()[:-1]
                        bool_flag_new_alignment_start = False
                        file.readline()
                        if "Alignment:" not in file.readline():
                            print("error")
                        if "score:" not in file.readline():
                            print("error")
                        content_consensus = file.readline()[:-1]
                        _ = file.readline()[:-1]
                        content_target = file.readline()[:-1]
                        content_quality = file.readline()[:-1]
                        index_start, value_counter = (
                            0,
                            0,
                        )  # retrive sequence start index and end index
                        for value in content_consensus.split(" ")[1:]:
                            if value_counter < 2:
                                ...
                                break
                        l_l_value__alignment_result.append(
                            [
                                str_query_sequence_name,
                                str_aligned_model_name,
                                content[
                                    l_l_aligned_section_field_annotation_start_end[0][0]
                                    - 2
                                ],
                                float(
                                    content[
                                ...
                                content[
                                    l_l_aligned_section_field_annotation_start_end[4][1]
                                    + 1 : l_l_aligned_section_field_annotation_start_end[
                                        5
                                    ][
                                        0
                                    ]
                                ...
                                content[
                                    l_l_aligned_section_field_annotation_start_end[6][1]
                                    + 1 : l_l_aligned_section_field_annotation_start_end[
                                        7
                                    ][
                                        0
                                ...
                                content[
                                    l_l_aligned_section_field_annotation_start_end[8][1]
                                    + 1 : l_l_aligned_section_field_annotation_start_end[
                                        9
                                    ][
                                        0
                                ...
                                content_consensus[index_start:index_end],
                                content_target[index_start:index_end],
                                content_quality[index_start:index_end],
                            ]
                        )
            while True:
                line = file.readline()
                if len(line) == 0:
                    ...
                    content = line[:-1]
                if len(content) == 0:
                    break  # finished the loop when empty content is detected
                if "------ inclusion threshold ------" in content:
                    continue  # ignore the content indicating inclusion threshold
                l_l_value__search_result.append(
                    [
                        float(content[slice_field_e_value]),
                        float(content[slice_field_score]),
                        float(content[slice_field_bias]),
                        int(content[slice_field_start]),
                        int(content[slice_field_end]),
                        str_query_sequence_name,
                        content[slice_field_model].strip(),
                    ]
                )
            if len(line) == 0:
                break
    df_search_result = pd.DataFrame(
        l_l_value__search_result,
        columns=[
            "e_value",
            "score",
            "bias",
    ...
    df_alignment_result = HMMER_Add_CIGAR_String(
        pd.DataFrame(
            l_l_value__alignment_result,
            columns=[
                "query_sequence_name",
                "model_name",
                "inclusion",
        ...
        dict_hmm_model_name_to_consensus_length,
    )
    ...
    with open(path_file) as file:
        ...
        while (
            ...
            if len(line) == 0:
                ...
                content = line[:-1]
            if (
                "# query file:" in content
            ):  # check whether current line containing HMM db directory
                path_file_hmm_db = content.split("# query file: ")[1].strip()
                if path_file_hmm_db.rsplit(".", 1)[1].lower() in [
                    "fa",
                    "fasta",
                ]:  # if query file is a fasta file
                    dict_fasta = FASTA_Read(path_file_hmm_db)
                    dict_hmm_model_name_to_consensus_length = dict(
                        (
                            path_file_hmm_db.rsplit("/", 1)[1].rsplit(".", 1)[0],
                            len(dict_fasta[hmm_model_name]),
                        )
                        for hmm_model_name in dict_fasta
                    )  # retrive length of sequence # model name is filename
        ...
        while True:
            while True:
                ...
                if (
                    ...
                    if "Query:" in content:
                        str_query_sequence_name = (
                            content.split("Query:")[1].split("[M=")[0].strip()
                        )
                        int_query_sequence_length = int(
                            content.split("[M=")[1].split("]", 1)[0]
                        )
                        ...
                        slice_field_Sequence = slice(
                            l_l_field_annotation_start_end[3][0],
                            l_l_field_annotation_start_end[3][1],
                        )
                        slice_field_start = slice(
                            l_l_field_annotation_start_end[3][1] + 1,
                            l_l_field_annotation_start_end[4][1],
                        )
                        slice_field_end = slice(
                            l_l_field_annotation_start_end[4][1] + 1,
                            l_l_field_annotation_start_end[5][1],
                        )
                        slice_field_Description = slice(
                            l_l_field_annotation_start_end[5][1] + 1, None
                        )
                        bool_flag_search_result_table_start = False
                        int_n_extra_characters_for_Sequence = (
                            0  # number of extra characters for Sequence annotation
                        )
                        break
                    elif ">>" in content:  # retrive alignment records
                        bool_flag_new_alignment_start = True
                        int_description_start = content.find("dna:")
                        str_Sequence, str_Description = (
                            content[3:int_description_start].strip(),
                            content[int_description_start:].strip(),
                        )
                    elif "-------" in content and bool_flag_new_alignment_start:
                        str_aligned_section_field_locations = content
                        l_l_aligned_section_field_annotation_start_end = (
                        ...
                        if "score:" not in file.readline():
                            print("error")
                        str_first_line_alignment = file.readline()[
                            :-1
                        ]  # retrieve first line of the alignment
                        content_consensus = (
                            file.readline()[:-1]
                            if " RF" in str_first_line_alignment
                            else str_first_line_alignment
                        )  # if a mask of reference sequence showing indels is present in the first line of the alignment record, use the next line for consensus sequence
                        _ = file.readline()[:-1]
                        content_target = file.readline()[:-1]
                        content_quality = file.readline()[:-1]
                        index_start, value_counter = (
            ...
            while True:
                ...
                if len(content) == 0:
                    break  # finished the loop when empty content is detected
                if "------ inclusion threshold ------" in content:
                    continue  # ignore the content indicating the inclusion threshold
                try:
                    int(content[slice_field_start])
                except:
                    while True:
                ...
                l_l_value__search_result.append(
                    [
                        ...
                        float(content[slice_field_score]),
                        float(content[slice_field_bias]),
                        content[slice_field_Sequence].strip(),
                        int(content[slice_field_start]),
                        int(content[slice_field_end]),
                        content[slice_field_Description].strip(),
                        str_query_sequence_name,
                    ]
                )
...
def NHMMER_Plot_Frequency(
    df_alignment_result, len_hmm, n_seq=None, thres_frequency_for_consensus_seq=0.6
):
    """
    # 2021-02-04 14:16:44
    ...
    dict_residue_to_index = dict(
        (residue, index) for index, residue in enumerate(str_residues)
    )
    ...
    for record in df.iloc[:n_seq].to_dict(orient="record"):
        ...
        int_start_hmm = (
            record["hmmfrom"] - 1 - 1
        )  # 0-based coordinate, before (-1) the first index
        int_start_target = -1
    ...
    arr_residue = np.full(
        len_hmm, "N", dtype=object
    )  # 1d array containing consensus-base-called nucleotide residues
    for index_residue, index_pos in zip(
        *np.where(arr_frequency > thres_frequency_for_consensus_seq)
    ):
...
def BLAST_Parse_BTOP_String(
    str_btop, query_seq=None, subject_seq=None
):  # 2020-07-02 17:48:53
    ...
    if query_seq is None and subject_seq is None:
        return (
            -1
        )  # either 'query_seq' or 'subject_seq' should be given to build aligned query and subject sequences.
    ...
    while True:
        try:
            char = next(iter_str_btop)
        except:
            break
        if char in "0123456789":
            str_number += char  # if encountered is a number character of a number indicating the number of local matches, add up these number characters
        else:
            if (
                len(str_number) > 0
            ):  # if there was a local match, retrieve a locally matched sequence from either query or subject sequences and append the sequence to aligned aligned query and subject sequences.
                n_matches = int(str_number)
                if bool_flag_use_query_seq:
                    seq_matched = query_seq[index : index + n_matches]
                else:
                    seq_matched = subject_seq[index : index + n_matches]
            ...
            char_2 = next(
                iter_str_btop
            )  # if mismatch or gap is encountered, update aligned sequences accordingly
            query_seq_aligned += char
            subject_seq_aligned += char_2
            if char == "-" and not bool_flag_use_query_seq:
                index += 1
            elif char_2 == "-" and bool_flag_use_query_seq:
                index += 1
            elif char != "-" and char_2 != "-":
                index += 1
    if (
        len(str_number) > 0
    ):  # if there was a local match just before the end of a given btop string, retrieve a locally matched sequence from either query or subject sequences and append the sequence to aligned aligned query and subject sequences.
        n_matches = int(str_number)
        if bool_flag_use_query_seq:
            seq_matched = query_seq[index : index + n_matches]
        else:
...
def BLAST_Read(
    path_file,
    dict_qaccver_to_seq=None,
    dict_saccver_to_seq=None,
    path_file_output=None,
    float_transfer_pidenta=None,
    float_transfer_evalueb=None,
    **dict_Select,
):  # 2020-11-15 17:21:00
    """# 2022-01-17 22:18:45
    ...
    """
    if dict_qaccver_to_seq is None and dict_saccver_to_seq is None:
        return (
            -1
        )  # For BTOP String parsing, either 'dict_qaccver_to_seq' or 'dict_saccver_to_seq' should be given as a template sequence.
    bool_flag_use_query_seq = (
        dict_qaccver_to_seq is not None
    )  # set a flag for using query_seq
    ...
    if path_file_output is None:
        ...
        try:
            ...
            print(f"{path_file} file contains empty blast output")
            return -1
        df_blastp.columns = [
            "qaccver",
            "saccver",
        ...
        df_blastp = PD_Select(
            df_blastp, **dict_Select
        )  # retrieve search result of multiple-sequence-aligned sequences
        ...
        l_l_value = (
            list()
        )  # build aligned query and subject sequences for blastp search results
        for qaccver, qstart, qend, saccver, sstart, send, btop in df_blastp[
            ["qaccver", "qstart", "qend", "saccver", "sstart", "send", "btop"]
        ].values:
            query_seq_aligned, subject_seq_aligned = (
                BLAST_Parse_BTOP_String(
                    btop, query_seq=dict_qaccver_to_seq[qaccver][qstart - 1 : qend]
                )
                if bool_flag_use_query_seq
                else BLAST_Parse_BTOP_String(
                    btop, subject_seq=dict_saccver_to_seq[saccver][sstart - 1 : send]
                )
            )  # parse BTOP string according to the given type of sequence
            l_l_value.append([query_seq_aligned, subject_seq_aligned])
        df_blastp = df_blastp.reset_index(drop=True).join(
            pd.DataFrame(
                l_l_value, columns=["query_seq_aligned", "subject_seq_aligned"]
            )
        )
        return df_blastp
    else:
        ...
        try:
            file = (
                gzip.open(path_file, "rb")
                if flag_is_input_file_gzipped
                else open(path_file, "r")
            )
        except:
            print(f"[Error] error while opening the file {path_file}")
            return -1
        """
        open an output file
        """
        ...
        l_index_col_for_retrieving_aligned_seq = list(
            l_name_col.index(name_col)
            for name_col in [
                "qaccver",
                "qstart",
                "qend",
        ...
        )  # retrieve indices of the columns of interest
        index_col_pident = l_name_col.index("pident")
        index_col_evalue = l_name_col.index("evalue")
        """ write a new header """
        newfile.write(
            (
                "\t".join(l_name_col + ["query_seq_aligned", "subject_seq_aligned"])
                + "\n"
            ).encode()
        ...
        while True:
            ...
            )  # parse values as string values
            """ filter records based on thresholds """
            if (
                float_transfer_pidenta is not None
                and l_value[index_col_pident] < float_transfer_pidenta
            ):
                continue
            if (
                float_transfer_evalueb is not None
                and l_value[index_col_evalue] > float_transfer_evalueb
            ):
                continue
            qaccver, qstart, qend, saccver, sstart, send, btop = tuple(
                l_value[i] for i in l_index_col_for_retrieving_aligned_seq
            )  # retrieve values for retrieving aligned sequence
            query_seq_aligned, subject_seq_aligned = (
                BLAST_Parse_BTOP_String(
                    btop, query_seq=dict_qaccver_to_seq[qaccver][qstart - 1 : qend]
                )
            ...
            newfile.write(
                (
                    "\t".join([line[:-1], query_seq_aligned, subject_seq_aligned])
                    + "\n"
                ).encode()
            )  # write a new record
        newfile.close()
...
def CIF_Read(path_file):  # 2020-07-20 20:31:16
    ...
    bool_flag_gzipped = (
        path_file[-3:] == ".gz"
    )  # flag for gzipped status of a given cif file
    dict_cif = dict()
    ...
    with gzip.open(path_file, "rb") if bool_flag_gzipped else open(path_file) as file:
        while True:
            ...
            if len(line) == 0:
                ...
                line = (
                    line.strip()
                )  # read one line and strip empty characters at both ends
            if len(line) == 0:
                continue  # ignore empty line
            if (
                str_spanning_multiple_line is not None
            ):  # when the current line belongs to a multiple-lines-spanning string
                if (
                    line[0] == ";"
                ):  # when the end of a multiple-lines-spanning string is detected
                    if bool_flag_datetype_category_is_tabular and len(
                        l_l_value[0]
                    ) == len(l_l_value[-1]):
                        l_l_value.append(
                            [str_spanning_multiple_line]
                        )  # add a value spanning multiple lines as a first value of a new row if category datatype is tabular datatype and the number of values in the previous row is the same as the number of attribute names
                    else:
                        l_l_value[-1].append(
                            str_spanning_multiple_line
                        )  # add a value spanning multiple lines to the current row (eigher key-value or tabular category datatype)
                    str_spanning_multiple_line = (
                        None  # flush a string containing value spanning multiple-lines
                    )
                else:
                    str_spanning_multiple_line += line.replace(
                        "\t", " "
                    )  # when '#' character inside a value spanning multiple lines
            elif (
                "data_" == line[:5]
            ):  # when new data_block starts, add 'dict_data_block' to 'dict_cif' with 'str_name_data_block' and empties 'dict_data_block'
                if len(dict_data_block) > 0:
                    dict_cif[str_name_data_block] = dict_data_block
                str_name_data_block = line[5:]
                dict_data_block = dict()
            elif (
                "#" == line[0]
            ):  # when new category start, add category data (Series format for key-value data and DataFrame format for tabular data) to 'dict_data_block' with 'str_name_category'
                if len(l_l_value) > 0:
                    str_category = (
                    ...
                    dict_data_block[str_name_category] = (
                        pd.read_csv(
                            StringIO(str_category),
                            sep="\t",
                            low_memory=False,
                            quoting=csv.QUOTE_NONE,
                            keep_default_na=False,
                        )
                        if bool_flag_datetype_category_is_tabular
                        else pd.read_csv(
                            StringIO(str_category),
                            sep="\t",
                            header=None,
                    ...
                    )  # csv.QUOTE_NONE to deal with single quotation mark inside quotation # keep_default_na = False (prevent automatic conversion of NA chain to np.nan)
                ...
                "loop_" == line[:5]
            ):  # when 'loop_' directive is used, set 'bool_flag_datetype_category_is_tabular' to True (category datatype is tabular datatype)
                ...
                line = line[1:]
                if (
                    bool_flag_datetype_category_is_tabular
                ):  # if category datatype is tabular, retrieve attribute name
                    ...
                    l_l_value.append(
                        list(
                            value.replace("\t", " ")
                            for value in PARSE_Empty_Space_Delimited_with_Quotations(
                                line
                            )
                        )
                    )  # split a line into values using empty-space delimiter while ignoring delimiters inside quotations. # remove '\t' from value to be compativle with parsing method
                    str_name_category, l_l_value[-1][0] = l_l_value[-1][0].split(
                        "."
                    )  # retrieve category name and attribute name from the current CIF token name
            elif line[0] == ";":
                str_spanning_multiple_line = line[1:].replace(
                    "\t", " "
                )  # when the start of a multiple-lines-spanning string is detected
            elif bool_flag_datetype_category_is_tabular:  # for tabular category data
                l_value = list(
                    value.replace("\t", " ")
                    for value in PARSE_Empty_Space_Delimited_with_Quotations(line)
                )  # split a line into values using empty-space delimiter while ignoring delimiters inside quotations. # remove '\t' from value to be compativle with parsing method
                if len(l_l_value[0]) == len(l_l_value[-1]):
                    l_l_value.append(
                        l_value
                    )  # if the number of values in the previous row is the same as the number of attribute names, add the parsed row as a new row
                else:
                    l_l_value[-1].extend(
                        l_value
                    )  # if the number of values in the previous row is not equal to the number of attribute names, append the parsed row to the current row (these values of the same row are separated by a multiple-line-spanning value)
            else:
                l_l_value[-1].append(
                    line.replace("\t", " ")
                )  # for key-value category data, add a value spanning an entire line to the current row # remove '\t' from value to be compativle with parsing method
        if (
            len(l_l_value) > 0
        ):  # when file ends, add category data to 'dict_data_block', and add data_block to 'dict_cif'
            str_category = (
                "\n".join(list("\t".join(l_value) for l_value in l_l_value)) + "\n"
            )  # converting data to a 'file' and parse it using pandas.read_csv allows automatic data type detection and conversion when building pandas data
            dict_data_block[str_name_category] = (
                pd.read_csv(StringIO(str_category), sep="\t", low_memory=False)
                if bool_flag_datetype_category_is_tabular
                else pd.read_csv(
                    StringIO(str_category),
                    sep="\t",
        ...
        for str_name_data_block in dict_cif:  # for each data block
            ...
            for str_name_category in dict_data_block:  # for each category
                ...
                data = dict_data_block[
                    str_name_category
                ]  # retrive category data (either Series or DataFrame)
                if isinstance(data, pd.Series):
                    ...
                    file.write(
                        "\n".join(
                            ...
                            + str_name_category
                            + "."
                            + data.columns.values.astype(object)
                        )
                        + "\n"
                ...
                str_format = "".join(
                    list(
                        "{:<" + str(int_column_width) + "s}"
                        for int_column_width in np.array(
                            list(
                                list(len(value) for value in arr_value)
                                for arr_value in arr
                            )
                        ).max(axis=0)
                        + (3 if isinstance(data, pd.Series) else 1)
                    )
                )  # retrive str_format by using maximum column_width for each column. minimum empty space for Series data is '   ', while that for tabular data is ' '
                file.write(
...
def CIF_from_PDB(df, l_label_entity_id=None):  # 2020-08-08 15:21:53
    ...
    l_col_required = [
        ...
        "Cartn_z",
        "auth_asym_id",
    ]  # necessary columns in a mmCIF format file
    df.rename(
        columns={
            "ATOM_or_HETATM": "group_PDB",
            "Atom_serial_number": "id",
            "Atom_name": "label_atom_id",
            "Alternate_location_indicator": "label_alt_id",
            "Residue_name": "label_comp_id",
            "Chain_identifier": "label_asym_id",
            "Residue_sequence_number": "label_seq_id",
            "X_orthogonal_Å_coordinate": "Cartn_x",
            "Y_orthogonal_Å_coordinate": "Cartn_y",
            "Z_orthogonal_Å_coordinate": "Cartn_z",
        },
        inplace=True,
    )
    ...
    df["label_entity_id"] = (
        1 if l_label_entity_id is None else l_label_entity_id
    )  # default value of l_label_entity_id is 1
...
def DSSP_Read_mkdssp_Result(
    path_file, flag_retrieve_coords=False
):  # 2020-07-06 17:09:09
    """# 2022-05-14 15:08:47
    ...
    if isinstance(path_file, (str)):
        ...
        if "." in name_file:  # if name_file has a file extension
            ...
            if (
                str_file_extension == "pdb" or str_file_extension == "cif"
            ):  # if a given file has a file extension for protein structures (an input of MKDSSP program)
                content = subprocess.run(
                    ["mkdssp", "-i", path_file], capture_output=True
                ).stdout.decode()  # run mkdssp program to read the output of mkdssp program
        if (
            content is None
        ):  # if given file appears to not contain protein structures, read output files
            with open(path_file, "r") as file:
                content = file.read()
    ...
    if flag_retrieve_coords:
        for line in l_line[28:-1]:
            try:
                ...
                )  # retrieve valid line in the dssp output file
            except:
                continue
    else:
        for line in l_line[28:-1]:
    ...
    df = pd.DataFrame(
        ...
        + (["x", "y", "z"] if flag_retrieve_coords else []),
    )
    df["Structure_Simple"] = list(
        structure[0] if structure[0] != " " else "C" for structure in df.structure
    )  # retrive one letter representation of a secondary structure

    dict_amino_acid_to_MaxASA__Tien_et_al__2013__emp__ = {
        "A": 121.0,
        "R": 265.0,
        "N": 187.0,
        "D": 187.0,
    ...
    df["relative_surface_accessibility"] = list(
        min(
            1,
            max(
                0,
                record["accessibility"]
                / dict_amino_acid_to_maxasa[record["amino_acid"]],
            ),
        )
...
def Bookshelves_pipeline_run_a_function_in_command_line_as_a_script(
    func, path_folder, title, import_bookshelves=True
):
    """# 2021-01-04 04:18:02
    ...
    if (
        import_bookshelves
    ):  # update bookshelves python script if 'import_bookshelves' is set to True
        os.system(
            f"jupyter nbconvert --to script {path_folder_code_bookshelves}Bookshelves.ipynb"
        )
    ...
    with open(path_file_code, "w") as newfile:
        if import_bookshelves:
            with open(f"{path_folder_code_bookshelves}Bookshelves.py", "r") as file:
                str_bookshelves = file.read()
...
def Base64_Encode(path_file_binary, path_file_binary_base64=None, header=None):
    ...
    """
    path_file_binary_base64 = (
        path_file_binary + ".base64.txt"
        if path_file_binary_base64 is None
        else path_file_binary_base64
    )
...
def Base64_Decode(path_file_binary_base64, path_file_binary=None):
    ...
    """
    path_file_binary = (
        path_file_binary_base64 + ".binary"
        if path_file_binary is None
        else path_file_binary
    )
...
def Correlation_Matrix_Pearsonr(arr, dtype=None):
    ...
    arr_mean_centered_divided_by_norm = arr_mean_centered / np.vstack(
        list(scipy.linalg.norm(a) for a in arr_mean_centered)
    )
    arr_r = np.matmul(
        arr_mean_centered_divided_by_norm, arr_mean_centered_divided_by_norm.T
    )
    ...
    ab = n / 2 - 1
    arr_prob = np.array(
        list(scipy.special.btdtr(ab, ab, e) for e in (1 - np.abs(arr_r.ravel())) / 2),
        dtype=dtype,
    ).reshape(arr_r.shape)
...
def Pearsonr(
    ...
    normxm=None,
    normym=None,
    xm_divided_by_normxm=None,
    ym_divided_by_normym=None,
    dtype=None,
...
def SRA_Retrieve_Info(path_glob_SraRunTable, path_glob_xml):
    ...
    df_metadata = pd.concat(
        list(pd.read_csv(path_file) for path_file in glob.glob(path_glob_SraRunTable))
...
class Offload_Works:
    ...
    def collect_results(self, flag_return_results: bool = False):
        ...
        if flag_return_results:
            ...
            self._dict_res = (
                dict()
            )  # initialize the internal container for saving results
            return dict_result
        else:
            return int_num_collected_results

    def submit_work(
        self, func, args: tuple = (), kwargs: dict = dict(), associated_data=None
    ):
        """# 2023-08-12 16:59:57
        submit a work
        ...
        def __get_result():
            ...
            return (
                ...
                else {"result": res, "associated_data": associated_data}
            )
    ...
    def wait(self, str_uuid_work: str):
        ...
        if str_uuid_work in self._dict_worker:
            ...
            self._dict_res[str_uuid_work] = worker[
                "get_result"
            ]()  # wait until the result become available and collect and store the result
            worker["process"].join()  # dismiss the worker process
            del self._dict_worker[str_uuid_work]  # delete the record of the worker
